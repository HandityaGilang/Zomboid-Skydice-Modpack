...
$BuilderVersion = "2.0"
$DrivePath = "Mods/Lifestyle/common/media/texturepacks/LS_Artwork.pack"
...
for ($i = 0; $i -lt $files.Count; $i++) {
    ...
    $url = if ($relativePath -eq $DrivePath) {
        ...
        # Path lokal/client tetap Mods/..., tetapi URL GitHub harus mods/... lowercase.
        $rawRelativePath = $relativePath
        if ($rawRelativePath.StartsWith("Mods/", [StringComparison]::OrdinalIgnoreCase)) {
            $rawRelativePath = "mods/" + $rawRelativePath.Substring(5)
        }
        Get-RawUrl -RawBase $RawBase -RelativePath $rawRelativePath
    }
