...
local function onPreFillWorldObjectContextMenu(player, context, worldobjects, test)
    ...
    local isoPipe = WPIso.GetPipe(square)
    if isoPipe then
        local pipe = WPVirtual.PipeGet(sx, sy, sz)
        if pipe then
            print ("[DEBUG] Pipe at " .. sx .. "," .. sy .. "," .. sz .. " s: " .. pipe.s .. ", amount: " .. pipe.w)
        end
    end

    -- BUILDING DEBUG
    local isoBuilding = WPIso.GetBuilding(square)
    if isoBuilding then
        local building = WPVirtual.BuildingGet(sx, sy, sz)
        if building then
            print ("[DEBUG] Building at " .. sx .. "," .. sy .. "," .. sz )
        end
    end
end
