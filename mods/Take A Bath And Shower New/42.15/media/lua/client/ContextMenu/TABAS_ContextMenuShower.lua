...
function TABAS_ContextMenuShower.createMenu(player, showerObj, context, worldObjects, test)
...
local function getSoapsInInventory(playerObj, remaining)
    local playerInv = playerObj:getInventory()
    local soapList = {}
    local total = remaining or 0
    for soapType,_ in pairs(TABAS_Utils.SoapTypes) do
        local list = playerInv:getItemsFromType(soapType, true)
...
local function getSoapsOnSquare(square, remaining)
    local worldObjects = square:getWorldObjects()
    local soapList = {}
    local total = remaining or 0
    for i=0, worldObjects:size()- 1 do
        local item = worldObjects:get(i):getItem()
        if item and item:IsDrainable() and TABAS_Utils.SoapTypes[item:getType()] then
            soapList[#soapList + 1] = item
            total = total + item:getCurrentUses()
        end
    end
...
local function soapListToId(list)
    local ids = {}
    for i=1, #list do
        local soap = list[i]
        if soap then ids[#ids + 1] = soap:getID() end
    end
    return ids
end
...
function TABAS_ContextMenuShower.doShowerMenu(player, object, context, inBath)
    ...
    if soapRequired > 1 then
        soapList1, soapRemaining = getSoapsInInventory(playerObj, soapRemaining)
        soapList2, soapRemaining = getSoapsOnSquare(object:getSquare(), soapRemaining)
        -- World items are converted to ID.
        soapList2 = soapListToId(soapList2)
    end
...
function TABAS_ContextMenuShower.onTakeShower(player, object, soapList1, soapList2, comsumeSoap, towel, keepClothes, makeOff, useHot)
    ...
    TABAS_UnEquipItems.doUnequip(player, keepClothes, nil, prepSq)

    ISTimedActionQueue.add(ISWalkToTimedAction:new(playerObj, objectSq))

    ISTimedActionQueue.add(TABAS_TakeShower:new(playerObj, object, objectSq, facing, soapList1, soapList2, comsumeSoap, makeOff, useHot, isInTub))
...
function TABAS_ContextMenuShower.onTakeShowerInBath(player, object, soapList1, soapList2, comsumeSoap, towel, keepClothes, makeOff, useHot)
    ...
    TABAS_UnEquipItems.doUnequip(player, keepClothes, true, prepSq)

    -- local climbObj = TABAS_Iso.getBathObjectOnSquare(targetSq)
    -- Required walk to a precise location for climb over the tub.
    if not TABAS_MoveUtils.onEnterTheTub(playerObj, baseSq, prepSq, targetSq) then
        return
    end
