...
function TABAS_ShowerOptionPanel:onClick(_btn)
    ...
    if _btn.internal == "TAKESHOWER" and self._mainPanel then
        ...
        if mainPanel.isTakingBath then
            TakeShowerContext.onTakeShowerInBath(player, self.tfc_Base.bathObject, nil, nil, nil, nil, nil, nil, useHot)
        else
...
function TABAS_ShowerOptionPanel:refreshShowerPanel(availabilityState)
    ...
    local canUseShower = (not bathingPhaseStarted) and (not using) and piped and (not tooHot) and (not notEnoughMin)
