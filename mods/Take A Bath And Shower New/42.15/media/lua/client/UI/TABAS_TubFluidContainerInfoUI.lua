...
local BATHING_PHASE_ACTION_TEXT_KEYS = {
    ISWalkToTimedAction = "IGUI_TABAS_Bathing_Preparing",
    ISWalkToTimedActionF = "IGUI_TABAS_Bathing_Preparing",
...
function TABAS_TubFluidContainerInfoUI:createChildren()
    ...
    self.bathSessionPanel = TABAS_BathSessionPanel:new(
        self.entryPanel:getX(),
        self.autoBathTimePanel:getY(),
        self.entryPanel:getWidth(),
        self.entryPanel:getBottom() - self.autoBathTimePanel:getY(),
        self.playerObj,
        self.tfc_Base
    )
...
function TABAS_TubFluidContainerInfoUI:resolveBathingPhaseState()
    ...
    local hasAnchorAction = curActionType and BATHING_PHASE_ANCHOR_ACTIONS[curActionType] == true

    if takingBath or isBathing or hasAnchorAction then
        playerState.bathingPhaseStarted = true
    elseif playerState.bathingPhaseStarted then
        playerState.bathingPhaseStarted = nil
    end

    local textKey = curActionType and BATHING_PHASE_ACTION_TEXT_KEYS[curActionType]
    if not textKey then
        if takingBath then
...
function TABAS_TubFluidContainerInfoUI:checkAvailability()
    ...
    self.tfcPanel.notAvailable = unavailable
    self.temperaturePanel.notAvailable = unavailable
...
function TABAS_TubFluidContainerInfoUI:onGainJoypadFocus(joypadData)
    ...
    self:insertNewLineOfButtons(self.actionPanel.btnBathActions, self.actionPanel.btnTakeBath, self.actionPanel.btnConfig)
