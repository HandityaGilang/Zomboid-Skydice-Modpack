...
function TABAS_Utils.isNotExcludedClothing(item, excludeBathWear, ignoreOptions)
    ...
    if not bodyLocation and not item:getAttachmentReplacement() then
        return false
    end
    ...
    if excludeBathWear then
        local excludeItemType = TABAS_BodyLocations.Exclude.BathWear
    ...
    if item:getAttachmentReplacement() then return true end -- is Bag
    ...
    elseif (item:getAttachmentsProvided() and item:getActualWeight() <= 0.5) and (TABAS_Utils.ModOptionsValue("NotTakeOff_Belts") or ignoreOptions) then
        ...
        if TABAS_Utils.ModOptionsValue("NotTakeOff_Glasses") or ignoreOptions then
