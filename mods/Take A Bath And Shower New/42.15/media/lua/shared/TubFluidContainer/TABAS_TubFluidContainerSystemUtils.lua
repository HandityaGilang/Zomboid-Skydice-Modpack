...
function TFC_Utils.getTfcBaseOnServer(x, y, z, _bathObject)
    ...
    if not TFC_SBathtub then
        TFC_SBathtub = require("TubFluidContainer/TABAS_STubFluidContainerBathtub")
    end
    local tfc_Base = TFC_SBathtub:new(x, y, z, _bathObject)
    if tfc_Base then return tfc_Base end
    return nil
end

function TFC_Utils.getTfcBaseOnClient(x, y, z, _bathObject)
    if isServer() then return end
    if not TFC_CBathtub then
        TFC_CBathtub = require("TubFluidContainer/TABAS_CTubFluidContainerBathtub")
    end
    local tfc_Base = TFC_CBathtub:new(x, y, z, _bathObject)
    if tfc_Base then return tfc_Base end
    return nil
end
