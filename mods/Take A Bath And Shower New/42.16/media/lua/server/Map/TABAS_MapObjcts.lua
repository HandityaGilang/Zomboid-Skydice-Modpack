...
-- for Melos Tiles Bathtub
local ModBathSprites = require("Compat/TABAS_ModBathSprites")
local melosSprites = ModBathSprites.MelosTiles

if melosSprites and melosSprites.enabled and getActivatedMods():contains(melosSprites.modDir) then
    for _, def in ipairs(melosSprites.Bathtub) do
        local sheet, base = def.sheet, def.base
        MapObjects.OnLoadWithSprite(sheet .. "_" .. base, OnLoadBathObjectTub, PRIORITY)
        MapObjects.OnLoadWithSprite(sheet .. "_" .. (base + 1), OnLoadBathObjectFaucet, PRIORITY)
        MapObjects.OnLoadWithSprite(sheet .. "_" .. (base + 2), OnLoadBathObjectFaucet, PRIORITY)
        MapObjects.OnLoadWithSprite(sheet .. "_" .. (base + 3), OnLoadBathObjectTub, PRIORITY)
        MapObjects.OnLoadWithSprite(sheet .. "_" .. (base + 4), OnLoadBathObjectFaucet, PRIORITY)
        MapObjects.OnLoadWithSprite(sheet .. "_" .. (base + 5), OnLoadBathObjectTub, PRIORITY)
        MapObjects.OnLoadWithSprite(sheet .. "_" .. (base + 6), OnLoadBathObjectTub, PRIORITY)
        MapObjects.OnLoadWithSprite(sheet .. "_" .. (base + 7), OnLoadBathObjectFaucet, PRIORITY)
    end
end
