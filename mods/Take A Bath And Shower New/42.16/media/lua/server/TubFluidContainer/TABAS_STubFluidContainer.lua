...
local function removeTfcObject(tfcObject, bathObject)
    ...
    if bathObject and bathObject:haveSpecialTooltip() then
...
function STubFluidContainer:create()
    ...
    if self:hasLinked() then
        ...
        local linkedSquare = self:getLinkedSquare()
        if self.linkedBathObject and linkedSquare then
            self.linkedTfcObject = createTfcObject(self.linkedBathObject, linkedSquare, t.tub.empty)
            if self.linkedTfcObject then
            ...
            TFC_Utils.noise("create linked invalid", TFC_Utils.formatCoords(self.linkedX, self.linkedY, self.z))
        end
    end
