...
function TABAS_ContextMenuBathShared.addTubFluidContainerMenu(player, tfc_Base, subMenu, ctx, opts)
    ...
    if ctx.hasTfc then
        ...
        if isEnabled(opts, "vanillaFluid") then
            TABAS_Common.vanillaFluidMenu(player, tfc_Base.tfcObject, tfcSubMenu)
        end
...
function TABAS_ContextMenuBathShared.addFaucetMenu(player, faucetObj, subMenu)
    if not faucetObj or not subMenu then return end
    ...
    local hasActualWaterSource = (waterSourceCount or 0) > 0
    if not hasActualWaterSource then
    ...
    TABAS_Common.vanillaFluidMenu(player, faucetObj, faucetSubMenu)
end
