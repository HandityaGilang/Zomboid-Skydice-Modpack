...
local TABAS_VanillaFluid = require("ContextMenu/TABAS_ContextMenuVanillaFluid")
...
function TABAS_ContextMenuCommon.vanillaFluidMenu(player, bathingObj, context, worldObjects)
    if not bathingObj or not context then return end

    local playerObj = getSpecificPlayer(player)
    if not playerObj then return end

    if bathingObj:hasWater() then
        TABAS_VanillaFluid.doDrinkMenu(player, bathingObj, context, playerObj, worldObjects)
        TABAS_VanillaFluid.doFillMenu(player, bathingObj, context, playerObj, worldObjects)
        TABAS_VanillaFluid.doWashMenu(player, bathingObj, context, playerObj)
    end
    -- local fluidContainer = bathingObj:getFluidContainer()
    -- TABAS_VanillaFluid.doEmptyMenu(player, fluidContainer, context)
end
