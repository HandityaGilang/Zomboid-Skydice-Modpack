...
local BATHING_TYPES = {"Bathtub", "Shower"}
...
function TABAS_Iso.findBathingObjectOnSquare(square)
    ...
    for i=1, square:getObjects():size() do
        ...
        if not object:isFloor() then
            ...
            if spriteName then
                for j=1, #BATHING_TYPES do
                    local type = BATHING_TYPES[j]
                    local map = maps[type]
...
function TABAS_Iso.isObjectOfSpriteType(object, typeKey)
    if not object or not instanceof(object, "IsoObject") then return false end
    local spriteName = object:getSpriteName()
    if not spriteName then return false end

    local map = TABAS_Iso.getSpritesTable("Index", "ModelTypeBySprite", typeKey)
    return map and (map[spriteName] ~= nil)
end

function TABAS_Iso.isBathObject(object)
    return TABAS_Iso.isObjectOfSpriteType(object, "Bathtub")
end
...
function TABAS_Iso.isShowerObject(object, allowedOnBath)
    ...
    return TABAS_Iso.isObjectOfSpriteType(object, "Shower")
end
