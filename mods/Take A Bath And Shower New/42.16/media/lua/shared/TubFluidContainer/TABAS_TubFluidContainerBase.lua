...
function TFC_Base:confirmLinkedObject()
    ...
    if (not self.linkedBathObject) and (not self._restoredFromRegistered) then
        return false
    end
    if (self.tfcObject and not self.linkedTfcObject) or (not self.tfcObject and self.linkedTfcObject) then
        return false
    end
    return true
end
...
function TFC_Base:getLinkedSquare()
    ...
    if (not self.linkedSquare) or self._linkedSquareX ~= self.linkedX or self._linkedSquareY ~= self.linkedY then
        self.linkedSquare = getCell():getGridSquare(self.linkedX, self.linkedY, self.z)
        self._linkedSquareX = self.linkedX
        self._linkedSquareY = self.linkedY
    end
...
function TFC_Base:getTfcObject()
    ...
    local linkedObj = self:hasLinked() and TFC_Utils.getTfcObject(self.linkedX, self.linkedY, self.z) or nil
    return mainObj, linkedObj
end

function TFC_Base:refreshTfcRefs()
    local mainObj, linkedObj = self:getTfcObject()
    local mainChanged = mainObj ~= self.tfcObject
    local linkedChanged = linkedObj ~= self.linkedTfcObject

    if mainChanged or linkedChanged then
...
function TFC_Base:restoreRegisteredState()
    local data = self:getRegisteredData()
    if not data then
        self._restoredFromRegistered = false
        return false
    end

    self._restoredFromRegistered = true

    if data.facing then
        self.facing = data.facing
    end
    if data.linkedX ~= nil and data.linkedY ~= nil then
        self.linkedX = data.linkedX
        self.linkedY = data.linkedY
    end
    self:resolveLinkedBathObject()
    return true
end

function TFC_Base:resolveLinkedBathObject()
    if not self:hasLinked() then
        self.linkedBathObject = nil
        self.linkedSquare = nil
        self._linkedSquareX = nil
        self._linkedSquareY = nil
        return nil
    end

    local square = self:getLinkedSquare()
    self.linkedBathObject = square and TABAS_Iso.getBathObjectOnSquare(square) or nil
...
function TFC_Base:hasLinked()
    return self.linkedX ~= 0 and self.linkedY ~= 0
end
