...
function LSMirrorMenu:onConfirmChanges(button)
	...
	if isClient() then
		...
		sendClientCommand(self.character, "LS", "SetMirrorMakeup", {{self.hairDyeItem, self.beardDyeItem, self.itemsList.MakeupTattooNeedle, self.acidBrush and self.itemsList.MakeupTattooBrush}, makeupData})
	end
...
function LSMirrorMenu:initialise()
	...
	if false and getActivatedMods():contains("Ellie'sTattooParlor") or getActivatedMods():contains("ElliesTattooParlor[RF6]") then -- disabled for now
