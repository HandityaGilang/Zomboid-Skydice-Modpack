...
local function applyGameModOptions(force)
	...
	if not force
	and not MHP_OptionsDirty
	and not MHP_ShouldRun(MHP_LastOptionsApplyTick, MHP_OPTIONS_SAFETY_INTERVAL, false) then
		return false
	end
...
local function ensureMiniHealthForPlayer(playerObj)
	...
	applyGameModOptions(force or createdOrChanged)
end
