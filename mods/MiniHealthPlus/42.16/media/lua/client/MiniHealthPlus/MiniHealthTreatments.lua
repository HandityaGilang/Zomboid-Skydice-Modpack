...
MHP.MiniHealthTreatment = MHP.MiniHealthTreatment or {}
...
function BaseHandler:isInjured()
	...
	if bodyPart:getFractureTime() > 0 and not hasOtherWound then
		return false
	end

	return bodyPart:HasInjury() or bodyPart:stitched() or bodyPart:getSplintFactor() > 0
end
...
function BaseHandler:getDoctor()
    return getMHPHandle() and getMHPHandle().player
end
...
function HReplaceBandage:addToMenu(context, optionsCount)
    if type(MHP_SB_IsReplaceBandageEnabled) == "function" and not MHP_SB_IsReplaceBandageEnabled() then
        return optionsCount
    end

    local types = self:getAllItemTypes(self.items.ITEMS)

    if #types > 0 and self.bodyPart:bandaged() and self.bodyPart:getBandageLife() <= 0 then
        optionsCount = optionsCount + 1

        local optionText = getText("UI_MHP_ReplaceBandage")
        local option = context:addOption(optionText, nil)
        local subMenu = context:getNew(context)
        context:addSubMenu(option, subMenu)
        ...
        for i = 1, #types do
            local item = self:getItemOfType(self.items.ITEMS, types[i])
            local subOption = subMenu:addOption(item:getName(), self, self.onMenuOptionSelected, types[i])
...
function HReplaceBandage:isValid(itemType)
    ...
    return self.bodyPart:bandaged()
        and self.bodyPart:getBandageLife() <= 0
        and self:getItemOfType(self.items.ITEMS, itemType) ~= nil
end
...
function HReplaceBandage:perform(previousAction, itemType)
    ...
    local removeAction = ISApplyBandage:new(self:getDoctor(), self:getPatient(), nil, self.bodyPart, false)
    ISTimedActionQueue.addAfter(previousAction, removeAction)

    local afterTransfer = self:toPlayerInventory(item, removeAction)

    local addAction = ISApplyBandage:new(self:getDoctor(), self:getPatient(), item, self.bodyPart, true)
    ISTimedActionQueue.addAfter(afterTransfer, addAction)
end
...
function MiniHealthTreatment:checkItems(handlers)
    local containers = ISInventoryPaneContextMenu.getContainers(getMHPHandle() and getMHPHandle().player)
    local done = {}
    local childContainers = {}
    for i=1,containers:size() do
        local container = containers:get(i-1)
...
function MiniHealthTreatment:doBodyPartContextMenu(bodyPart, context)
    local playerNum = getMHPHandle() and getMHPHandle().playerIndex
    ...
    if optionsCount <= 0 then
        context:addOption(getText("UI_MHP_NoActionAvailable"))
    end
end

MiniHealthTreatment._MiniHealthOwn_doBodyPartContextMenu = MiniHealthTreatment.doBodyPartContextMenu

function MiniHealthTreatment.restoreOwnContextMenu()
	if MiniHealthTreatment._MiniHealthOwn_doBodyPartContextMenu then
		MiniHealthTreatment.doBodyPartContextMenu = MiniHealthTreatment._MiniHealthOwn_doBodyPartContextMenu
	end
end