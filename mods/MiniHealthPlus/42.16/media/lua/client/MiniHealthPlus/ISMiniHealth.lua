...
local function MHP_BodyPartNeedsTreatment(bodyPart, includeStitchInspect)
	...
	if bodyPart:getDeepWoundTime() > 0
	or bodyPart:deepWounded()
	or bodyPart:haveGlass()
	or bodyPart:haveBullet()
	or bodyPart:isInfectedWound() then
		return true
	end
	...
	return hasBasicWound and not hasBandage
end
...
function MHPPanel.playerNeedsTreatment(playerObj)
	...
	local includeStitchInspect = true
	if type(MHP_SB_IsStitchInspectIndicatorEnabled) == "function" then
		includeStitchInspect = MHP_SB_IsStitchInspectIndicatorEnabled() == true
	end

	for i = 0, 16 do
		if MHP_BodyPartNeedsTreatment(bodyParts:get(i), includeStitchInspect) then
			return true
		end
	end
...
local function MHP_BodyPartNeedsDisplayAttention(bodyPart, includeStrains, showSafeRemoveIndicator, showStitchInspectIndicator)
	...
	if MHP_BodyPartNeedsTreatment(bodyPart, showStitchInspectIndicator) then
		return true
	end
...
local function MHP_IsMiniHealthOwnedBCIPopup(popup)
	...
	return popup._MHP_OpenedAsIdleAvatar == true
		and popup._MHP_KeepOpenWithCharacterInfo == true
...
local function MHP_ForceRemoveBCIPopup(popup)
	...
	popup._MHP_KeepOpenWithCharacterInfo = false
...
local function MHP_PatchBCIPopupForMiniHealth(popup, ownerPanel)
	...
	popup._MHP_KeepOpenWithCharacterInfo = true
...
local function MHP_OpenBCIIdleAvatar(playerIndex, playerObj, ownerPanel)
	local api = MHP_GetBCIFloatingAvatarAPI()
	if not api then
		return nil
	end

	playerIndex = playerIndex or (playerObj and playerObj.getPlayerNum and playerObj:getPlayerNum()) or 0
	playerObj = playerObj or getSpecificPlayer(playerIndex)
	...
	if playerObj.isDead and playerObj:isDead() then
		MHP_EnforceSingleBCIIdleAvatar(playerIndex, nil)
		return nil
	end

	local existing = api._instances and api._instances[playerIndex] or nil
	...
	if existing then
		MHP_PatchBCIPopupForMiniHealth(existing, ownerPanel)
		existing:setVisible(true)
		MHP_AttachBCISettingsButton(existing, ownerPanel)

		MHP_ApplyBCIPopupZOrder(existing, ownerPanel)

		MHP_EnforceSingleBCIIdleAvatar(playerIndex, existing)
		return existing
	end

	MHP_EnforceSingleBCIIdleAvatar(playerIndex, nil)

	local info = getPlayerInfoPanel and getPlayerInfoPanel(playerIndex) or nil
	local originalInfoClose = info and info.close or nil

	if info and type(originalInfoClose) == "function" then
		info.close = function() end
	end

	local popup = api.openForPlayer(playerIndex, playerObj, {
		persistActive = false,
	...
	if popup then
		MHP_PatchBCIPopupForMiniHealth(popup, ownerPanel)
		MHP_AttachBCISettingsButton(popup, ownerPanel)
		MHP_ApplyBCIPopupZOrder(popup, ownerPanel)
		MHP_EnforceSingleBCIIdleAvatar(playerIndex, popup)
	else
		MHP_EnforceSingleBCIIdleAvatar(playerIndex, nil)
	end
...
function MHPPanel:prerender()
	...
	self:drawTextureScaled(self.backgroundTex[self.isFemale], bodyX, 0, bodyW, bodyH, 0.5 * self.alpha, 0, 0, 0)

	local outlineColor = (self.health / 80) - 0.2
	self:drawTextureScaled(self.outlineTex[self.isFemale], bodyX, 0, bodyW, bodyH, 1 * self.alpha, 1, outlineColor, outlineColor)
...
function MHPPanel:update()
	...
	if totalHealth ~= self.previousHealth then
		self.hideTimer = 40
	...
	if self.showBCIAvatarOnIdle == true
	and (
...
function MHPPanel:refreshGameModOptionVisibility(enabled, onlyShowTreatmentNeeded)
	...
	local bciIdleActive = self.showBCIAvatarOnIdle == true
		and type(TwisTonFire_MHP_IsBetterCharacterInfoActive) == "function"
		and TwisTonFire_MHP_IsBetterCharacterInfoActive() == true
	if bciIdleActive then
		MHP_EnforceSingleBCIIdleAvatar(self.playerIndex, TTF_BCI_FloatingAvatarUI and TTF_BCI_FloatingAvatarUI._instances and TTF_BCI_FloatingAvatarUI._instances[self.playerIndex] or nil)
	end
	...
	if bciIdleActive then
		if onlyShowTreatmentNeeded then
			-- Game ModOption has priority:
			-- Mini Health Plus may only replace the BCI avatar when real treatment is needed.
			shouldShowMiniHealth = self.needsTreatment == true
		else
			-- Normal BCI idle behavior:
			-- Mini Health Plus replaces the avatar for wounds, blue/purple indicators, or strain attention.
			shouldShowMiniHealth = self.needsDisplayAttention == true
		end

		shouldShowIdleAvatar = not shouldShowMiniHealth

	elseif onlyShowTreatmentNeeded and not self.needsTreatment then
		shouldShowMiniHealth = false
	end
	...
	if shouldShowMiniHealth then
		...
		if self.forceVisibleForBCIAttention then
...
function MHPPanel:readConfig()
	...
	if loadedScale == nil or loadedScale < 0.75 or loadedScale > 2.0 then
		loadedScale = 1.0
	end
