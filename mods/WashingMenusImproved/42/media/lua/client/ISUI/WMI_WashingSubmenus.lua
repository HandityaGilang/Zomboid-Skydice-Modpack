...
-- Ensure the Clean Bandages submenu injection is loaded on Legacy builds.
require "ISUI/WMI_CleanBandagesContext"

-- Optional compatibility with the "Take A Bath And Shower" mod (bathtub context menu).
...
local function onWashClothing3(playerObj, sink, soapList, washList, singleClothing, noSoap, closeCtx)
    -- Move the player next to the water source if not adjacent
	...
	for _, item in ipairs(washList) do
		...
		-- Compute blood/dirt exactly like vanilla expects for Clothing
		...
		if instanceof(item, "Clothing") then
			...
			-- weapons/containers keep using their own blood metric
		...
		if originalContainer then
			ISTimedActionQueue.add(ISInventoryTransferAction:new(playerObj, item, playerObj:getInventory(), originalContainer))
		end
	end
	...
	-- was: context:closeAll()
	-- Close the VISIBLE menu, not the tmp builder
...
local function createInventorySubOption(label, list, playerObj, sink, soapList, closeCtx, noSoap, waterRemaining, soapRemaining, parentMenu)
    ...
    if ISWashClothing.GetSoapRemaining(soapList) < soapRequired then
			...
			if #washListData.invItems > 0 then
				...
				for i,item in ipairs(washListData.invItems) do
					...
					if item:IsClothing() and item:getDirtyness() > 0 then
						tooltip.description = tooltip.description .. " <RGB:1,1,1> " .. " <LINE> " .. getText("Tooltip_clothing_dirty") .. ": " .. math.ceil(item:getDirtyness()) .. " %"
					end
...
local function _isWaterObject(o)
    ...
    local okAmt, amt = pcall(function() return o:getFluidAmount() end)
    local okFC, fc   = pcall(function() return o:getFluidContainer() end)
    ...
    local okFT2, ft2 = pcall(function() return o:getFluidType() end)
    if okFT2 and (ft2 == Fluid.Water or ft2 == Fluid.TaintedWater) then
        return okAmt and type(amt) == "number" and amt >= 1
    end
...
-- Build the full WMI "Wash" submenu inside an existing ISContextMenu.
-- This is used by compatibility shims (e.g., TABAS bathtubs) that have their own container submenu.
