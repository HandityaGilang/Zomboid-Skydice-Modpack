...
-- 	this is so players with a remote equipped can interact with televisions
	if (self.player and self.deviceData:canPlayerRemoteInteract(self.player)) or self:doWalkTo() then
...
function RWMMergedTV:chooseVideo()
	...
	local function changeVideo(name)
		if name ~= self.videoName then
			self.video = getVideo(name, 160, 120)
			self.videoName = name
		end
	end
	...
	if self.deviceData then
		if self.deviceData:isPlayingMedia() then
			...
			if title == "Exposure Survival" then changeVideo("TV_203_S.bik") return
			elseif title == "Woodcraft" then changeVideo("TV_203_W.bik") return
			elseif title == "The Cook Show" then changeVideo("TV_203_C.bik") return
			elseif title == "Carzone" then changeVideo("VHS_CarZone.bik") return
			end
			changeVideo("VHS_Noise.bik")
		else
			if getGameTime():getWorldAgeHours() < 185 then
				local ch = self.deviceData:getChannel()
				if self.deviceData:isReceivingSignal() then
					if ch == 200 then changeVideo("TV_200.bik") return
					elseif ch == 201 then changeVideo("TV_201.bik") return
					elseif ch == 203 then
						local hour = getGameTime():getHour()
						if hour >= 18 then changeVideo("TV_203_S.bik") return end
						if hour >= 12 then changeVideo("TV_203_W.bik") return end
						if hour >= 6 then changeVideo("TV_203_C.bik") return end
					elseif ch == 204 then changeVideo("TV_204.bik") return
					elseif ch == 205 then changeVideo("TV_205.bik") return
					elseif ch == 206 then changeVideo("TV_206.bik") return
					elseif ch == 207 then changeVideo("TV_207.bik") return
					elseif ch == 208 then changeVideo("TV_208.bik") return
					elseif ch == 209 then changeVideo("TV_209.bik") return
					elseif ch == 210 then changeVideo("TV_210.bik") return
					end
				else
					if ch == 209 then changeVideo("TV_209.bik") return end
					changeVideo("TV_Ad.bik") return
				end
			else
				changeVideo("TV_Static.bik") return
			end
		end
	end
end
...
function RWMMergedTV:readFromObject( _player, _deviceObject, _deviceData, _deviceType )
	...
	self.canMedia = self.deviceData:getDeviceName() ~= getItemNameFromFullType("Base.TvAntique")
...
function RWMMergedTV:update()
	...
	if self.deviceData then
		...
		if self.video ~= nil and self.deviceData:getIsTurnedOn() then
			self:chooseVideo()
		end
    end
...
function RWMMergedTV:prerender()
	...
	if self.deviceData and self.deviceData:getIsTurnedOn() then
		if self.video ~= nil then
				self.video:RenderFrame()
				self:drawTextureScaled(self.video, 7, 6, 160, 120, 1, 1, 1, 1);
		else
			self:drawTexture(self.noSignal, 7, 6, 160, 120, 1, 1, 1, 1)
		end
	end
...
function RWMMergedTV:new (x, y, width, height)
	...
	o.canMedia = false
	o.video = getVideo("TV_Static.bik", 160, 120);
	o.videoName = "TV_Static.bik"
