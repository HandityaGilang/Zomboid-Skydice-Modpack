...
Hold_RemoveHiveFrameAction = ISBaseTimedAction:derive("Hold_RemoveHiveFrameAction")

function Hold_RemoveHiveFrameAction:isValid()
    if self.resource:isEmpty() then return false end
	return true
end

function Hold_RemoveHiveFrameAction:update()
	self.character:setMetabolicTarget(Metabolics.LightWork)
	self.character:faceThisObject(self.entity)
end

function Hold_RemoveHiveFrameAction:start()
    ...
    if self.frameSlot then
        self.frameSlot.actionRemove = self
    end
...
function Hold_RemoveHiveFrameAction:stop()
    ...
    if self.frameSlot then
        self.frameSlot.actionRemove = nil
    end
end

function Hold_RemoveHiveFrameAction:perform()
    self:stopSound()
    if self.item then
...
function Hold_RemoveHiveFrameAction:complete()
	...
	if removedItem then
        ...
        beehiveObject:removeFrame(self.slotIndex)
...
function Hold_RemoveHiveFrameAction:checkBeeSting()
...
function Hold_RemoveHiveFrameAction:isWearingProtectSuit()
...
function Hold_RemoveHiveFrameAction:getDuration()
    local base = 100
...
function Hold_RemoveHiveFrameAction:stopSound()
	if self.sound and self.character:getEmitter():isPlaying(self.sound) then
		self.character:stopOrTriggerSound(self.sound)
	end
end

function Hold_RemoveHiveFrameAction:new(character, entity, resource, slotIndex, item)
