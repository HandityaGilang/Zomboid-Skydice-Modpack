...
function ISInventoryPane:drawItemDetails(item, y, xoff, yoff, red)
    ...
    -- items which have uses/max TODO: not hasTag("HideRemaining")
    ...
    if instanceof(item, "HandWeapon") or
        item:hasTag(ItemTag.SHOW_CONDITION) or
        item:hasTag(ItemTag.SHARPENABLE) or
    ...
    if item:getMaxAmmo() > 0 then
        ...
        if config.showLabels == true then
            ...
            local itemKey = item:getAmmoType():getItemKey()
            local ammoItem = getScriptManager():FindItem(itemKey)
    ...
    if instanceof(item, "Food") then
        ...
        if shouldHandle == true then
            ...
            --- Traits
            if config.showNutritionist == true and getPlayer():hasTrait(CharacterTrait.NUTRITIONIST2) then
...
local function getSortFunction(item)
    ...
    elseif item:hasTag(ItemTag.SHOW_CONDITION) or item:hasTag(ItemTag.SHARPENABLE) or item:getDisplayCategory() == "Tool" then
