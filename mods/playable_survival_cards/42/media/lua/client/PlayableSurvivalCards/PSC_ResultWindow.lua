...
require "PlayableSurvivalCards/PSC_ResultMessages"
require "PlayableSurvivalCards/PSC_ResultMusic"

PSC_ResultWindow = ISCollapsableWindow:derive("PSC_ResultWindow")
...
local function opponentName(state, winner)
    ...
    return PSC.text("IGUI_PSC_YourOpponent", "Your opponent")
end

local function victoryMessage(state, winner, roundNo)
    local messages = PSC_RESULT_MESSAGES or { "You won the game!" }
    local seed = tonumber(roundNo) or 1
    for i = 1, string.len(tostring(winner or "")) do
        seed = seed + string.byte(tostring(winner), i)
    end
    local message = tostring(messages[(seed % #messages) + 1] or "You won the game!")
    return string.gsub(message, "{opponent}", opponentName(state, winner))
end

function PSC_ResultWindow:new(x, y, width, height, mode, state)
    local o = ISCollapsableWindow.new(self, x, y, width, height)
    o.mode = mode
    o.state = state
    o.title = PSC.text("IGUI_PSC_WindowTitle", "Playable UNO")
    o.resizable = false
    o.alwaysOnTop = true
    o.createdAt = nowMs()
    o.cancelled = false
...
function PSC_ResultWindow:initialise()
    ISCollapsableWindow.initialise(self)
    if self.mode == "winner" then
        PSC_ResultMusic.play(self)
    end
end

function PSC_ResultWindow:close()
    self.cancelled = true
    PSC_ResultMusic.stop(self)
    self:removeFromUIManager()
    self:setVisible(false)
end

function PSC_ResultWindow:drawConfetti(elapsed)
...
function PSC_ResultWindow:prerender()
    ...
    if self.mode == "winner" then
        ...
        if revealed then
            ...
            self:drawTextCentre(PSC.text("IGUI_PSC_ResultWinner", "WINNER"), cx + 3, 112, 0.10, 0.08, 0.04, 0.92, UIFont.Large)
            self:drawTextCentre(PSC.text("IGUI_PSC_ResultWinner", "WINNER"), cx, 108, 1.00, 0.82, 0.18, 1, UIFont.Large)
            ...
            self:drawTextCentre(PSC.text("IGUI_PSC_ResultWinner", "WINNER"), cx, 108, 0.10, 0.10, 0.10, 0.78, UIFont.Large)
            self:drawTextCentre(PSC.text("IGUI_PSC_FinalVerdict", "The final verdict approaches..."), cx, 220, 0.46, 0.45, 0.42, 1, UIFont.Small)
        end
    elseif self.mode == "loser" then
        self:drawTextCentre(PSC.text("IGUI_PSC_ResultLoser", "LOSER"), cx + 2, 112, 0.04, 0.02, 0.02, 0.95, UIFont.Large)
        self:drawTextCentre(PSC.text("IGUI_PSC_ResultLoser", "LOSER"), cx, 108, 0.76, 0.12, 0.10, 1, UIFont.Large)
        self:drawTextCentre(string.gsub(PSC.text("IGUI_PSC_WonGame", "%1 won the game."), "%%1", self.winner), cx, 205, 0.76, 0.74, 0.68, 1, UIFont.Medium)
    else
        self:drawTextCentre(PSC.text("IGUI_PSC_ResultWinner", "WINNER"), cx, 108, 1.00, 0.82, 0.18, 1, UIFont.Large)
