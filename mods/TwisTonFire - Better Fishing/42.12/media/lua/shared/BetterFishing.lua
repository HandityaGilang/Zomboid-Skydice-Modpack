...
pcall(require, "BetterFishingModOptions")
...
local function TwF_getFishSizeChancesBySkillLevel(lvl, isNearShore, fishNum)
    ...
    if _BF_IsTwF("size") and isNearShore then
        local blend = math.max(0, math.min(1, _BF_GetNum("TwF_SizeBlend", 1.0)))
        if blend > 0 then
...
local function TwF_getTemperatureParams(player)
    ...
    local coeff = _BF_GetNum("TwF_TempCoeff", 1.0)
    return { temperature = temperature, coeff = coeff }
end

local function TwF_getWeatherParams()
    local fog  = getClimateManager():getFogIntensity() >= 1
    local wind = getClimateManager():getWindPower() >= 1
    local rain = RainManager.isRaining()
    local fogC  = _BF_GetNum("TwF_FogCoeff",  1.0)
    local windC = _BF_GetNum("TwF_WindCoeff", 1.0)
    local rainC = _BF_GetNum("TwF_RainCoeff", 1.2)

    local coeff = 1.0
    if rain then       coeff = rainC
    elseif fog then    coeff = fogC
    elseif wind then   coeff = windC
    end
...
local function TwF_getTimeParams()
    ...
    local c  = _BF_GetNum("TwF_TimeCoeff", 1.4)
...
local _lastAppliedUseTw = nil
local function _applySizeTablesSelection()
    local useTw = _BF_IsTwF("sizetables")
    if useTw == _lastAppliedUseTw then return end
    if useTw then
        ...
        print("[BetterFishing] Size tables applied: Better Fishing")
    else
        ...
        print("[BetterFishing] Size tables applied: Vanilla")
    end
    _lastAppliedUseTw = useTw
end

local function BetterFishing_ApplyTablesAtGameStart()
    _lastAppliedUseTw = nil
    _applySizeTablesSelection()
end

if Events and Events.OnGameStart then
    Events.OnGameStart.Add(BetterFishing_ApplyTablesAtGameStart)
end
if Events and Events.OnCreatePlayer then
    Events.OnCreatePlayer.Add(function(playerIndex, playerObj)
        BetterFishing_ApplyTablesAtGameStart()
    end)
end

function BetterFishing_ForceApplySizeTables()
    local isDbg = false
    if type(isDebugEnabled) == "function" then
        local ok, v = pcall(isDebugEnabled); isDbg = ok and v == true
    end
    if not isDbg then
        print("[BetterFishing] Table switch is load-only. Please reload your save or return to the main menu.")
        return
    end
    _lastAppliedUseTw = nil
    _applySizeTablesSelection()
    print("[BetterFishing] (debug) Tables reapplied.")
end
