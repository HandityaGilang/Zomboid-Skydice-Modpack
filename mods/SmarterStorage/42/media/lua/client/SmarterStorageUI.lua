...
function SmarterStorageUI.OnOpenPanel(inventoryPage, targetButton)
    if SmarterStorageUI.instance == nil then
        SmarterStorageUI.instance = SmarterStorageUI:new(0, 0, 300, 500, getPlayer(), inventoryPage, targetButton)
    ...
    SmarterStorageUI.instance.targetButton = targetButton
...
function ISInventoryPage:refreshBackpacks()
    ...
    -- self.targetButton.inventory = self.inventory
end
...
function SmarterStorageUI:createChildren()
    ...
    -- NAME
    local name = self:getSmarterStorageData().name or self.inventoryPane.title or "not found"
    ...
    self.color.backgroundColor = self:getSmarterStorageData().color or {r = 0, g = 0, b = 0, a = 1}
    ...
    self.list.iconPad = 5
    self.list.textX = self.list.iconSize + (self.list.iconPad * 3)
...
function SmarterStorageUI:onColor(button)
    ...
    self.colorPicker:setPickedFunc(SmarterStorageUI.onPickedColor)
...
function SmarterStorageUI.onFilterChange(filter)
    ...
    for _, row in ipairs(list.allItems) do
        local name = row.item.name
        if string.match(string.lower(name), filterTxt) then
            list:addItem(row.text, row.item)
        end
    end
    ...
    -- for _, row in ipairs(list.allItems) do
    --     local data = row.item  -- wrapper { item=ScriptItem, tex=Texture }
    --     local txtToCheck = string.lower(data.item:getDisplayName())

    --     if checkStringPattern(filterTxt) and string.match(txtToCheck, filterTxt) then
    --         list:addItem(data.item:getDisplayName(), data)
    --     end
    -- end
end
...
function SmarterStorageUI:getItemIcon(item)
    ...
    -- local itemClass = instanceItem(item:getFullName())
    -- if not itemClass then return nil end
    -- return itemClass:getTexture()


    -- item is a ScriptItem from getAllItems()
...
function SmarterStorageUI:generateItemList()
    ...
    for i=0, allItems:size()-1 do
        ...
        if not item:getObsolete() and not item:isHidden() then
            local icon = SmarterStorageUI:getItemIcon(item)
            if icon and icon:getName() ~= "Question_On" then
                self.list:addItem(item:getDisplayName(), {
                    item = item,
    ...
    table.sort(self.list.items, function(a, b)
        return string.lower(a.item.item:getDisplayName()) < string.lower(b.item.item:getDisplayName())
    end)

    self.list.allItems = {}
    for i, row in ipairs(self.list.items) do
        self.list.allItems[i] = row
    end
end
...
function SmarterStorageUI:onClick(button)
    ...
    elseif button.internal == "ok" then
        ...
        local row = self.list.selected > 0 and
            button.parent.list.items[button.parent.list.selected]

        local item = row and row.item and row.item.item
        item = item and item:getFullName() or nil
        local color = self.color.isSelected and self.color.backgroundColor or nil
        self:setSmarterStorageData(name, item, color, self.inventoryPage)
...
function SmarterStorageUI.closeIfDifferent(targetButton, currentInventory)
    if SmarterStorageUI.instance and targetButton and targetButton.inventory ~= currentInventory then
        SmarterStorageUI.instance:setVisible(false)
        SmarterStorageUI.instance:removeFromUIManager()
        SmarterStorageUI.instance = nil
    end
end

function SmarterStorageUI:setTargetButton(targetButton)
...
function SmarterStorageUI:setSmarterStorageData(name, itemName, color, inventoryPage)
    ...
    if self.targetButton and self.targetButton.inventory and self.targetButton.inventory:getParent() then
        local modData = self.targetButton.inventory:getParent():getModData()
        ...
        self.targetButton.inventory:getParent():transmitModData()
    end
...
function SmarterStorageUI:new(x, y, width, height, player, inventoryPage, targetButton)
    local o = {}
