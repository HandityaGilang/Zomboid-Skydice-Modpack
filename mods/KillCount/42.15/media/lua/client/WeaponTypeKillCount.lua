...
function KillCountWeaponType.addToKillCount(wielder,victim,weapon,damage)
    if not wielder or not victim or not lcl.player_isLocalPlayer(wielder) then return end
    if instanceof(wielder, "IsoZombie") then return end--in MP HOST, pipe bomb kills wielder are invalid (IsoZombie)
