...
local function pickSprite(obj,container)
    ...
    local function getSpriteAttempt(itemTex,lineNumber,colorInfo,item)
        ...
        if not sprite or (sprite and sprite:hasNoTextures()) and (CL.SpriteLookup[itemTex] or CL.SpriteLookup[item:getType()]) then
            local itemLookup = CL.SpriteLookup[itemTex]
            if not itemLookup then
                itemLookup = CL.SpriteLookup[item:getType()]
                if not itemLookup then
                    itemLookup = CL.SpriteLookup[item:getPalette()]
                end
            end
...
--updates the sprites in this container
---@param dest itemContainer
function CL.UpdateSprite(dest)
    if dest then
        local obj = dest:getParent()
        if obj then
            local tex = obj:getTextureName()
            if tex and CL.textureNames[tex] then
                if tex:contains("appliances") then
                    local spriteCount,container,itemCount,hasItems,isEmpty,itemContainer,objTexture,contentsWeight,isCenterLine = obj:getAttachedAnimSpriteCount(),
                    obj:getItemContainer():getItems(),obj:getItemContainer():getItems():size(),not obj:getItemContainer():isEmpty(),
                    obj:getItemContainer():isEmpty(),obj:getItemContainer(),obj:getTextureName(),dest:getContentsWeight(),
                    string.find(tex,"appliances_laundry_01_30") or string.find(tex,"appliances_laundry_01_31")
                    ...
                    elseif hasItems and (spriteCount ~= itemCount) then
                        ...
                        if spriteList then
                            ...
                            if #spriteList == 1 then
                                ...
                                obj:getAttachedAnimSprite():add(spriteList[1]:newInstance())
                               ...
                               -- CLDebug(obj:getTextureName() .. ": addAttachedAnimSprite" .. ": "..spriteList[1]:getName())
                                ...
                                obj:getAttachedAnimSprite():add(spriteList[r]:newInstance())
...
local originalInventoryTransferPerform = ISInventoryTransferAction.perform

--make it only change in inventory transfer
function ISInventoryTransferAction:perform()
    originalInventoryTransferPerform(self)
    local conts = {self.destContainer,self.srcContainer}
    for _, cont in pairs(conts)do
        if cont then
            ...
            if obj then
                local tex = obj:getTextureName()
                if tex and CL.textureNames[tex] then
                    if string.find(tex,"appliances") then
                        CL.UpdateSprite(cont)
                    end
                end
            end
        end
