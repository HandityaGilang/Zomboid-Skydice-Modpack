...
FA.loadMachineTemp = function(Machine)
	...
	if Machine:getSprite():getProperties():get("CustomName") == "Machine" then
		if Machine:getSprite():getProperties():get("GroupName") == "Small Soda" then
			Machine:getContainer():setCustomTemperature(0.25)
			Machine:getModData()['CustomTemp'] = "Cold" -- set to activate cold mode by default
		elseif Machine:getSprite():getProperties():get("GroupName") == "Large" then
