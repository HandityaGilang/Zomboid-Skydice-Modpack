...
function UseSodaFountain:complete()
	...
	if self.jug then
      		self.jug:syncItemFields()
        	sendItemStats(self.jug)
	end

	if self.syrup then
        	self.syrup:syncItemFields()
       		sendItemStats(self.syrup)
	end

       	self.tank:syncItemFields()
       	sendItemStats(self.tank)

        self.container:syncItemFields()
        sendItemStats(self.container)

	return true
end

function UseSodaFountain:new(character, machine, soundFile, container, syrup, tank, jug)
	local o = {}
	setmetatable(o, self)
	self.__index = self
