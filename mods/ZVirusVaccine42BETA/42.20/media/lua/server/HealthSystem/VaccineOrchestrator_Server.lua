...
print("[ZVirusVaccine] ========================================")
print("[ZVirusVaccine] MOD VERSION FOR: B42.14 to B42.19 - August 04, 2026")
...
local waterPurificationEventsRegistered = false

local function RegisterWaterPurificationEvent()
    if waterPurificationEventsRegistered then 
        return 
    end
    
    if not SandboxVars.ZombieVirusVaccineBETA then
        Events.OnTick.Add(function()
            Events.OnTick.Remove(RegisterWaterPurificationEvent)
            RegisterWaterPurificationEvent()
        end)
        return
    end
    
    if LabSandboxOptions.IsWaterPurificationEnabled() and LabWaterPurification and LabWaterPurification.ProcessWaterPurification then
        Events.EveryTenMinutes.Add(LabWaterPurification.ProcessWaterPurification)
        print("[ZVirusVaccine] Water Purification Event Enabled")
    end
    
    waterPurificationEventsRegistered = true
end
...
Events.OnGameStart.Add(RegisterWaterPurificationEvent)

if isServer() then
    print("[ZVirusVaccine] ========================================")
    print("[ZVirusVaccine] Registering Blood Aging Event on server...")
    Events.OnServerStarted.Add(RegisterBloodAgingEvent)
    print("[ZVirusVaccine] Registering Water Purification Event on server...")
    Events.OnServerStarted.Add(RegisterWaterPurificationEvent)
end
