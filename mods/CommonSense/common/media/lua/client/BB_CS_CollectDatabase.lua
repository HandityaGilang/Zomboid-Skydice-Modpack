...
BB_CS_CollectDatabase = {}

BB_CS_CollectDatabase.Database = {
    -- Forest
    ...
    -- Missing world sprites in the original mod
    -- d_generic_1_84 = "",
	-- d_plants_1_0 = "",
	-- d_plants_1_1 = "",
	-- d_plants_1_2 = "",
...
---@return databaseItem[]
function BB_CS_CollectDatabase.GetItems(itemsList)

    local items = {} --[[@as (databaseItem[])]]
    local itemStrArr = BB_CS_Utils.splitString(itemsList, ",") --[[@as (string[])]]

    for _, str in ipairs(itemStrArr) do

        local itemParts = BB_CS_Utils.splitString(str, ":") --[[@as (string[])]]

        if #itemParts == 2 then
            local item = {} --[[@class databaseItem]]
            item.name = itemParts[1]
            item.value = math.floor(tonumber(itemParts[2])--[[@cast -?]] * SandboxVars.CommonSense.LootMultiplier)
            table.insert(items,  item)
        elseif isDebugEnabled() then
            print("[Common Sense] Invalid item format: ", str)
        end
    end
    return items
end