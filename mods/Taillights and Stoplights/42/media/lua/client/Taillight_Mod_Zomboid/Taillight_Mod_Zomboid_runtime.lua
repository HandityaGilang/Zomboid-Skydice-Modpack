...
function M.updateVehicleLights(vehicle)
    if vehicle then
        ...
        if M.vehicles and M.vehicles[keyId] and M.vehicles[keyId].lightData then
            lightData = M.vehicles[keyId].lightData
        else
            M.addVehicleToRuntime(vehicle)
            lightData = M.vehicles[keyId].lightData
        end
...
function M.isReverseLightOn(vehicle)
        if not TaillightModZomboid.getOptReverseLights() then
            return false
        end
...
function M.addVehicleToRuntime(vehicle)
    ...
    lights.toggleVanillaTaillight(vehicle, false)
end 
