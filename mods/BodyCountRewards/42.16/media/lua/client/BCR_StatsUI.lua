...
-- Shows all traits with status: rarity tier, or reason unavailable
...
function BCRStatsWindow:buildCatalogText(bcrData, opts)
    ...
    local function renderTraitLine(traitID, cost, poolLookup, isPositive)
        ...
        elseif isPositive and BCR.playerHasTrait(player, traitID) then
            ...
            if hasConflict and blockerID then
                ...
                -- Pass blockerName directly as %1 substitution argument to getText
                line = line .. getText("UI_BCR_StatusConflict", blockerName)
            else
                line = line .. getText("UI_BCR_StatusUnavailable")
            end
        end
    ...
    if not opts.givePositiveTraits then
        ...
        for _, entry in ipairs(BCR.PositiveTraitsList) do
            text = text .. renderTraitLine(entry.trait, entry.cost, earnablePool, true)
        end
    end
    ...
    if not opts.removeNegativeTraits then
        ...
        for _, entry in ipairs(BCR.NegativeTraitsList) do
            ...
            if isRelevant then
                ...
                text = text .. renderTraitLine(entry.trait, entry.cost, removablePool, false)
            end
        end
...
function BCRStatsWindow:updateContent()
    ...
    if allComplete then
        ...
        -- Normal: show past 2 + next 5, capped at last available reward
        local earnableCount = #(BCR.getEarnableTraits(self.player) or {})
        local removableCount = #(BCR.getRemovableTraits(self.player) or {})
        local rewardsLeft = earnableCount + removableCount
        msStart = math.max(1, rewardsGiven - SHOW_PAST + 1)
        msEnd = math.min(rewardsGiven + SHOW_FUTURE, rewardsGiven + rewardsLeft)
    end
