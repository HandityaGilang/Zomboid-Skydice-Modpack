...
-- dark flash, live finding). Pin the overrides EVERY tick with zero
...
-- (live finding: a refused restore looked like a dead button because the
...
function Aegis.canSee(area)
    ...
    -- could open this page and grant himself every area (player report).
...
-- admin status is deliberately NOT enough anymore (user decision July 15
-- 2026: only admins set via /setaccess may see the icon at all). This
...
function Aegis.allowed(chr)
    ...
    if isClient() then
        ...
        -- levels too (live finding: everyone saw the panel). The
