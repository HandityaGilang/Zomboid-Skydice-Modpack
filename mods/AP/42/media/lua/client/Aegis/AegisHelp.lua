...
function AegisHelp:createChildren()
    ...
    self.textPanel = ISPanel:new(0, 0, self.scroll.width - 14, 10)
...
function AegisHelp:layoutText()
    ...
    -- every frame and lays out again when they drift apart. Live report:
    -- shrinking the window left the old wrap running under the scrollbar
    -- on nearly every page, in both tabs
    ...
    if self.tab == "help" then
        ...
        for _, entry in ipairs(self.content.changelog) do
            ...
            table.insert(rows, { gap = 6 })
            -- sectioned entries (new/fixed, user request) carry their
...
function AegisHelp:placeChrome()
    ...
    if self.scroll.vscroll then
        self.scroll.vscroll:setX(self.scroll.width - 16)
        self.scroll.vscroll:setHeight(self.scroll.height)
    end
    self.textPanel:setWidth(self.scroll.width - 14)
end
