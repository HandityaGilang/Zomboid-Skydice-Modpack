...
local function decompose(set)
    ...
    for _, z in ipairs(cells) do
        if isFree(z.x, z.y) then
            ...
            -- losing that piece of the zone (review finding). Two capped
...
function AegisPageZones:setList(list, stray, complete)
    ...
    local prevKey = prev and (prev.item.x .. "|" .. prev.item.y) or nil
    self.list:clear()
    ...
    if self.list.selected < 1 and self.list.items[1] then
        self.list.selected = 1
    end
end
...
-- disable buttons while the selected zone is open at another admin
function AegisPageZones:updateButtons()
    local e = self:selectedZone()
    local locked = e ~= nil and e.lockedBy ~= ""
    if self.editBtn then self.editBtn:setEnabled(not locked) end
    if self.brushBtn then self.brushBtn:setEnabled(not locked) end
end
...
Events.OnServerCommand.Add(function(module, command, args)
    ...
    elseif command == "shShape" or command == "shNew" or command == "shNewShape" then
        ...
        elseif args and args.reason == "locked" then
            ...
            -- without this branch it read as "zone gone" (review finding)
