...
function AegisPageFactions:updateButtons()
    ...
    local visible = sh ~= nil and Aegis.canSee("world")
    self.jumpBtn:setVisible(visible)
    self.jumpBtn:setEnabled(visible)
end
...
function AegisPageFactions:drawSafehouse(e, dx, dw, pad)
    ...
    Aegis.text(self, getText("UI_Aegis_FactionMembers"), x, yy, UIFont.Small, c.gold)
    yy = yy + lineH + 4
    local bottom = self.height - pad - (self.jumpBtn and self.jumpBtn:isVisible() and 60 or 16)
...
Events.OnServerCommand.Add(function(module, command, args)
    ...
    if command ~= "factionList" then return end
