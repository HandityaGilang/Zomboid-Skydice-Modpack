...
-- (live finding 27 July: mirror showed the new role one rung below the
...
function AegisRoleTag.createAndAssign(username, name, color)
    ...
    -- actor's own live rank when the role was his own (review finding)
...
-- both rank gates while doing it (review finding)
...
Events.OnTick.Add(function()
    ...
    if run.step == "position" then
        ...
        -- first push (live test finding: "role ranks above own" on
        -- self-tag)
        ...
        if pos >= targetNow and pos <= upperBound then
            ...
            -- lag ~1.4s behind the server (live finding 16 July: client read
    ...
    if run.step == "assign" then
        ...
        -- comes back (live test finding: assignment "unconfirmed", own
