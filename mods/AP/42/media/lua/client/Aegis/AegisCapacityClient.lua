...
-- container instead of the object's own container (live finding: the
...
-- vehicles get 1000 (bytecode finding, explains "stuck at 100" no matter
...
-- survived saves and backup restores (player report). The stamp replaces
...
function AegisCapacityClient.containerSpec(c)
    ...
    pcall(function()
        ...
        if chr then
            ...
            -- even the ROOT inventory sometimes (live finding: main
...
Events.OnServerCommand.Add(function(module, command, args)
    ...
    if args.ok == false then return end
...
-- character stops stowing (user report); log every failing condition so
-- the exact culprit shows in the log
require "TimedActions/ISInventoryTransferAction"

-- hijacked phases for aegisServerMove actions: no vanilla transaction
-- (the server rejected those, live finding), no rejection polling, the
...
function ISInventoryTransferAction:start()
    if self.aegisServerMove then
        ...
        -- real duration or it loads forever (live finding). Time first,
        -- the anim call must never block it
        local time = tonumber(self.maxTime) or 0
        if time <= 0 or time > 200 then
            local weight = 1
            pcall(function() weight = self.item:getUnequippedWeight() end)
            time = math.min(120, math.max(25, math.floor(weight * 6)))
        end
        self.maxTime = time
...
function ISInventoryTransferAction:perform()
    if self.aegisServerMove then
        ...
        -- perform fires every frame (live finding: 533 sends)
        pcall(function()
...
function ISInventoryTransferAction:isValid()
    ...
    -- the item snaps back (live finding). Admin moves into vehicle
    ...
    if ok and isClient() then
        ...
        pcall(function()
            ...
            local admin = Aegis.allowed(self.character)
            if not admin and getAccessLevel then
                local level = tostring(getAccessLevel() or ""):lower()
                admin = level ~= "" and level ~= "none"
            end
    ...
    if not ok and admin then
        ...
        -- and the queued action still holds the old instance (log
        -- finding: src/dest fine, action dies anyway). The integrity
        ...
        if pass then
            ...
            -- transaction (live finding: rescued actions never arrived),
...
-- while the display still showed the stamp (user finding: 350 shown,
-- ~110 usable)
...
Events.OnServerCommand.Add(function(module, command, args)
    ...
    elseif args.kind == "obj" then
        ...
        -- the client at the sprite capacity (~100 on crates, live finding)
        ...
        for _, btn in ipairs(page.backpacks) do
            ...
            if c then
                ...
                if part then
                    ...
                    -- session (log finding). The body is served by the
