...
local EDGE = 12
-- resize floor, deliberately below the old default size (user request);
-- the nav already packs itself on tight heights
local MIN_W = 860
local MIN_H = 560
-- free corner under a scroll host: its scrollbar used to run into the
-- resize grip, both wanted the same pixels (live finding)
local GRIP_CHIN = 26
...
-- (live finding on the powers page)
...
function AegisWindow.toggle()
    ...
    if AegisWindow.instance then
        ...
        -- (user request, the X button in the header stays the real close path)
...
function AegisWindow:createChildren()
    ...
    -- manual and changelog, reachable any time (user request 26.7.)
    ...
    -- be dragged into a new order, closed = order fixed (user request)
...
-- a full size empty body (live finding, twice); with the window simply
...
-- restructure the child list mid dispatch (live finding, a rebuild
...
function AegisWindow:rebuildPages()
    ...
    -- (review finding). switchPage restores lazily when a page comes back
    self.pendingStates = {}
...
function AegisWindow:openRestoreMenu()
    ...
    -- (review finding), so its height is capped and it scrolls instead
...
-- second never looked smooth (live findings at 350ms and 120ms), so the
...
function AegisWindow:prerender()
    ...
    -- fixed offset collided with the help button (live finding)
    ...
    -- list used to run straight through the role line in the footer (live
    -- finding). Past that floor the strip scrolls with the wheel instead
