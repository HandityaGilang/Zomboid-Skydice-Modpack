...
-- requests the server drops (review finding)
local RETRY_MS = 6000
local ROW_H = 56
...
-- with "tried to call nil", once per tick (live finding)
-- ==================================================================
local KNOX_SCAN = 30
...
local function carFor(e)
    ...
    -- were happily updating from the very same vehicle (user finding)
    local near = false
    pcall(function()
        local p = getPlayer()
        if not p then return end
...
function AegisPlayerVehNavi.start(entry)
    ...
    if not AegisPlayerVehNavi.instance then
        ...
        -- click even with mouse events off (live finding, same trap the
...
local function naviChip(el, cx, cy, text)
    ...
    -- clamp against the real screen, the element is 1x1 (review finding:
...
function AegisPlayerVehNavi:render()
    ...
    if self.foundUntil then
        ...
        -- (user finding: the safehouse navi greeted with "vehicle found")
...
-- are not the metres this formula needs (live finding, an M923 filled
...
function AegisPlayerVehScene:onMouseDown(x, y)
    ...
    -- and the pan is the only way to see them at all (user finding, a
...
-- (user finding: a roofed rig sat outside the frame with no way back)
...
function AegisPlayerPageVehicles:applyPreview()
    ...
    pcall(function()
        ...
        -- too high in the frame (user finding). dragView takes raw mouse
...
function AegisPlayerPageVehicles.drawRows(panel)
    ...
    if page.gotList and #page.entries == 0 then
        ...
        -- sides and the clip cut its first letters (live finding)
...
-- one button, called release everywhere (user request): a ledger row is
...
function AegisPlayerPageVehicles:update()
    ...
    if self.saveBtn then
        self.saveBtn:setEnabled(self.nearVeh ~= nil)
