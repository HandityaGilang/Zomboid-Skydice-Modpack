...
function AegisPageWorld:createChildren()
    ...
    -- card on a narrow window (live finding: apply and delete vanished);
...
Events.OnServerCommand.Add(function(module, command, args)
    ...
    -- like nothing happened (user finding). One-shot snapping is not
    -- enough: an engine SyncClockPacket already in flight with the OLD
    -- time can land right after and throw the clock back (second live
    -- finding, race depends on packet timing). The snap is therefore held
...
local function applyTimeSnap(tod)
    ...
    -- while a snap was held (live finding via the in-game Lua debugger)
