...
function AegisPageRoles:createChildren()
    ...
    -- time, the save applies it to every online wearer (user request)
...
function AegisPageRoles.onAssign(self)
    ...
    -- the role itself decides about the head tag (pp.tag, user request)
    local wantTag = true
    if roleIdx > 1 and self.roles[roleIdx - 1] then
    ...
    local function doAssign()
        ...
        -- (user request): create and assign the matching engine head tag in
    ...
    -- active confirmation before assigning (user request): states in
...
-- scoreboard cache has entries (user report: head tag dropdown stayed empty,
-- cause not conclusively reproducible, population logic is identical for
-- both combos), do a cheap check on every frame and refill on demand
