...
-- range, budget and overlap untouched (review finding). Only finite
...
Commands.claimSet = function(player, name, args)
    ...
    -- as a broken zone in every list (live finding). Touching passes,
...
Commands.claimRelease = function(player, name, args)
    ...
    -- that blocks every future claimSet forever (review finding)
    if not AegisZones.registryWritable() then
