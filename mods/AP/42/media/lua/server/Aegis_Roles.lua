...
-- The panel key is legacy: every player has the panel now (user decision
-- 26.7.), it is still parsed and written so old roles.txt keeps loading
...
local function isVanillaAdmin(player)
    ...
    local ok, res = pcall(function()
        ...
        -- are levels too (live finding: everyone saw the panel). The
...
-- page and hand himself every area (player report). The vanilla
...
--
-- admin only (user decision 25.7.): an Aegis role can only NARROW what a
...
-- the panel (user decision 26.7.), admins included, so this always
