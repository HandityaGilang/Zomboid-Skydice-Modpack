...
-- dies with every server restart, so entries went husk (user report,
-- no position, no bars). Saving now stamps the key into the vehicle's
...
local function findByStamp(key)
    local found = nil
    pcall(function()
        local list = VehicleManager.instance:getVehicles()
...
-- an owner drives or parks with the key inside (review finding, a
...
Commands.vehState = function(player, name, args)
    ...
    -- would otherwise pass the expensive path on every packet (review
    -- finding). A player pushes at most one vehicle every few seconds
