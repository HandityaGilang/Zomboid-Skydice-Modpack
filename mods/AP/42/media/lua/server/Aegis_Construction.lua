...
local function objectKind(obj)
    ...
    -- remaining steps (user finding: two "bau" lines at the SAME square).
    ...
    -- while the rest of the door stayed missing (user finding)
...
-- tile (user finding: stairs and garage doors came back incomplete)
...
-- powers page only half worked (user report: materials still required).
...
-- (user finding: walkable straight through). Ask the sprite itself,
...
Commands.constructionRestore = function(player, args)
    -- the tools area right is the whole gate (user decision 26.7.), same
