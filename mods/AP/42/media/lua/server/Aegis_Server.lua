...
local function allowed(player, capName)
    ...
    local ok, res = pcall(function()
        ...
        -- access level, not on the flaky per player role wrapper (live
        -- findings in both directions). Exact vanilla semantics for
...
local function permitted(player, capName, area)
    ...
    -- vanilla-admin fallthrough (live finding: the role object drops
...
-- (user finding). File lines U|username|value; the minute sweep re-applies
...
Events.OnTick.Add(function()
    ...
    carryLoad()
    -- Kahlua on the dedicated server has no global next (live finding:
    -- the sweep threw every 3 seconds)
    local empty = true
...
Commands.horde = function(player, args)
    ...
    -- fall never reaches the clients in B42 MP (live finding, zombies hung
    ...
    local ok, err = pcall(function()
        for i = 0, car:getPartCount() - 1 do
            ...
            if part then
                ...
                -- with swappable variants - live test finding 16 July)
...
-- strict self-validation for movers WITHOUT the tools area (live
-- finding: the 1.1 clients route vehicle transfers of ordinary players
...
local function moveValidated(player, args)
    ...
    pcall(function()
        ...
        if not srcInv or not destInv or srcInv == destInv then return end
        local function near(spec)
            ...
            if spec.kind == "veh" then
                ...
                x = (tonumber(spec.x) or 0) + 0.5
                y = (tonumber(spec.y) or 0) + 0.5
            end
            return IsoUtils.DistanceTo(player:getX(), player:getY(), x, y) <= 4
        end
        if not near(args.src) or not near(args.dest) then return end
        local item = srcInv:getItemWithID(tonumber(args.itemId) or -1)
        if not item then return end
        if not destInv:isItemAllowed(item) then return end
        local cap = nil
        ...
        if destInv:getCapacityWeight() + item:getUnequippedWeight() > cap then return end
        okAll = true
    end)
    return okAll
end
...
Commands.adminMoveItem = function(player, args)
    if not toolsPermittedQuiet(player) and not moveValidated(player, args) then
        toClient(player, "adminMoveItem", { ok = false, reason = "denied" })
        return
    end
    if not args then return end
    local srcInv = resolveMoveContainer(player, args.src)
    ...
    if not srcInv or not destInv or srcInv == destInv then
        toClient(player, "adminMoveItem", { ok = false, reason = "container not found" })
        return
    end
    local item = nil
    pcall(function() item = srcInv:getItemWithID(tonumber(args.itemId) or -1) end)
    if not item then
        toClient(player, "adminMoveItem", { ok = false, reason = "item not found" })
        return
    end
    local ok, err = pcall(function()
        srcInv:Remove(item)
...
Commands.vehicleContainerLevel = function(player, args)
    ...
    local ok, err = pcall(function()
        ...
        -- sees it without this sync (live finding 16 July: slider had no
...
-- SendCommandToServer that were hardened in the last suite review):
...
-- the logs whether any client ever picked the relay up (community report
-- "alerts ran, restart never came" was unanswerable)
...
local function restartWatch()
    ...
    if remaining <= 0 then
        ...
        if isServer() then
            -- if the planner is offline, any authorized online admin may
            -- run the quit relay
            local relay = { by = restartPlan.by }
            if not findByUsername(restartPlan.by or "") then
                relay = { anyone = true }
            end
            ...
            print("[Aegis] restart relay sent, attempt " .. restartPlan.tries .. " ("
                .. (relay.anyone and "anyone with QuitWorld" or ("planner " .. tostring(relay.by)))
                .. "), " .. able .. " client(s) able to quit")
            ...
            -- ran for nothing (community report, dedicated server). A
