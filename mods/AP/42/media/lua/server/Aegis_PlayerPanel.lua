...
-- assigns or edits a role (live finding: the client only asked at game
-- start, a grant during the session never arrived until reconnect)
AegisPlayerPanel = AegisPlayerPanel or {}

-- server switch (sandbox, user request from the workshop): with the
...
function AegisPlayerPanel.push(player)
    ...
    if pp then
        ...
        -- panel every player gets (user decision 26.7.)
...
-- args table where the kit id was expected (review finding)
...
-- refused at join never asked again after a turn on (review finding).
-- This watcher pushes everyone whenever one of the four values changes
local lastFeat = nil
