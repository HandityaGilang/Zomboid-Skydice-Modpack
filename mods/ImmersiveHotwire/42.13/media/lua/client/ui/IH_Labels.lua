    ...
    if level == 1 then
        ...
        if hasBattery then
            if #possibleParts == 0 then
                return string.format(TEXT.FORMAT_ACTION_BATTERY_WIRE, action)
            end
            ...
            return string.format(TEXT.FORMAT_ACTION_WIRE_LABELED, action, suffix)
        end
        if #rawParts == 1 then
            return string.format(TEXT.FORMAT_ACTION_WIRE_POSSIBLY, action, rawParts[1])
        end
        local suffix = Utils.joinWithAnd(possibleParts, TEXT.LABEL_SEP_MIDDLE, TEXT.LABEL_SEP_LAST)
        return string.format(TEXT.FORMAT_ACTION_WIRE_LABELED, action, suffix)
    end
    return string.format(TEXT.FORMAT_ACTION_WIRE_LABELED, action, labelLower)
end
