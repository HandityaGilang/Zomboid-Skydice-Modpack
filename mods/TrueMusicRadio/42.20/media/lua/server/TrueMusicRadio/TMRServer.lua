...
TMRadioServer.FillMedia = function(player, args) 
	--print("TMRadio Server: Filling Media in Terminal")
	if args then
		local item = instanceItem(args.media)
		local square = getSquare(args.tx, args.ty, args.tz)
		local Terminal = nil
		...
		if Terminal then
			Terminal:getInventory():AddItem(item);
			sendAddItemToContainer(Terminal, item);
		end
	end
end
