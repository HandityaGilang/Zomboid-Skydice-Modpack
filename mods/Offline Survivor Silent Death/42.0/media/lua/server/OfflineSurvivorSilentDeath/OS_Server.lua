...
-- See Offline Survivor's server script: B42 releases the temporary native
-- drag surrogate after GrapplerLetGo, so a final move is reconciled one
-- server pass later to avoid duplicate corpses.
...
local function markVisualClone(item, record)
    ...
    data.offlineSurvivorSilentDeathOwner = tostring(record.steamId)
end
...
local function createOfflineCorpse(record, square, visualSource)
    ...
    local ok, result = pcall(function()
        ...
        -- clients immediately.
        sendCorpse(corpse)
        -- sendCorpse serializes the ModData above. A second ModData packet in
        -- the same tick can arrive before AddCorpse on a client and produces
        -- "ObjectChangePacket ... IsoDeadBody ID ... can't be found".
        return corpse
    end)
...
local function createOfflineCorpseObject(record, visualSource)
    ...
    if not square then
        ...
        if not record.squareWaitReported then
            ...
            print("[Offline Survivor Silent Death] Logout square is not loaded for " .. tostring(record.username) .. "; waiting for it to stream in")
        end
        return nil
    end
    ...
    if rebuilds > MAX_CORPSE_REBUILDS then
        ...
        if not record.corpseFailureReported then
            ...
            print("[Offline Survivor Silent Death] Stopped rebuilding the offline body for " .. tostring(record.username)
    ...
    if not corpse then
        ...
        if not record.corpseFailureReported then
            record.corpseFailureReported = true
            print("[Offline Survivor Silent Death] Native corpse failed for " .. tostring(record.username) .. ": " .. record.corpseFailure)
        end
        return nil
    end
    ...
    print("[Offline Survivor Silent Death] Created native player corpse for " .. tostring(record.username) .. " on " .. tostring(record.placement or "floor"))
    return corpse
end
...
-- B42 reanimates a grabbed corpse as an IsoZombie, then creates a new corpse
-- only after the native release completes. Both carry this mod's ModData.
-- Waiting for that transition avoids racing the engine and leaving a second,
-- ordinary corpse beside the protected sleeping-survivor proxy.
...
-- Clients renew this lease every five seconds during a native drag. Keeping
-- it short prevents a failed action from falsely blocking the body for long.
    ...
    for steamId, move in pairs(pendingRepositions) do
        ...
        if not connected[steamId] or not player then
            ...
            elseif current >= numberOr(move.untilAt, 0) then
                ...
                pendingRepositions[steamId] = nil
                print("[Offline Survivor Silent Death] Could not settle reconnect position for " .. tostring(steamId))
            else
...
-- The real murder corpse already owns the victim's original inventory.  A
-- reconnect can instantiate a fresh player object from its saved data, so
-- clear that temporary copy before sending the native death event; otherwise
-- a second corpse could duplicate the victim's items.
...
local function markOffline(record)
    ...
    if shouldBlockAdminOfflineBody(record) then
        ...
        record.objectCreated = nil
        print("[Offline Survivor Silent Death] Offline body suppressed for administrator " .. tostring(record.username))
    else
        record.bodySuppressed = nil
        print("[Offline Survivor Silent Death] Player disconnected: " .. tostring(record.username))
...
local function markAdminTestOffline(record)
    ...
    discardPendingEntriesFromSnapshot(record)
    createOfflineCorpseObject(record)
    print("[Offline Survivor Silent Death] Administrator test body created for " .. tostring(record.username))
    persistentDataChanged()
end
...
-- The server loads a fresh IsoPlayer from players.db before Lua sees a
-- reconnect.  Queue the death instead of applying it in that first tick: the
-- connection has time to finish loading, then the saved duplicate inventory
-- is cleared and the normal death packet is sent to that client.
local function applyMurderOnReconnect(record, player)
    local steamId = tostring(record.steamId)
    local pending = pendingMurderDeaths[steamId]
    if pending then
        -- Only restart the short load grace period when this pending entry
        -- lost its player because the victim disconnected during loading.
        -- Resetting it on every server scan postpones the death forever.
    ...
    pendingRepositions[steamId] = nil
    pendingMurderDeaths[steamId] = {
        player = player,
        dueAt = nowSeconds() + 2,
        phase = "kill",
        murder = record.murder or {},
    }
    print("[Offline Survivor Silent Death] Queued offline murder for reconnecting player " .. tostring(player:getUsername()))
end

local function finishMurderRecord(record)
    local proxy = findOfflineObject(record)
    ...
    record.realCorpseX, record.realCorpseY, record.realCorpseZ = nil, nil, nil
...
local function trackOnlinePlayer(player)
    ...
    -- A murder already recorded by this variant must still be applied even
    -- if an administrator disables the mod before the victim reconnects.
    -- Disabling stops new offline bodies; it must not erase an existing PvP
    -- consequence.
    if not OS.isEnabled() then
        local murderedRecord = records[steamId]
        if murderedRecord and murderedRecord.state == "murdered" then
            runtimePlayers[steamId] = player
            applyMurderOnReconnect(murderedRecord, player)
            murderedRecord.username = player:getUsername()
            murderedRecord.lastSeen = nowSeconds()
            records[steamId] = murderedRecord
            persistentDataChanged()
        end
        return
    end
    ...
    elseif previousState == "murdered" then
        applyMurderOnReconnect(record, player)
    ...
    if playerIsDead then
        -- Keep the record in the murdered state until the scheduled cleanup
        -- removes its empty temporary death body.
        if pendingMurderDeaths[steamId] then
            records[steamId] = record
            return
        end
        -- A player at the death/respawn screen must not leave another offline
        -- sleeper when their connection closes.
        record.state = "dead"
        records[steamId] = record
        return
    end

    -- Do not turn a murdered record back into "online" while its reconnect
    -- death is queued.  That used to make processPendingMurderDeaths discard
    -- the pending kill in the next maintenance tick, leaving the player alive
    -- until a server restart.
    if record.state == "murdered" or pendingMurderDeaths[steamId] then
        records[steamId] = record
        return
    end
    ...
    if not lastHeartbeatLog[steamId] or nowSeconds() - lastHeartbeatLog[steamId] >= 10 then
        ...
        print("[Offline Survivor Silent Death] SERVER tracking active player " .. tostring(record.username))
    end
    if previousState == "offline" or previousState == "murdered" or previousState == "test_offline" or not hadRecord then persistentDataChanged() end
end
...
local function getRecordSafehouse(record)
    if not record or not SafeHouse then return nil end
    local square = getRecordSquare(record)
    if not square then return nil end

    local ok, safehouse = pcall(function() return SafeHouse.getSafeHouse(square) end)
    return ok and safehouse or nil
end

local function isSafehouseAssassinationBlocked(record)
    return OS.getOption("BlockSafehouseAssassinations", true) ~= false
        and getRecordSafehouse(record) ~= nil
end

-- Replaces the sealed visual proxy with a normal native player corpse.  The
-- IsoDeadBody constructor transfers the disconnected player's *real*
-- ItemContainer, WornItems and AttachedItems into the corpse, then clears the
-- player object.  Unlike the proxy, this body intentionally has no offline
-- marker: the base game may loot it, move it and burn it normally.
local function replaceOfflineProxyWithRealCorpse(record, victim, killer)
    ...
    local ok, corpse = pcall(function()
        ...
        local realCorpse = victim:dieNetwork(killer, nil, true, nil)
        if not realCorpse then error("the engine did not create the real corpse") end
    ...
    record.objectCreated = nil
    if not ok then
        -- Preserve the sleeping proxy if a third-party error prevents the
        -- conversion. No assassination is recorded in this case.
        print("[Offline Survivor Silent Death] Real corpse conversion failed for " .. tostring(record.username) .. ": " .. tostring(corpse))
        createOfflineCorpseObject(record)
    ...
    print("[Offline Survivor Silent Death] Created real lootable corpse for " .. tostring(record.username))
    return corpse, nil
end

local function hasMurderableOfflineObject(record)
...
-- An offline survivor is represented by a corpse-like static object, so
-- zombies cannot acquire it as a normal living AI target. This server-side
-- proximity check supplies that missing interaction: a live, toothed zombie
-- that reaches the sleeping survivor converts it to a real native corpse.
...
local function killOfflineSurvivorByZombie(record, zombie)
    if not hasMurderableOfflineObject(record) then return false end
    ...
    if record.testBody == true then
        ...
        persistentDataChanged()
        print("[Offline Survivor Silent Death] Zombie defeated administrator test body for " .. tostring(record.username))
        return true
    end

    -- Save the consequence before changing world objects. The existing
    -- reconnect-death pipeline is deliberately reused so the victim cannot
    -- return alive with a duplicated inventory.
    local previousState = record.state
    record.state = "murdered"
    record.murder = {
        cause = "zombie",
        killerName = "Zombies",
        at = nowSeconds(),
    ...
    if not victim then
        ...
        print("[Offline Survivor Silent Death] Deferred zombie death for " .. tostring(record.username) .. " until reconnect")
        return true
    end
    ...
    local realCorpse, corpseReason = replaceOfflineProxyWithRealCorpse(record, victim, zombie)
    if not realCorpse then
        ...
        record.deferredDeath = nil
        persistentDataChanged()
        print("[Offline Survivor Silent Death] Zombie death conversion failed for " .. tostring(record.username) .. ": " .. tostring(corpseReason))
        return false
    end
    ...
    print("[Offline Survivor Silent Death] " .. tostring(record.username) .. " was killed by zombies while offline")
    return true
end
...
local function reportZombieFeeding(player, args)
    ...
    if current - lastLogged >= 10 then
        ...
        print("[Offline Survivor Silent Death] Confirmed zombie feeding on " .. tostring(record.username))
    end
end
...
local function requestSilentKill(player, args)
    local killerSteamId = OS.getSteamId(player)
    if not killerSteamId then return end

    if not OS.isEnabled() then
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, { message = "Offline Survivor Silent Death is disabled on this server." })
        return
    end

    local targetSteamId = args and args.targetSteamId and tostring(args.targetSteamId) or nil
    if not targetSteamId or targetSteamId == "" or sameSteamId(killerSteamId, targetSteamId) then
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, { message = "Invalid offline survivor." })
        return
    end

    local record = records[targetSteamId]
    if not hasMurderableOfflineObject(record) then
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, { message = "This survivor is no longer sleeping here." })
        return
    end
    if isJaxeRevivalProtected(record) then
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, {
            message = "This survivor is protected by Jaxe Revival while incapacitated.",
        })
        return
    end
    if isBodyBeingDragged(record) then
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, { message = "This sleeping survivor is being moved right now." })
        return
    end
    if not isNearRecord(player, record) then
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, { message = "You are too far away to assassinate this survivor." })
        return
    end
    if isSafehouseAssassinationBlocked(record) then
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, {
            message = "This sleeping survivor is protected by a safehouse.",
        })
        return
    end

    local weapon = OS.findUsableKnife(player)
    if not weapon then
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, { message = "You need a usable knife to assassinate this survivor." })
        return
    end
    ...
    if record.testBody == true then
        local proxy = findOfflineObject(record)
        if not proxy then
            sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, { message = "The administrator test body is no longer available." })
            return
        end
        record.testDefeated = true
        record.testDeathCause = "player"
        record.testDeathAt = nowSeconds()
        removeTrackedObject(record)
        record.objectCreated = nil
        createOfflineCorpseObject(record)
        persistentDataChanged()
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, {
            message = "Administrator test body defeated. The real player was not killed.",
        })
        return
    end

    -- Persist the consequence before changing any world object.  This is the
    -- authoritative record used when the victim reconnects, so an
    -- assassination is never forgotten if the server saves between the corpse
    -- replacement and that player's next login.
    local previousState = record.state
    local murder = {
        cause = "player",
        killerName = tostring(player:getUsername() or "Unknown"),
        killerSteamId = tostring(killerSteamId),
        weapon = tostring(weapon:getFullType() or ""),
        at = nowSeconds(),
    }
    record.state = "murdered"
    record.murder = murder
    record.realCorpseCreated = nil
    local victim = runtimePlayers[targetSteamId]
    if not victim then
        local square = getRecordSquare(record)
        if square then
            local x, y = getCorpseRenderPosition(record, square)
        ...
        record.deferredDeath = true
        markOfflineProxyPendingDeath(record)
        clearLootSession(killerSteamId)
        persistentDataChanged()
        print("[Offline Survivor Silent Death] Deferred assassination for " .. tostring(record.username) .. " until reconnect")
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, {
            message = "You assassinated " .. tostring(record.username or "the survivor") .. ". Their death will be finalized when they reconnect.",
        })
        return
    end
    ...
    record.deferredDeath = nil
    persistentDataChanged()
    local realCorpse, corpseReason = replaceOfflineProxyWithRealCorpse(record, victim, player)
    if not realCorpse then
        -- Do not leave a false murder record when native corpse conversion
        -- failed. The sealed sleeping proxy is restored by the helper above.
        record.state = previousState
        record.murder = nil
        record.realCorpseCreated = nil
        record.deferredDeath = nil
        persistentDataChanged()
        sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, {
            message = "Unable to turn this body into a real corpse: " .. tostring(corpseReason),
        })
        return
    end
    ...
    record.realCorpseAt = nowSeconds()
    persistentDataChanged()
    print("[Offline Survivor Silent Death] " .. tostring(record.username) .. " was assassinated while offline by " .. tostring(record.murder.killerName))
    sendTo(player, OS.COMMAND_SILENT_KILL_RESULT, {
        message = "You assassinated " .. tostring(record.username or "the survivor") .. " while they were sleeping.",
    })
end
...
-- Runs only after the reconnecting player has been present for two server
-- maintenance ticks.  This avoids the initial players.db load overwriting the
-- death state, which was the source of the duplicated inventory report.
local OFFLINE_DEATH_MAX_ATTEMPTS = 15
...
local function processPendingMurderDeaths(connected)
    local current = nowSeconds()
    for steamId, pending in pairs(pendingMurderDeaths) do
        local record = records[steamId]
        if not record then
            pendingMurderDeaths[steamId] = nil
        elseif record.state ~= "murdered" and pending.phase ~= "removeTemporaryBody" then
            pendingMurderDeaths[steamId] = nil
        elseif not connected[steamId] and pending.phase ~= "removeTemporaryBody" then
            -- The player disconnected during the loading screen. Keep the
            -- murder record; the next reconnect queues a fresh attempt.
            pending.player = nil
        elseif pending.phase == "kill" and current >= numberOr(pending.dueAt, current) then
            ...
            elseif pending.killIssued then
                ...
                persistentDataChanged()
                sendTo(player, OS.COMMAND_SLEEP_MURDERED, {
                    cause = tostring((pending.murder or {}).cause or "player"),
                    killerName = tostring((pending.murder or {}).killerName or "Unknown"),
                    weapon = tostring((pending.murder or {}).weapon or ""),
                })
                print("[Offline Survivor Silent Death] Applied offline murder to reconnecting player " .. tostring(record.username))
            else
                ...
                if not reallyDead then
                    ...
                    pending.dueAt = current + 1
                    if not ok then
                        print("[Offline Survivor Silent Death] Could not force reconnect death for " .. tostring(record.username) .. ": " .. tostring(temporaryBody))
                    end
                else
                    -- Consume the death only now that it is confirmed. Leaving
                    -- the record as "murdered" afterwards would kill every new
                    -- character this player created, forever.
                    record.state = "dead"
                    record.deferredDeath = nil
                    persistentDataChanged()
                    pending.phase = "removeTemporaryBody"
                    ...
                    print("[Offline Survivor Silent Death] Applied offline murder to reconnecting player " .. tostring(player:getUsername()))
                end
            end
            ...
            -- loading screen.
            if pending.phase == "kill" and numberOr(pending.attempts, 0) >= OFFLINE_DEATH_MAX_ATTEMPTS then
                if not pending.retryWarningShown then
                    pending.retryWarningShown = true
                    print("[Offline Survivor Silent Death] Keeping the offline death pending for "
        ...
        elseif pending.phase == "removeTemporaryBody" and current >= numberOr(pending.removeAt, current) then
            ...
            elseif expectedBodyId and waited >= TEMPORARY_BODY_MAX_ATTEMPTS then
                ...
                print("[Offline Survivor Silent Death] Waiting to remove temporary death body for " .. tostring(record.username))
            elseif waited >= TEMPORARY_BODY_MAX_ATTEMPTS then
                if not cleared then
                    print("[Offline Survivor Silent Death] No temporary death body was published for " .. tostring(record.username))
                end
                finishMurderRecord(record)
                pendingMurderDeaths[steamId] = nil
                persistentDataChanged()
        ...
        if isSleepingRecord(record) and not record.despawned and not isBodyBeingDragged(record)
            ...
            if proxy and (reportedFeeding or serverFeeder) then
                ...
                if elapsed <= OS.SERVER_SCAN_SECONDS or math.floor(elapsed) % 10 == 0 then
                    ...
                    print("[Offline Survivor Silent Death] Zombie progress for " .. tostring(record.username)
...
local function maintainOfflineRecords(connected)
    ...
    for steamId, record in pairs(records) do
        ...
        if record.state == "dead" and record.temporaryDeathBodyId and not pendingMurderDeaths[steamId] then
            local resolved, removed = cleanupRecordedTemporaryDeathBody(record)
            if resolved then
                changed = true
                if removed then
                    print("[Offline Survivor Silent Death] Removed recovered temporary death body for " .. tostring(record.username))
                end
            end
        end
...
local function runStep(name, step, argument)
    ...
    if not ok then print("[Offline Survivor Silent Death] " .. name .. " failed: " .. tostring(err)) end
    return ok
end
...
local function onTick()
    ...
    runStep("processPendingMurderDeaths", processPendingMurderDeaths, connected)
...
local function onInitGlobalModData()
    ...
    if not sandboxPage or sandboxPage.AllowZombieDeaths == nil then
        print("[Offline Survivor Silent Death] Sandbox page '" .. OS.MODULE
        ...
        print("[Offline Survivor Silent Death] Zombies Can Kill Sleeping Survivors = "
            .. tostring(OS.getOption("AllowZombieDeaths", false)))
    end
    print("[Offline Survivor Silent Death] SERVER script initialized.")
end
...
local function onClientCommand(module, command, player, args)
    ...
    elseif command == OS.COMMAND_REQUEST_SILENT_KILL then
        requestSilentKill(player, args)
    end
end
