...
local function Buttstroke_HitZombie(zombie, player)
    ...
    if player:isDoShove() and player:isAiming() and player:getVariableBoolean("bShoveAiming") then
        ...
        triggerEvent("OnWeaponHitCharacter", player, zombie, weapon, (stockDamage / 100) * playerStats:get(CharacterStat.ENDURANCE))
        zombie:setHealth(zombie:getHealth() - (stockDamage / 100) * playerStats:get(CharacterStat.ENDURANCE))
        ...
        playerStats:set(CharacterStat.ENDURANCE, playerStats:get(CharacterStat.ENDURANCE) - enduranceLost)
