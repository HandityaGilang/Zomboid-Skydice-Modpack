...
PlaVar.playerInitMap = {}

function isCmdReady()
    return PlaVar.computeAndGetSync(0)
end

function PlaVar.onPlayerUpdateInit(isoPlayer)
    local key = tostring(isoPlayer)
    if not PlaVar.playerInitMap[key] then
        PlaVar.playerInitMap[key] = true
        PlaVar.computeAndGetSync(isoPlayer:getPlayerNum())
    end
end

--Events.OnPlayerUpdate.Add(PlaVar.onPlayerUpdateInit)
