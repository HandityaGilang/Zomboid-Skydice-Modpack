...
local onPumpFromBarrel = function(playerObj, part, barrel)
    ...
    if barrel then
        ...
        ISTimedActionQueue.add(UB_RefuelVehicleAction:new(playerObj, part, barrel.isoObject))
    end
end
    ...
    for i=1,vehicle:getPartCount() do
        ...
        if part 
            ...
            and part:getContainerContentAmount() < part:getContainerCapacity() then
                local partSquare = UB_Utils.GetVehiclePartSquare(vehicle, part)
                local barrels = UB_Utils.GetBarrelsNearby(partSquare, UB_Const.VEHICLE_SCAN_DISTANCE, Fluid.Petrol, true)

                if #barrels > 0 then
                    local barrel = barrels[1]
                    local worldObjects = UB_Utils.GetWorldItemsNearby(barrel.square, UB_Const.TOOL_SCAN_DISTANCE)
                    local hasHoseNearby = UB_Utils.HasItemNearbyOrInInv(worldObjects, playerObj:getInventory(), UB_Registries.ItemTag.RUBBER_HOSE)
                    if slice then
                        ...
                        if SandboxVars.UsefulBarrels.CarRefuelRequiresHose and not hasHoseNearby then 
                            UB_Utils.DisableOptionAddTooltip(option, getText("Tooltip_UB_HoseMissing", getItemName("Base.RubberHose"))) 
                        end
                    end
                end
        end
