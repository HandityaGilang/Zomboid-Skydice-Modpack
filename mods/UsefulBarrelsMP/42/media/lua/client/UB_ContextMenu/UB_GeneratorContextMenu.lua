...
function UB_GeneratorContextMenu.doAddFuelGenerator(worldobjects, generator, barrel, playerObj)
    if luautils.walkAdj(playerObj, generator:getSquare()) then
        if generator:getFuel() < 100 then
            ISTimedActionQueue.add(UB_RefuelGeneratorAction:new(playerObj, generator, barrel.isoObject));
        end
    end
end
...
function UB_GeneratorContextMenu:DoRefuelMenu(player, context)
    local generatorMenu = context:getOptionFromName(getText("ContextMenu_Generator"))
    if not generatorMenu then return end
    local subMenu = context:getSubMenu(generatorMenu.subOption)

    local fillOption

    if subMenu:getOptionFromName(getText("ContextMenu_GeneratorAddFuel")) then 
        fillOption = subMenu:insertOptionAfter(getText("ContextMenu_GeneratorAddFuel"), getText("ContextMenu_UB_RefuelFromBarrel"))
    elseif subMenu:getOptionFromName(getText("ContextMenu_GeneratorInfo")) then
        fillOption = subMenu:insertOptionAfter(getText("ContextMenu_GeneratorInfo"), getText("ContextMenu_UB_RefuelFromBarrel"))
    end
    ...
    for _,barrel in ipairs(self.barrels) do
        local worldObjects = UB_Utils.GetWorldItemsNearby(barrel.square, UB_Const.TOOL_SCAN_DISTANCE)
...
function UB_GeneratorContextMenu:new(player, context, worldobjects, test)
    ...
    o.barrels = UB_Utils.GetBarrelsNearby(o.generator:getSquare(), UB_Const.GENERATOR_SCAN_DISTANCE, Fluid.Petrol)

    if not SandboxVars.UsefulBarrels.EnableGeneratorRefuel then return end

    if #o.barrels == 0 then return end
    if not UB_Utils.CanCreateGeneratorMenu(o.generator:getSquare(), o.playerObj) then return end
