...
function NB_BuildingPanel:createBuildIsoEntity(dontSetDrag)
    ...
    -- Read current selection from BuildLogic (defensive: methods differ across versions/mod states).
    local logic = self.logic
    local recipe = (logic and logic.getRecipe) and logic:getRecipe() or nil
    local info   = (logic and logic.getSelectedBuildObject) and logic:getSelectedBuildObject() or nil

    -- Wall covering params exist for paint/plaster/wallpaper/sign recipes in B42.13+.
    local wallParams = (logic and logic.getWallCoveringParams) and logic:getWallCoveringParams() or nil

    -- If it's a wall-covering recipe, create the correct cursor like vanilla does.
    if info == nil and recipe ~= nil and wallParams ~= nil then
        -- Convert actionType to string safely (enum in B42.13+: WallCoveringType).
        local actionType = wallParams.actionType
        local actionStr = nil
        if actionType and actionType.toString then
            actionStr = actionType:toString()
        elseif actionType ~= nil then
            actionStr = tostring(actionType)
        end

        -- Detect wallpaper action.
        local isWallpaper = false
        if WallCoveringType and actionType == WallCoveringType.WALLPAPER then
            isWallpaper = true
        elseif actionStr and actionStr:lower() == "wallpaper" then
            isWallpaper = true
        end

        -- Vanilla behavior: for wall covering, set the drag cursor immediately.
        if isWallpaper and ISPaperCursor and WallPaper and wallParams.wallpaperType then
            local paperDef = (WallPaper["wall"] and WallPaper["wall"][wallParams.wallpaperType]) or nil
            self.buildEntity = ISPaperCursor:new(player, wallParams.wallpaperType, paperDef)
            getCell():setDrag(self.buildEntity, player:getPlayerNum())
            return
        end

        if (not isWallpaper) and ISPaintCursor and actionStr then
            self.buildEntity = ISPaintCursor:new(player, actionStr, wallParams)
            getCell():setDrag(self.buildEntity, player:getPlayerNum())
            return
        end

        -- If required classes are missing for any reason, fall back to default NB behavior below.
    end
...
function NB_BuildingPanel:updateCollapseState()
    ...
    -- Safely check mouse-over state; BuildingInfoPanel may not exist yet on first frames.
    local infoPanel = self.BuildingInfoPanel
    local infoMouseOver = false
    if infoPanel and infoPanel.isMouseOver then
        infoMouseOver = infoPanel:isMouseOver()
    end
    shouldExpand = self:isMouseOver() or infoMouseOver
    end
