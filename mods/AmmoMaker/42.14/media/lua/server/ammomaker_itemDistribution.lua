...
local function initItemDistribution()

    --OnPreDistributionMerge => initItemDistribution is the earliest event used by Ammo Maker
    --Happens before OnInitGlobalModData => initRecipes
    --Functions and variables from other mods are available at this point
    --Gets called on server and client in multiplayer
    --Gets called in single player

    ammoMakerUpdateCompatibleMods();

    local addAmmoTypesFunctions = {
    ...
    for _, addAmmoTypesFunction in ipairs(addAmmoTypesFunctions) do

        if type(addAmmoTypesFunction) == "function" then

            addAmmoTypesFunction();
...
Events.OnPreDistributionMerge.Add(initItemDistribution);