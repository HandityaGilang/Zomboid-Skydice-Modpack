                ...
                local produceAmmoScript = [[
                    {
                        ...
                        inputs
                        {
                            ...
                            item 1 tags[ammomaker_ReloadingPress] mode:keep,
                        }
                        outputs
                        {
    ...
    disAmmoScript = [[
        {
            inputs
            {
                item 1 []] .. disAmmoInputItems .. [[] mappers[casingType;bulletType],
    ...
    packAmmoPartsScript = [[
        {
            inputs
            {
                item 25 []] .. packAmmoPartsInputItems .. [[] flags[IsFull;ItemCount;IsExclusive] mappers[ammoPartType],
            }
            outputs
            {
    ...
    unpackAmmoPartsScript = [[
        {
            inputs
            {
                item 1 []] .. unpackAmmoPartsInputItems .. [[] mappers[ammoPackType],
            }
            outputs
            {
    ...
    polCasingScript = [[
        {
            inputs
            {
                item 1 []] .. polCasingInputItems .. [[] mappers[casingType],
...
local function updateRecipesInactive()
    ...
    if SandboxVars.ammomakerOptions.AllowConvertRecipes == false then
        ...
        inactiveRecipes["Covert556to223"]           = true; --3183820077/Guns93TestB42
        inactiveRecipes["Covert20556to20223"]       = true; --3183820077/Guns93TestB42
        inactiveRecipes["Covert556Boxto223Box"]     = true; --3183820077/Guns93TestB42
        inactiveRecipes["Convert556to223"]          = true; --3183820077/Guns93TestB42 (corrected)
        inactiveRecipes["Convert20556to20223"]      = true; --3183820077/Guns93TestB42 (corrected)
        inactiveRecipes["Convert556Boxto223Box"]    = true; --3183820077/Guns93TestB42 (corrected)
        inactiveRecipes["Convert.223to5.56"]        = true; --3387222454/B42RainsFirearmsAndGunParts
        inactiveRecipes["Convert5.56to.223"]        = true; --3387222454/B42RainsFirearmsAndGunParts
        inactiveRecipes["Convert.308to7.62x51"]     = true; --3387222454/B42RainsFirearmsAndGunParts
        inactiveRecipes["Convert7.62x51to.308"]     = true; --3387222454/B42RainsFirearmsAndGunParts
        inactiveRecipes["Convert.223Boxto5.56Box"]  = true; --3387222454/B42RainsFirearmsAndGunParts
        inactiveRecipes["Convert5.56Boxto.223Box"]  = true; --3387222454/B42RainsFirearmsAndGunParts
        inactiveRecipes["Convert50.308to7.62x51"]   = true; --3387222454/B42RainsFirearmsAndGunPartsExpanded
        inactiveRecipes["Convert507.62x51to.308"]   = true; --3387222454/B42RainsFirearmsAndGunPartsExpanded

    end
    
end

local function deactivateRecipe(recipe, recipeName)
...
local function initRecipes()
    ...
    for i=0,recipes:size()-1 do
        ...
        elseif inactiveRecipes[recipeName] then
            ...
            deactivateRecipe(recipe, recipeName);
