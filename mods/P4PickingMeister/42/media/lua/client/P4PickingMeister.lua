...
-- Common
P4PickingMeister.queue = P4PickingMeisterQueue:new()
		...
		if instanceof(v.param1, "CraftRecipe") then
			...
			if menu then
				...
				if menu[1] == 1 then
					...
					elseif P4PickingMeister.queue:isNotEmpty() then
						option.notAvailable = true
						option.toolTip = ISInventoryPaneContextMenu.addToolTip()
				...
				elseif menu[1] == 3 then
					local option = context:insertOptionAfter(v.name, P4PickingMeister.Menu_TakeCigaretteAndSmoke, item, P4PickingMeister.onTakeAndSmoke, func, recipe, player, all, 1)
					option.iconTexture = v.iconTexture
					...
					if not found then
						local cigarette = instanceItem("Base.CigaretteSingle")
						local list = cigarette:getRequireInHandOrInventory()
						for i = 0, list:size() - 1 do
							local fullType = moduleDotType(cigarette:getModule(), list:get(i))
...
P4PickingMeister.onOpenAndEat = function(item, func, recipe, player, all, percent)
	...
	local playerObj = getSpecificPlayer(player)
	P4PickingMeister.queue:enqueue(percent)
...
P4PickingMeister.onTakeAndSmoke = function(item, func, recipe, player, all, percent)
	-- Add a Walk Check to the vanilla menu as there is no check.
	-- If the vanilla side is fixed, this check will no longer be necessary.
	local playerInv = getPlayerInventory(player).inventory
	local parent = item:getContainer()
...
function Actions.addOrDropItem(playerObj, item)
	...
	if P4PickingMeister.queue:isNotEmpty() then
		local percent = P4PickingMeister.queue:dequeue()
