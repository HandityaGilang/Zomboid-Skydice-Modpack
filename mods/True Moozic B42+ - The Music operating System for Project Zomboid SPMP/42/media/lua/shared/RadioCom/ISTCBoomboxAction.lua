...
-- Client-only UI/function files: never present on a dedicated server
-- (require them there = "require failed" WARN spam at boot).
if not isServer() then
    require "TCMusicClientFunctions"
    pcall(function() require "RadioCom/ISTCBoomboxWindow" end)
end

-- B42.20 MP anti-cheat: sendAddItemToContainer/sendRemoveItemFromContainer are
-- SERVER->CLIENT broadcasts. Calling them from a CLIENT fires SyncItemAdd/SyncItemDelete
-- packets that anti-cheat rejects (reason="SyncItemDelete: no capability" -> kick).
-- These file-local shadows gate every call site in this file behind isServer().
local _globalSendAdd = sendAddItemToContainer
local _globalSendRemove = sendRemoveItemFromContainer
local function sendAddItemToContainer(container, item)
	if isServer() and _globalSendAdd and container and item then
		_globalSendAdd(container, item)
	end
end
local function sendRemoveItemFromContainer(container, item)
	if isServer() and _globalSendRemove and container and item then
		_globalSendRemove(container, item)
	end
end
	...
	if self.device:getModData().tcmusic.isPlaying then
		...
		if not (self.device:getModData().tcmusic.deviceType == "VehiclePart") then
			...
			ModData.getOrCreate("trueMusicData")["now_play"][musicId] = {
				...
				startId = self.device:getModData().tcmusic.startId,
			}
			if self.device:getModData().tcmusic.deviceType == "InventoryItem" then
				ModData.getOrCreate("trueMusicData")["now_play"][musicId]["itemid"] = self.device:getID()
			end
			if isServer() then ModData.transmit("trueMusicData") end
		end
	end
end
	...
	if self:isValidToggleOnOff() then
		...
		if self.device:getModData().tcmusic.deviceType == "InventoryItem" then
			if self.device:getModData().tcmusic.isPlaying then
				...
				-- Server side: no local sound handle exists here; playing a stop on
				-- the character emitter with a nil id crashed this function mid-way
				-- (musicId was an undefined global too) leaving the server device
				-- state permanently desynced. Just clear the authoritative row.
...
function ISTCBoomboxAction:completeRemoveBattery()
	...
	if self.device:getModData().tcmusic.deviceType == "InventoryItem" then
		if self.device:getModData().tcmusic.isPlaying then
			self.device:getModData().tcmusic.isPlaying = false
			-- Server side: no local sound handle; the old stopSound(nil) +
			-- undefined-global musicId crashed here and the battery was never
			-- removed on the server (battery/state desync).
			local musicId = getPortableMusicId(self.character)
			if musicId ~= nil then
				ModData.getOrCreate("trueMusicData")["now_play"][musicId] = nil
				if isServer() then ModData.transmit("trueMusicData") end
...
function ISTCBoomboxAction:completeRemoveHeadphones()
	...
	if self.device:getModData().tcmusic.deviceType == "InventoryItem" and self.device:getFullType() and TCMusic.WalkmanPlayer[self.device:getFullType()] then
		if self.device:getModData().tcmusic.isPlaying then
			self.device:getModData().tcmusic.isPlaying = false
			-- Server side: no local sound handle; old code crashed on
			-- stopSound(nil) + undefined-global musicId.
			local musicId = getPortableMusicId(self.character)
			if musicId ~= nil then
				ModData.getOrCreate("trueMusicData")["now_play"][musicId] = nil
				if isServer() then ModData.transmit("trueMusicData") end
...
function ISTCBoomboxAction:completeTogglePlayMedia()
	-- SERVER SIDE (this file's complete* only ever runs on the MP server; the
	-- client file re-derives the class without complete*).
	--
	-- Deliberately a no-op. The old body did two catastrophic things in MP:
	--  1. It TOGGLED tcmusic.isPlaying on the server's copy of the item. If the
	--     client and server copies ever disagreed, every subsequent press
	--     flipped them in OPPOSITE directions forever (play/stop button flap,
	--     phantom "now playing" rows, permanent desync).
	--  2. It called playSound on the character's SERVER emitter, which the
	--     engine broadcasts to every nearby client as an unmanaged one-shot
	--     sound that none of our stop paths can reach (music kept playing
	--     after the tape was removed).
	--
	-- The client perform() is authoritative for local audio, and it mirrors
	-- ABSOLUTE state to the server via the 'setPortablePlayback' /
	-- 'setWorldDevicePlayback' / 'setMediaItemToVehiclePart' commands, which
	-- also stamp the server-side item modData (see TCServerCommands.lua).
	return true
end
...
function ISTCBoomboxAction:completeAddMedia()
	...
	local container = self.secondaryItem:getContainer()
		...
		-- B42.20 MP: send*ItemContainer must only ever run on the SERVER.
		-- Calling them from a client fires SyncItemDelete/SyncItemAdd packets that
		-- anti-cheat rejects ("no capability" kick). Local removal below is enough
		-- on the client; TMItemMPSync server commands keep the server authoritative.
