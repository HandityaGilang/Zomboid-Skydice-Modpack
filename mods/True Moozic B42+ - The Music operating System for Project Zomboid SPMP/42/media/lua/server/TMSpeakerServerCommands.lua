...
local function isMasterDevice(obj)
    ...
    -- Converted TM jukebox (IsoThumpable carrying the tm_jukebox container)
    if obj.getContainer then
        local c = obj:getContainer()
        if c and c.getType and c:getType() == "tm_jukebox" then return true end
    end
    -- Jukebox by transmitted modData (conversion stamps md.tmJukebox and
    -- transmits it - survives even when the server's container/sprite state
    -- lags behind the client's).
    if obj.getModData then
        local md = obj:getModData()
        if md and md.tmJukebox then return true end
    end
    -- Jukebox by sprite property (CustomName == "Jukebox") - covers both the
    -- vanilla tile and the converted IsoThumpable regardless of container
    -- sync state, matching the detection the client used to offer connect.
    if obj.getSprite then
        local spr = obj:getSprite()
        if spr and spr.getProperties then
            local props = spr:getProperties()
            if props and props:get("CustomName") == "Jukebox" then return true end
        end
    end
...
local function consumeWire(player, ftNeeded)
    ...
    for _, entry in ipairs(sorted) do
        ...
        if newFt <= 0 then
            ...
            -- Broadcast the removal so clients don't keep a ghost bundle.
            if sendRemoveItemFromContainer then
                sendRemoveItemFromContainer(inv, entry.item)
            end
        else
...
------------------------------------------------------------------
-- Command: consumeWireItem
-- Client-side wire-bundle merge emptied a bundle; remove the
-- server's copy authoritatively (client already removed its own).
------------------------------------------------------------------
function TMSpeakerCommands.consumeWireItem(player, args)
    ...
    for i = 0, items:size() - 1 do
        ...
        if it and it.getID and it:getID() == args.itemId then
            local container = it:getContainer() or inv
