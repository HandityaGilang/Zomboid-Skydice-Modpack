...
-- CD albums from packs (e.g. TM_CDTEST.CD_AlbumKornCD) start with "CD_" or end in "CD".
-- Exclude devices/cases (CDPlayer, CDCase, CDCarryingCase).
local function isCDAlbumType(typeName)
    if type(typeName) ~= "string" then return false end
    local lower = string.lower(typeName)
...
local function splitDetectedModItems(mediaItems)
    ...
    if not mediaItems then
        return cassetteItems, vinylItems, suppressedRecordItems, cdItems
    end
    ...
    for _, media in ipairs(mediaItems) do
        ...
        if typeName then
            ...
            elseif isCDAlbumType(typeName) then
                table.insert(cdItems, media)
            end
        end
    end
...
local function ensureDetectedMediaHasDistribution(cassetteItems, vinylItems, cdItems)
    local cassetteLists = {
    ...
    local cdLists = {
        {name = "MusicStoreCDs", weight = 2.5},
        {name = "CrateCompactDiscs", weight = 2.0},
        {name = "MusicStoreOthers", weight = 1.5},
        {name = "ElectronicStoreMusic", weight = 1.0},
        {name = "LivingRoomShelf", weight = 0.2},
        {name = "BedroomDresser", weight = 0.1},
        {name = "SchoolLockers", weight = 0.1},
    }
    ...
    for _, media in ipairs(cdItems or {}) do
        local fullType = media and media.fullType or nil
        if fullType then
            for _, cfg in ipairs(cdLists) do
                addItemToProcListIfMissing(cfg.name, fullType, cfg.weight)
            end
            addItemToVehicleDistIfMissing("GloveBox", fullType, 0.05)
            addItemToAllVehicleTrunksIfMissing(fullType, 0.1)
        end
    end
end
...
function TCSpawnController.AdjustAllDistributions()
    ...
    if TCMusicModDetector and TCMusicModDetector.CassetteItems and TCMusicModDetector.DetectedMods then
        for modID, mediaItems in pairs(TCMusicModDetector.CassetteItems) do
            ...
            local cdMediaRate = scaleSpawnRate((TCMusicSandbox.GetCDSpawnRate and TCMusicSandbox.GetCDSpawnRate() or masterRate) * modRate)
            local modInfo = TCMusicModDetector.DetectedMods[modID]
        
            if modInfo then
                dlog(string.format("TCSpawnController: %s cassette rate: %.2f | vinyl media rate: %.2f | cd rate: %.2f", modInfo.name, cassetteRate, vinylMediaRate, cdMediaRate))

                local cassetteItems, vinylItems, suppressedRecordItems, cdItems = splitDetectedModItems(mediaItems)
                ...
                for _, v in ipairs(cdItems) do table.insert(combined, v) end
                ...
                if shouldInject then
                    ensureDetectedMediaHasDistribution(cassetteItems, vinylItems, cdItems)
                end
                ...
                if #cdItems > 0 then
                    TCSpawnController.ApplyMultipliersToMod(modID .. "_cds", cdMediaRate, cdItems)
                end
