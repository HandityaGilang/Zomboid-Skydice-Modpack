...
-- FMOD culls file sounds beyond the sound script's distanceMax (base packs
-- 120, third-party packs often 50-75). A culled sound reports
-- isPlaying=false — indistinguishable from a finished track. Beyond
-- FAR_TRUST_DIST we therefore never treat "not playing" as a track end
-- (it would stop/advance the shared device state for EVERYONE).
...
local function stopCD(state)
    ...
    stopSound(state.cdSkipWaitEmitter, state.cdSkipWaitSound)
    state.cdSound   = nil
    state.cdEmitter  = nil
    state.cdStartMs = nil
    state.cdSkipDelay = nil
    state.cdSkipWaitSound = nil
    state.cdSkipWaitEmitter = nil
end
...
local function manageCD(obj, md, dd, state)
    ...
    local wvKey = TCMusic and TCMusic.worldVolKey and TCMusic.worldVolKey(obj:getX(), obj:getY(), obj:getZ()) or nil
    local baseVol = (TCMusic and TCMusic.getListenVolume and TCMusic.getListenVolume(dd:getDeviceVolume(), wvKey) or dd:getDeviceVolume()) * 0.4
    local vol = calculateVolumeByDistance(obj, baseVol)
    local p = getPlayer()
    local dist = p and getDistance(p, obj) or 0
    local trusted = dist <= FAR_TRUST_DIST

    -- Scratch skip effect playing: hold everything until it finishes.
    if state.cdSkipWaitSound and state.cdSkipWaitEmitter then
        if state.cdSkipWaitEmitter:isPlaying(state.cdSkipWaitSound) then
            return
        end
        state.cdSkipWaitSound = nil
        state.cdSkipWaitEmitter = nil
    ...
    -- Scratched CD: force the current track to end once its rolled delay elapses.
    if state.cdSound and state.cdEmitter and state.cdSkipDelay and state.cdStartMs then
        if (getTimestampMs() - state.cdStartMs) >= state.cdSkipDelay * 1000 then
            state.cdSkipDelay = nil
            state.cdEmitter:stopSound(state.cdSound)
            -- Play a random skip effect; the next track waits for it to finish.
            local skipName = TCMusic and TCMusic.getRandomCDSkipSound and TCMusic.getRandomCDSkipSound() or nil
            if skipName then
                local sid = playWorldSound(emitter, skipName, obj)
                if sid then
                    if emitter.set3D then emitter:set3D(sid, true) end
                    emitter:setVolume(sid, vol)
                    state.cdSkipWaitSound = sid
                    state.cdSkipWaitEmitter = emitter
                    return
                end
            end
        end
    ...
    if state.cdSound and state.cdEmitter then
        if not state.cdEmitter:isPlaying(state.cdSound) then
            if not trusted then
                -- Far: engine culled our local sound, the track did NOT end.
                state.cdSound = nil
                state.cdEmitter = nil
            ...
            state.cdSound   = nil
            state.cdEmitter  = nil
            local noLoop = TCMusic and TCMusic.isScratchNoLoop and TCMusic.isScratchNoLoop(md.hifiCD) or false
            local atEnd = (md.hifiCD.currentTrack or 1) >= (md.hifiCD.totalTracks or 1)
            if md.hifiCD.totalTracks and md.hifiCD.totalTracks > 1 and not (noLoop and atEnd) then
                ...
                -- Single-track CD, or 100% scratched CD after the last track: stop, don't loop.
                md.hifiCD.isPlaying = false
                if obj.transmitModData then obj:transmitModData() end
                return
            end
    ...
    if not state.cdSound then
        ...
        if track and track.soundName then
            ...
            if state.cdSound then
                ...
                state.cdVolume = vol
                -- Scratched CD: roll the forced-skip timer for this track.
                state.cdStartMs = getTimestampMs()
                state.cdSkipDelay = TCMusic and TCMusic.getScratchSkipDelay
                    and TCMusic.getScratchSkipDelay(md.hifiCD, md.hifiCD.currentTrack or 1, md.hifiCD.totalTracks or 1)
                    or nil
            end
        end
    else
        ...
        if state.cdEmitter and state.cdSound then
            ...
            if TMMood then TMMood.report(vol) end
        end
    end
end
...
local function manageTape(obj, md, dd, state)
    ...
    if state.tapeSound and state.tapeEmitter then
        if not state.tapeEmitter:isPlaying(state.tapeSound) then
            if dist > FAR_TRUST_DIST then
                -- Far: engine culled it, the tape did NOT end.
                state.tapeSound = nil
                state.tapeEmitter = nil
                state.tapeFarCulledDist = dist
                return
            end
            md.hifiTape.isPlaying = false
            state.tapeSound   = nil
    ...
    if not state.tapeSound then
        if state.tapeFarCulledDist then
            if dist >= state.tapeFarCulledDist - 5 then return end
            state.tapeFarCulledDist = nil
        end
...
local function manageVinyl(obj, md, dd, state)
    ...
    if state.vinylSound and state.vinylEmitter then
        if not state.vinylEmitter:isPlaying(state.vinylSound) then
            if dist > FAR_TRUST_DIST then
                -- Far: engine culled it, the record did NOT end.
                state.vinylSound = nil
                state.vinylEmitter = nil
                state.vinylFarCulledDist = dist
                return
            end
            md.hifiVinyl.isPlaying = false
            state.vinylSound   = nil
    ...
    if not state.vinylSound then
        if state.vinylFarCulledDist then
            if dist >= state.vinylFarCulledDist - 5 then return end
            state.vinylFarCulledDist = nil
        end
