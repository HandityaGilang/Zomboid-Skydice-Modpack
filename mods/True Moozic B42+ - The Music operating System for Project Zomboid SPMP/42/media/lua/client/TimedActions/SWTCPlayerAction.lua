...
-- MP: the portable CD player's customMusic state lives in the ITEM's
-- modData. Without syncItemFields after a write the server keeps a STALE
-- copy of the item and B42's item-transaction check silently rejects
-- equipping/dropping the device (and the state dies on relog). Items
-- nested inside a bag are skipped (remote ItemStats ContainerID NPE);
-- they re-sync on the next direct mutation.
local function syncDeviceItem(device)
...
function SWTCPlayerAction:performAddMedia()
    ...
    if cdData then
        ...
        if TCMusic and TCMusic.copyScratchToDevice then
            TCMusic.copyScratchToDevice(modData.customMusic, inventoryItem)
        end
    else
        return
    end
    ...
    if inventoryItem:isInPlayerInventory() then
        ...
        -- B42.20: removeItemOnServer removed (client-side item-delete packet trips anti-cheat).
        -- TMItemMPSync's consumeCD server command performs the authoritative removal.
        container:DoRemoveItem(inventoryItem)
    end
    syncDeviceItem(self.device)
end
...
function SWTCPlayerAction:performRemoveMedia()
    ...
    if itemFullType then
        ...
        if inventory and inventory.AddItem then
            local newItem = inventory:AddItem(itemFullType)
            if newItem and TCMusic and TCMusic.restoreScratchToItem then
                TCMusic.restoreScratchToItem(newItem, modData.customMusic)
            end
        end
    end
    ...
    if TCMusic and TCMusic.clearScratchOnDevice then
        TCMusic.clearScratchOnDevice(modData.customMusic)
    end
    syncDeviceItem(self.device)
end
...
function SWTCPlayerAction:playCurrentDeviceAudio(soundName)
    ...
    if self.character:getModData().customMusicIds[deviceId] then
        local actualVolume = (TCMusic.getListenVolume and TCMusic.getListenVolume(self.deviceData:getDeviceVolume()) or self.deviceData:getDeviceVolume()) * 0.4
        self.character:getEmitter():setVolume(self.character:getModData().customMusicIds[deviceId], actualVolume)
        return true
    else
        return false
