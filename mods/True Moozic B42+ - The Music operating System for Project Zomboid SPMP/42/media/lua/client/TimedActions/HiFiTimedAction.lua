...
-- Placed (IsoObject) HiFi deck changes must go through the server: the
-- client-created IsoRadio is client-local, so a raw transmitModData is
-- DROPPED server-side ("object is null") - the deck state then never
-- reaches other players and dies on chunk reload. TCMusic.syncHiFiSlot
-- sends the deck through the 'setHiFiSubtable' server command which
-- stamps the floor item (persisted) and broadcasts.
-- Vehicle-part decks live in the PART's modData, which the server DOES
-- know - but only if we transmit it. Without transmitPartModData on
-- insert/eject the server's copy stays empty, and exiting/re-entering
-- the vehicle resyncs the part FROM the server, wiping the deck (the
-- "cassette shows for a split second then deletes itself" bug).
local function syncDeckState(device, slot)
    if instanceof(device, "IsoObject") then
        if TCMusic and TCMusic.syncHiFiSlot then
            TCMusic.syncHiFiSlot(device, slot)
        elseif device.transmitModData then
            device:transmitModData()
        end
    elseif instanceof(device, "VehiclePart") then
        local v = device:getVehicle()
        if v and v.transmitPartModData then
            v:transmitPartModData(device)
        end
    end
end
...
function HiFiTimedAction:performAddCD()
    ...
    if cdData then
        ...
        if TCMusic and TCMusic.copyScratchToDevice then
            TCMusic.copyScratchToDevice(md.hifiCD, item)
        end
        local wasInPlayer = item.isInPlayerInventory and item:isInPlayerInventory()
        local itemId = item.getID and item:getID() or nil
        local itemFT = item:getFullType()
        container:DoRemoveItem(item)
        -- MP: the local removal is client-only; the server must delete its
        -- copy too or the CD ghosts back / desyncs the inventory.
        if isClient() and wasInPlayer then
            sendClientCommand(self.character, "TMItemMPSync", "consumeCD", { itemId = itemId, fullType = itemFT })
        end
        syncDeckState(self.device, "hifiCD")
    end
end
...
function HiFiTimedAction:performRemoveCD()
    ...
    if fullType then
        -- MP: create the CD SERVER-side (giveCD broadcasts it back); a
        -- client-side AddItem makes a ghost copy the server never knew
        -- about (unequippable, undroppable, ItemStats errors for everyone).
        if isClient() then
            sendClientCommand(self.character, "TMItemMPSync", "giveCD", {
                fullType = fullType,
                scratch = md.hifiCD.scratch,
                scratchDelay = md.hifiCD.scratchDelay,
                scratchSafeRoll = md.hifiCD.scratchSafeRoll,
            })
        else
            local inv = self.character:getInventory()
            if inv and inv.AddItem then
                local newItem = inv:AddItem(fullType)
                if newItem and TCMusic and TCMusic.restoreScratchToItem then
                    TCMusic.restoreScratchToItem(newItem, md.hifiCD)
                end
            end
        end
    end
    ...
    if TCMusic and TCMusic.clearScratchOnDevice then
        TCMusic.clearScratchOnDevice(md.hifiCD)
    end
    syncDeckState(self.device, "hifiCD")
end
...
function HiFiTimedAction:performAddCassette()
    ...
    container:DoRemoveItem(item)
    if isClient() and wasInPlayer then
        sendClientCommand(self.character, "TMItemMPSync", "consumeTape", { itemId = itemId, fullType = itemFT })
    end
    syncDeckState(self.device, "hifiTape")
end
...
function HiFiTimedAction:performRemoveCassette()
    ...
    if mediaItem then
        -- MP: server-side give (client AddItem = ghost item).
        if isClient() then
            sendClientCommand(self.character, "TMItemMPSync", "giveTape", { fullType = mediaItem })
        else
            local inv = self.character:getInventory()
            if inv and inv.AddItem then
...
function HiFiTimedAction:performAddVinyl()
    ...
    syncDeckState(self.device, "hifiVinyl")
end
