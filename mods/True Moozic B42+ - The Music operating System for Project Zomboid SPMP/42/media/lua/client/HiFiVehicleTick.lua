...
local HEAR_DIST = 110         -- max tile distance to manage audio (mod fade hits 0 at 100)
local FADE_RADIUS = 80        -- full volume inside this range
local HEAR_RADIUS = 100       -- fades to zero here (matches world devices)
...
local function ensureState(vid)
    if not HiFiVehicleAudio.vehicles[vid] then
        HiFiVehicleAudio.vehicles[vid] = {
            ...
            tapeEmitter  = nil,
            lastCDTrack  = nil,
            vehicle      = nil,  -- vehicle ref for per-tick emitter repositioning
        }
    end
...
local function getVolume(player, vehicle, deviceData)
    local devVol = deviceData:getDeviceVolume()
    local vvKey = vehicle and ("V:" .. tostring(vehicle:getId())) or nil
    local setVol = (TCMusic and TCMusic.getListenVolume and TCMusic.getListenVolume(devVol, vvKey) or devVol) * 0.4
    ...
    if insideThisVehicle then
        ...
        -- Manual 80->100 tile fade: sound scripts keep the stream alive to
        -- distanceMax 500 (for mid-song resume), so the engine no longer
        -- fades it out by distance on its own.
        local dx = player:getX() - vehicle:getX()
        local dy = player:getY() - vehicle:getY()
        local d = math.sqrt(dx * dx + dy * dy)
        if d >= HEAR_RADIUS then return 0 end
        if d > FADE_RADIUS then
            setVol = setVol * (1 - (d - FADE_RADIUS) / (HEAR_RADIUS - FADE_RADIUS))
        end
        return setVol
    end
end
...
local function manageCD(player, vehicle, part, md, deviceData, state)
    ...
    local inside = (player:getVehicle() == vehicle)
    ...
    if not state.cdSound then
        ...
        if track and track.soundName then
            ...
            if state.cdSound then
                ...
                -- 2D for occupants: a 3D positional sound pans hard L/R as
                -- the (lagging) emitter shifts around the moving listener.
                if emitter.set3D then emitter:set3D(state.cdSound, not inside) end
            end
        end
    else
        ...
        if state.cdEmitter and state.cdSound then
            ...
            if state.cdEmitter.set3D then state.cdEmitter:set3D(state.cdSound, not inside) end
            if TMMood then TMMood.reportAt(vehicle:getX(), vehicle:getY(), vol) end
        end
    end
end
    ...
    if not state.tapeSound then
        ...
        if soundName then
            ...
            if state.tapeSound then
                ...
                if emitter.set3D then emitter:set3D(state.tapeSound, not inside) end
            end
        end
    else
        ...
        if state.tapeEmitter and state.tapeSound then
            ...
            if state.tapeEmitter.set3D then state.tapeEmitter:set3D(state.tapeSound, not inside) end
            if TMMood then TMMood.reportAt(vehicle:getX(), vehicle:getY(), vol) end
        end
    end
end
...
local function onTick()
    ...
    -- EVERY tick: glue active emitters to their vehicle. Repositioning only
    -- on the heavy 20-tick pass left the emitter trailing a moving vehicle,
    -- which made the 3D pan bounce between the L/R channels.
    for _, state in pairs(HiFiVehicleAudio.vehicles) do
        local veh = state.vehicle
        if veh then
            local vx, vy, vz = veh:getX(), veh:getY(), veh:getZ()
            if state.cdEmitter and state.cdEmitter.setPos then
                state.cdEmitter:setPos(vx, vy, vz)
            end
            if state.tapeEmitter and state.tapeEmitter.setPos then
                state.tapeEmitter:setPos(vx, vy, vz)
            end
        end
    end
