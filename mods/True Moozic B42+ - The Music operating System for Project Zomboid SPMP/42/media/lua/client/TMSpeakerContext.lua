...
-- Lets the player connect a nearby music device (Vinyl Player, HiFi Stereo
-- or Jukebox) so the speaker acts as a synced emitter
...
local function speakerSyncStart(speakerObj)
    ...
    if tms and tms.connected then
        ...
        if vx and vy and vz and getHifiSongData then
            ...
            if tms.masterType == "hifi" or tms.masterType == "jukebox" then
                local hifiSong = getHifiSongData(vx, vy, vz)
                if not hifiSong and TMJukebox and TMJukebox.getPlayingSong then
                    hifiSong = TMJukebox.getPlayingSong(vx, vy, vz)
                end
                if hifiSong then
