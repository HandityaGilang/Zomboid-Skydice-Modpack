...
function TCRWMMedia:update()
    ...
    if self.player and self.device and self.deviceData and self.device:getModData().tcmusic then
        ...
        elseif self.device:getModData().tcmusic.deviceType == "InventoryItem" then
            -- Walkman/boombox sessions (client-local play state): the session
            -- IS the play state - item modData is never written for play/stop,
            -- so a server echo can't flip this button anymore.
            local ws = TCMusic.WalkmanSession
            local wsPlaying = ws and self.device.getID and ws.itemId == self.device:getID()
            local bs = TCMusic.BoomboxSession
            local bsPlaying = bs and self.device.getID and bs.itemId == self.device:getID()
            -- Stop button while a session claims this device (covers the
            -- sync-start window before the sound handle exists) OR while the
            -- item still SAYS playing (legacy builds) OR while our portable
            -- sound handle is actually running.
            if wsPlaying or bsPlaying or (self.device:getModData().tcmusic.mediaItem and
                    (self.device:getModData().tcmusic.isPlaying or
        ...
        if self.device:getModData().tcmusic.mediaItem then
            ...
            local sessionOnThis = self.device.getID and
                ((TCMusic.WalkmanSession and TCMusic.WalkmanSession.itemId == self.device:getID())
                or (TCMusic.BoomboxSession and TCMusic.BoomboxSession.itemId == self.device:getID()))
            if self.device:getModData().tcmusic.mediaItem and (self.device:getModData().tcmusic.isPlaying or sessionOnThis or (self.device:getModData().tcmusic.deviceType == "VehiclePart" and self.device:getVehicle():getEmitter() and self.device:getVehicle():getEmitter():isPlaying(self.device:getModData().tcmusic.mediaItem))) then
