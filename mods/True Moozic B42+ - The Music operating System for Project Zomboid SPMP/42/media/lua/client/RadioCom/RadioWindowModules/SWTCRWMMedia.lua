...
function SWTCRWMMedia:performAddMedia(item)
    ...
    if cdData then
        ...
        if TCMusic and TCMusic.copyScratchToDevice then
            TCMusic.copyScratchToDevice(modData.customMusic, item)
        end
        local container = item:getContainer()
        if container then
            -- MP: this drag-drop path bypasses SWTCPlayerAction (and its
            -- TMItemMPSync hook), so mirror the removal on the server here.
            if isClient() then
                sendClientCommand(self.player, "TMItemMPSync", "consumeCD", {
...
function SWTCRWMMedia:addOriginalCD(item)
    ...
    -- NOTE: vanilla DeviceData:addMediaItem removes the item from its
    -- container itself and handles MP sync natively (see vanilla
    -- ISDeviceMediaAction). The DoRemoveItem below is only a fallback no-op;
    -- do NOT send an item-consume command here or a second retail disc
    -- could be removed server-side.
    local container = item:getContainer()
    if container then
        container:DoRemoveItem(item)
    end
...
function SWTCRWMMedia:stopCurrentTrack()
    ...
    -- Also cancel a pending scratch-skip effect (manual stop / track change / power off).
    if self.skipWaitSoundId then
        self.player:getEmitter():stopSound(self.skipWaitSoundId)
        self.skipWaitSoundId = nil
        self.skipWaitAdvance = nil
    end
end
...
function SWTCRWMMedia:playCurrentTrack()
    ...
    -- Only snapshot the user's music volume once; on track advance it is already 0.
    if self.player:getModData().originalMusicVolume == nil then
        self.player:getModData().originalMusicVolume = getCore():getOptionMusicVolume()
    end
    ...
    if currentSoundName then
        ...
        if self.player:getModData().customMusicIds[deviceId] then
            ...
            -- Scratched CD: arm the forced-skip timer for this track.
            modData.customMusic.trackStartMs = getTimestampMs()
            modData.customMusic.trackSkipDelay = TCMusic.getScratchSkipDelay
                and TCMusic.getScratchSkipDelay(modData.customMusic, modData.customMusic.currentTrack, modData.customMusic.totalTracks)
                or nil
...
function SWTCRWMMedia:nextTrack()
    ...
    if TMSpeech then TMSpeech.announce(self.player or getPlayer(), "Next") end
...
function SWTCRWMMedia:previousTrack()
    if not self:hasCustomCD() then return end
    if TMSpeech then TMSpeech.announce(self.player or getPlayer(), "Prev") end
...
function SWTCRWMMedia:update()
    ...
    if self.player and self.device and self.deviceData then
        ...
        if isOn and self:isPlayingCustomCD() then
            ...
            if deviceId and self.player:getModData().customMusicIds and self.player:getModData().customMusicIds[deviceId] then
                ...
                if emitter and not emitter:isPlaying(self.player:getModData().customMusicIds[deviceId]) then
                    local modData = self.device:getModData()
                    local noLoop = TCMusic.isScratchNoLoop and TCMusic.isScratchNoLoop(modData.customMusic) or false
                    local atEnd = modData.customMusic and modData.customMusic.currentTrack and modData.customMusic.totalTracks
                        and modData.customMusic.currentTrack >= modData.customMusic.totalTracks or false
                    if modData.customMusic and modData.customMusic.tracks and modData.customMusic.totalTracks > 1 and not (noLoop and atEnd) then
                        modData.customMusic.currentTrack = modData.customMusic.currentTrack + 1
                        if modData.customMusic.currentTrack > modData.customMusic.totalTracks then
                        ...
                        -- Single-track CD, or a 100% scratched CD that reached the last track: stop.
                        modData.customMusic.isPlaying = false
                        self.player:getModData().customMusicIds[deviceId] = nil
                        local originalVolume = self.player:getModData().originalMusicVolume
                        if originalVolume then
        ...
        -- Scratch skip effect finished: start the deferred next track.
        if self.skipWaitSoundId then
            local em = self.player:getEmitter()
            if not (em and em:isPlaying(self.skipWaitSoundId)) then
                self.skipWaitSoundId = nil
                if self.skipWaitAdvance then
                    self.skipWaitAdvance = nil
                    local cm = self.device:getModData().customMusic
                    if isOn and cm and cm.isPlaying then
                        cm.currentTrack = (cm.currentTrack or 1) + 1
                        if cm.currentTrack > (cm.totalTracks or 1) then
                            cm.currentTrack = 1
                        end
                        self:playCurrentTrack()
                    end
        ...
        -- Scratched CD: force-skip the current track once its rolled delay elapses.
        if isOn and self:isPlayingCustomCD() and not self.skipWaitSoundId then
            local cm = self.device:getModData().customMusic
            if cm and cm.trackSkipDelay and cm.trackStartMs then
                if (getTimestampMs() - cm.trackStartMs) >= cm.trackSkipDelay * 1000 then
                    cm.trackSkipDelay = nil
                    local noLoop = TCMusic.isScratchNoLoop and TCMusic.isScratchNoLoop(cm) or false
                    local atEnd = cm.currentTrack and cm.totalTracks and cm.currentTrack >= cm.totalTracks or false
                    self:stopCurrentTrack()
                    -- Play a random skip effect on the player's emitter.
                    local skipId = nil
                    local skipName = TCMusic.getRandomCDSkipSound and TCMusic.getRandomCDSkipSound() or nil
                    if skipName then
                        skipId = playPlayerEmitterSound(self.player, skipName)
                        if skipId then
                            self.player:getEmitter():setVolume(skipId, self.deviceData:getDeviceVolume() * 0.4)
                        end
                    end
                    if noLoop and atEnd then
                        -- 100% scratched: don't loop, stop after the last track.
                        cm.isPlaying = false
                        self.skipWaitSoundId = skipId -- let the effect ring out, no advance
                        self.skipWaitAdvance = nil
                        local originalVolume = self.player:getModData().originalMusicVolume
                        if originalVolume then
                            getCore():setOptionMusicVolume(originalVolume)
                    ...
                    elseif skipId then
                        -- Wait for the skip effect to finish before the next track starts.
                        self.skipWaitSoundId = skipId
                        self.skipWaitAdvance = true
                    else
                        cm.currentTrack = (cm.currentTrack or 1) + 1
                        if cm.currentTrack > (cm.totalTracks or 1) then
                            cm.currentTrack = 1
