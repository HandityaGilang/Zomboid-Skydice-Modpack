...
-- ISItemDropBox is a vanilla global by mod load time; require path fails in B42 (WARN spam).
...
function HiFiCDPanel:onPrevTrack()
    ...
    if TMSpeech then
        if self.deviceType == "IsoObject" and self.device.getX then
            TMSpeech.announceDevice(self.device, "Prev")
        else
            TMSpeech.announce(self.player or getPlayer(), "Prev")
        end
    end
    ...
    if self.deviceType == "IsoObject" and self.device and TCMusic and TCMusic.syncHiFiSlot then
        TCMusic.syncHiFiSlot(self.device, "hifiCD")
    end
end
...
function HiFiCDPanel:onNextTrack()
    ...
    if TMSpeech then
        if self.deviceType == "IsoObject" and self.device.getX then
            TMSpeech.announceDevice(self.device, "Next")
        else
            TMSpeech.announce(self.player or getPlayer(), "Next")
        end
    end
    local wasPlaying = md.hifiCD.isPlaying
    if wasPlaying then self:stopAudio() end
...
function HiFiCDPanel:playAudio()
    ...
    if pmd.customMusicIds[did] then
        local vol = (TCMusic.getListenVolume and TCMusic.getListenVolume(self.deviceData:getDeviceVolume()) or self.deviceData:getDeviceVolume()) * 0.4
...
function HiFiCDPanel:update()
    ...
    if not isOn and md.hifiCD and md.hifiCD.isPlaying then
        md.hifiCD.isPlaying = false
        self:stopAudio()
        if self.deviceType == "IsoObject" and TCMusic and TCMusic.syncHiFiSlot then
            TCMusic.syncHiFiSlot(self.device, "hifiCD")
        end
    end
end
