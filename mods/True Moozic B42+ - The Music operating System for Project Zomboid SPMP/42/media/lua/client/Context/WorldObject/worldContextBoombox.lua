...
local function ensureIsoRadioForWorldItem(square, item)
    ...
    if objs then
        ...
        for i = 0, objs:size() - 1 do
            ...
            if tObj and instanceof(tObj, "IsoRadio") and tObj.getModData then
                ...
                -- MP: stale server-reassigned IDs - adopt by sprite instead of
                -- creating a duplicate client-local IsoRadio.
                if (not spriteFallback) and tObj.getSprite and tObj:getSprite()
                    and tObj:getSprite():getName() == spriteName then
                    spriteFallback = tObj
                end
            end
        end
        if spriteFallback then
            spriteFallback:getModData().RadioItemID = itemId
    ...
    if TCMusic and TCMusic.copyHiFiSlots then
        TCMusic.copyHiFiSlots(itemMd, radio:getModData(), true)
    end
    ...
    if itemDd and radioDd then
        -- Restore the persisted power state (server stamps tcmusic.turnedOn).
        radioDd:setIsTurnedOn(itemMd.tcmusic and itemMd.tcmusic.turnedOn == true or false)
        if itemMd.tcmusic and itemMd.tcmusic.volume ~= nil and radioDd.setDeviceVolume then
            radioDd:setDeviceVolume(itemMd.tcmusic.volume)
        elseif itemDd.getDeviceVolume and radioDd.setDeviceVolume then
    ...
    -- Jukebox-style: if the shared now_play table says a song is live on
    -- this tile, the rebuilt device must show ON + Stop (and Stop must work).
    ...
    for name, items in pairs(itemList) do
        ...
        if item and instanceof(item, "Radio") then
            ...
            if square and square.getObjects then
                ...
                local link = itemID;
                local spriteFallback = nil;
                local wantedSprite = TCMusic and TCMusic.WorldMusicPlayer and itemType and TCMusic.WorldMusicPlayer[itemType] or nil;
                for i = 0, objs:size() - 1 do
                    local tObj = objs:get(i);
                    if instanceof(tObj, "IsoRadio") then
                        ...
                        -- MP: server-reassigned item IDs make the place-time RadioItemID
                        -- stale; adopt by sprite so the menu never disappears.
                        if (not spriteFallback) and wantedSprite and tObj.getSprite and tObj:getSprite()
                            and tObj:getSprite():getName() == wantedSprite then
                            spriteFallback = tObj;
                        end
                    end
                end
                if (not obj) and spriteFallback and link then
                    obj = spriteFallback;
                    obj:getModData().RadioItemID = link;
                end
            end
