...
local function mergeWire(playerObj, targetItem)
    ...
    for _, entry in ipairs(others) do
        ...
        space = space - take
        if newFt <= 0 then
            -- MP: a client-side Remove is invisible to the server; ask the
            -- server to remove its copy authoritatively.
            if isClient() then
                sendClientCommand(playerObj, 'tmspeaker', 'consumeWireItem', {
                    itemId = entry.item:getID(),
                })
            end
            inv:Remove(entry.item)
        else
            nameWireBundle(entry.item, newFt)
            -- MP: push the modData/name change to the server copy, otherwise
            -- the stale item fails B42 item transactions (can't equip/drop).
            if TCMusic and TCMusic.safeSyncItem then
                TCMusic.safeSyncItem(entry.item)
            elseif isClient() and entry.item.syncItemFields then
    ...
    if TCMusic and TCMusic.safeSyncItem then
        TCMusic.safeSyncItem(targetItem)
    elseif isClient() and targetItem.syncItemFields then
        targetItem:syncItemFields()
    end
