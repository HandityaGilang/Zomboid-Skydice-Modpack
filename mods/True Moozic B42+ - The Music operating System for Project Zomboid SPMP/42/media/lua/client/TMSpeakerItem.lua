...
local function isMasterDevice(obj)
    ...
    -- Converted TM jukebox (IsoThumpable with the tm_jukebox container)
    if TMJukebox and TMJukebox.isConverted and TMJukebox.isConverted(obj) then return true end
            ...
            if sq2 then
                ...
                if objects then
                    for i = 0, objects:size() - 1 do
                        ...
                        elseif TMJukebox and TMJukebox.isConverted and TMJukebox.isConverted(o) then
                            local key = sq2:getX() .. "_" .. sq2:getY() .. "_" .. sq2:getZ() .. "_jukebox"
                            if not seen[key] then
                                seen[key] = true
                                table.insert(found, {
                                    obj  = o,
                                    name = getText("IGUI_TMSpeaker_DeviceJukebox") or "Jukebox",
                                    sq   = sq2,
                                    type = "jukebox",
                                })
                            end
...
-- Currently-playing jukebox track at position (same shape as getHifiSongData)
local function getJukeboxSongData(vx, vy, vz)
    if TMJukebox and TMJukebox.getPlayingSong then
        return TMJukebox.getPlayingSong(vx, vy, vz)
    end
    return nil
end
            ...
            if not tms or not tms.connected then
                ...
                if vx and vy and vz then
                    if not masterExistsAt(vx, vy, vz) then
                        ...
                        -- Source 1: HiFi modData / Jukebox state (check FIRST — these use
                        -- direct modData, not world emitters, so the vinylEmitterReady
                        -- gate would block them)
                        local hifiSong = getHifiSongData(vx, vy, vz) or getJukeboxSongData(vx, vy, vz)
                        ...
                        -- Mood: this speaker is actively playing something.
                        if TMMood and data.emitter and data.localmusicid and (hifiSong or mediaName) then
                            local moodSq = speakerObj:getSquare()
                            if moodSq then TMMood.reportAt(moodSq:getX(), moodSq:getY(), 1) end
                        end
                    end
                end
            end
        ...
        if isSpeakerWorldItem(o) then
            ...
            if md and md.tmspeaker and md.tmspeaker.connected then
                ...
                if sdata then
                    ...
                    if vx and vy and vz then
                        local midSong = false
                        if md.tmspeaker.masterType == "hifi" or md.tmspeaker.masterType == "jukebox" then
                            -- HiFi/Jukebox: check device state directly (skip trueMusicData)
                            local hifiSong = getHifiSongData(vx, vy, vz) or getJukeboxSongData(vx, vy, vz)
                        ...
                        if midSong then
                            ...
                            if midSong == true then
                                local hs = getHifiSongData(vx, vy, vz) or getJukeboxSongData(vx, vy, vz)
