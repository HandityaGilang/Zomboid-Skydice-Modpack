...
local rgbBad = PSR.UI.rgbBad

local WindowDebug = ISPanelJoypad:derive("PSRWindowDebug")
...
function WindowDebug:createChildren()
    ...
    local width = math.max(self.width, getTextManager():MeasureStringX(UIFont.Small, getText("IGUI_PSRWindow_Debug_ShowBackup")) + 10, getDebug() and getTextManager():MeasureStringX(UIFont.Small, "Connect Backup Generator") or 0)

    self.showBackupDetailsButton = ISButton:new(0, 0, width, buttonHeight, getText("IGUI_PSRWindow_Debug_ShowBackup"), self, self.showBackupDetails)
...
function WindowDebug:prerender()
    ...
    if getDebug() then
        ...
        if self.plugBackupButton:isVisible() then
            ...
            elseif PSR.WorldUtil.findTypeOnSquare(square,"PowerBank") then self.plugBackupButton.tooltip = rgbBad.rich .. "This is a Powerbank"
            else
...
function WindowDebug:showBackupDetails()
    ...
    -- Fix (signal joueur Damien_XIII 2026-07-25) : ce bouton ne pilotait QUE showBackupDetails
    -- (le bloc diagnostic backup en bas de l'onglet Details), mais PAS showDetails — le flag qui
    -- lève le mur "Can't See" (hors ligne de vue / hors zone valide) tout en haut de la vue
    -- (PSRStatusWindowDetailsView.lua:58 `if canSee and validArea or self.showDetails`).
    -- showDetails n'était JAMAIS assigné nulle part dans le mod -> impossible de voir Area
    -- Coverage/Details à distance ou sans ligne de vue, quoi qu'on clique. On pilote les deux
    -- flags ensemble : le bouton "Show/Hide Backup Area Details" doit bien lever tout le voile.
    self.parent.parent.detailsView.showDetails = show
    self.showBackupDetailsButton.title = show and getText("IGUI_PSRWindow_Debug_HideBackup") or getText("IGUI_PSRWindow_Debug_ShowBackup")
end

function WindowDebug:checkContainer()
    local luapb = self.parent.parent.luaPB
    PSR.PBSystem_Client:sendCommand(self.parent.parent.playerObj,"countBatteries", { x = luapb.x, y = luapb.y, z = luapb.z })
...
function WindowDebug:plugBackup()
    ...
    if generator then
        local pb = self.parent.parent.luaPB
        PSR.PBSystem_Client:sendCommand(self.parent.parent.playerObj,"plugGenerator", { pbList = { { x = pb.x, y = pb.y, z = pb.z } }, gen = { x = generator:getX(), y = generator:getY(), z = generator:getZ() }, plug = true })
    end
end

function WindowDebug:troubleshoot()
    local luapb = self.parent.parent.luaPB
    PSR.PBSystem_Client:sendCommand(self.parent.parent.playerObj, "troubleshoot", { x = luapb.x, y = luapb.y, z = luapb.z })
    self.troubleshootButton.enable = false
    self.troubleshootButton.disableFrame = 5 * getPerformance():getUIRenderFPS()
end

PSR.StatusWindowDebugView = WindowDebug
