...
function AZRecipe.OnCreate.Tie3Balloons(craftRecipeData, character)
    ...
    -- Loop through all inventory items and count balloons by color
    local items = inventory:getItems()
    for i = 0, items:size() - 1 do
        local item = items:get(i)
        if item then
            local fullType = item:getFullType()
            local color = balloonColorFromSingle[fullType]
            if color then
                balloonCounts[color] = (balloonCounts[color] or 0) + 1
            end
        end
    end

    -- Debug: Print the counts for each color
    for color, count in pairs(balloonCounts) do
        print("Tie3Balloons: Found " .. count .. " balloons of color " .. color)
    end

    -- Check if we have at least 3 of any one color
    ...
    if not targetColor then
        print("Tie3Balloons: Not enough balloons of any color (need 3)")
        return
    end

    -- Remove 3 balloons of the target color
    ...
    print("Tie3Balloons: Removed 3 balloons of color " .. targetColor)

    -- Add the balloon group
    local groupFullType = balloonGroupForColor[targetColor]
    if groupFullType then
        print("Tie3Balloons: Adding group item " .. groupFullType)
        character:getInventory():AddItem(groupFullType)
    else
        print("Tie3Balloons: No groupFullType defined for color " .. targetColor)
    end
end
