...
PSC_Distributions = PSC_Distributions or {}

local ITEM = "Base.SurvivalColorDeck"
...
function PSC_Distributions.apply()
    if PSC_Distributions.applied then
        return
    end
    PSC_Distributions.applied = true
    for listName, chance in pairs(PROCEDURAL_LOOT) do
        addProceduralItem(listName, chance)
    end
end

PSC_Distributions.apply()
