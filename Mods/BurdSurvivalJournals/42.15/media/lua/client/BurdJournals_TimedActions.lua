...
function BurdJournals.isRecordAllAuthorityStateConverged(player, journal, args)
    ...
    if type(writingToolState) == "table" and writingToolState.itemId ~= nil then
        local localTool = BurdJournals.findItemByIdInPlayerInventory
            and BurdJournals.findItemByIdInPlayerInventory(player, writingToolState.itemId) or nil
        if writingToolState.removed == true then
            if localTool then return false end
        elseif not localTool then
            return false
        elseif writingToolState.usedDelta ~= nil and localTool.getUsedDelta
            and (tonumber(localTool:getUsedDelta()) or 0) > (tonumber(writingToolState.usedDelta) or 0) + 0.0001
        then
            return false
        elseif writingToolState.drainableUses ~= nil and localTool.getDrainableUsesFloat
            and (tonumber(localTool:getDrainableUsesFloat()) or 0) > (tonumber(writingToolState.drainableUses) or 0) + 0.0001
        then
            return false
        end
    end
    ...
    if expectedRevision then
        ...
        if type(journalData) ~= "table" or journalData.entryStoreEnabled ~= true
            or (tonumber(journalData.entryStoreUpdatedAt) or 0) < expectedRevision
        then
            return false
        end
    end
...
function BurdJournals.queueRecordAllContinuationAfterAuthority(panel, player, action, args)
    if BurdJournals.isRecordAllAuthorityStateConverged(player, action and action.journal, args) then
        ISTimedActionQueue.add(action)
        return true
    end
    ...
    tick = function()
        ...
        if BurdJournals.isRecordAllAuthorityStateConverged(player, journal, args) then
            ...
            ISTimedActionQueue.add(action)
            BurdJournals.debugPrint("[BurdJournals] Record All authority state converged; started next batch")
            return
        end
        if nowMs - startedAt >= 5000 then
            ...
            panel.processingRecordQueue = false
            if panel.recordingState then panel.recordingState.active = false; panel.recordingState.interrupted = true end
            if BurdJournals.Client and BurdJournals.Client.requestJournalSync and journal then
                BurdJournals.Client.requestJournalSync(journal, "recordAllInventorySettleTimeout", nil, player)
            end
    ...
    panel.recordAllAuthoritySettleTick = tick
    Events.OnTick.Add(tick)
...
function BurdJournals.continueRecordAllAfterServerAck(panel, player, args)
    ...
    if not BurdJournals.queueRecordAllContinuationAfterAuthority(panel, player, action, args) then
        return false
    end
