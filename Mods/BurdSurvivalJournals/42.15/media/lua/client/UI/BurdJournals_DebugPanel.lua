...
function BurdJournals.UI.DebugPanel:createDebugTab(tabId)
    if self.tabPanels and self.tabPanels[tabId] then return self.tabPanels[tabId] end
    local y = self._debugContentY
    local height = self._debugContentHeight
    if not y or not height then return nil end
    if tabId == "spawn" then self:createSpawnPanel(y, height)
    elseif tabId == "character" then self:createCharacterPanel(y, height)
    elseif tabId == "baseline" then self:createBaselinePanel(y, height)
    elseif tabId == "snapshots" then self:createSnapshotsPanel(y, height)
    elseif tabId == "journal" then self:createJournalPanel(y, height)
    elseif tabId == "whitelist" then self:createWhitelistPanel(y, height)
    elseif tabId == "diagnostics" then self:createDiagnosticsPanel(y, height) end
    ...
    if panel and sharedTarget and sharedTarget ~= self.player then
        ...
        if tabId == "character" and self.refreshCharacterData then self:refreshCharacterData() end
        if tabId == "baseline" and self.refreshBaselineData then self:refreshBaselineData() end
    end
    return panel
end
...
local function getDebugTraitTextureIndex()
    if debugTraitTextureIndex ~= nil then return debugTraitTextureIndex end
    local index = {}
...
local function getDebugTraitTexture(traitId)
    ...
    local textureIndex = getDebugTraitTextureIndex()
    local indexedTexture = textureIndex[cacheKey] or textureIndex[string.gsub(cacheKey, "%s+", "")]
