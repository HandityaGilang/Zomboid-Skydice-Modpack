...
function BurdJournals.Client.sendSanitizeJournalRequest(journal, requestPlayer)
    ...
    local lookupArgs = BurdJournals.buildJournalCommandPayload
        ...
        or (BurdJournals.buildJournalCommandLookupArgs
    ...
    local payload = {
        ...
        -- B41 owns carried InventoryItem ModData on the client.  Send its
        -- snapshot only on this recovery handshake; ordinary record/claim
        -- commands must stay identity-only to avoid redundant large payloads.
        journalData = lookupArgs.b41ClientInventoryAuthority == true and journalData or lookupArgs.journalData,
        createIfMissing = lookupArgs.createIfMissing,
        b41ClientInventoryAuthority = lookupArgs.b41ClientInventoryAuthority,
    }
...
function BurdJournals.Client.requestCursedInsightPreview(player, item, journalData)
    ...
    journalData = type(journalData) == "table" and journalData or (BurdJournals.getJournalData and BurdJournals.getJournalData(item) or nil)
    local lookupArgs = BurdJournals.buildJournalCommandPayload
        and BurdJournals.buildJournalCommandPayload(item, journalData, true)
        or {
            journalId = tonumber(item:getID()),
            journalUUID = BurdJournals.getJournalIdentityUUID
                and BurdJournals.getJournalIdentityUUID(journalData) or nil,
        }
    local journalId = tonumber(lookupArgs.journalId)
    local journalUUID = lookupArgs.journalUUID
...
function BurdJournals.Client.requestJournalSync(journalOrId, reasonTag, journalData, requestPlayer)
    ...
    then
        ...
        local cachedUUID = BurdJournals.getJournalIdentityUUID
            and BurdJournals.getJournalIdentityUUID(cachedSnapshot)
            or nil
        if cachedUUID then
    ...
    if type(lookupArgs) == "table"
        and lookupArgs.b41ClientInventoryAuthority == true
        and type(syncJournalData) ~= "table"
    then
        syncJournalData = type(journalData) == "table" and journalData
            or (journal and BurdJournals.getJournalData and BurdJournals.getJournalData(journal))
            or nil
    end
    ...
    sendClientCommand(player, "BurdJournals", "syncJournalData", {
        ...
        createIfMissing = lookupArgs and lookupArgs.createIfMissing or true,
        b41ClientInventoryAuthority = lookupArgs and lookupArgs.b41ClientInventoryAuthority or nil,
        reason = reasonTag
    })
    return true
end
...
-- Each snapshot is indexed by both UUID and item ID so an MP item replacement
-- cannot strand authoritative entries behind the identity the old shell used.
...
local function pruneClientEntryChunkCaches(now)
    ...
    for aliasKey, snapshotKey in pairs(BurdJournals.Client._hydratedJournalSnapshotAliases) do
        if snapshots[snapshotKey] == nil then BurdJournals.Client._hydratedJournalSnapshotAliases[aliasKey] = nil end
    end
    ...
    for key, state in pairs(states) do
        ...
        if type(state) ~= "table"
            ...
            or stateAt <= 0
            or (now - stateAt) > ENTRY_CHUNK_SYNC_STATE_TTL_MS
        then
            states[key] = nil
        else
...
function BurdJournals.Client.getHydratedJournalSnapshot(journalOrId, journalUUID)
    ...
    -- An explicit/server UUID is authoritative. Never fall back from a known
    -- UUID to an item-ID alias, because IDs can be replaced during
    -- materialization and an old alias may belong to another journal.
    ...
    if itemUUID and (not journalUUID or tostring(itemUUID) ~= tostring(journalUUID)) then
        lookupKeys[#lookupKeys + 1] = "uuid:" .. tostring(itemUUID)
    end
...
function BurdJournals.Client.isJournalAuthorityRequestPending(journalOrId, journalData)
    ...
    local function matches(candidateId, candidateUUID)
        if journalUUID and candidateUUID then return tostring(journalUUID) == tostring(candidateUUID) end
    ...
    for _, pending in pairs(BurdJournals.Client._pendingSanitizeRequests or {}) do
        ...
        if type(payload) == "table" and matches(payload.journalId, payload.journalUUID) then return true end
    end
    for _, state in pairs(BurdJournals.Client._entryChunkSyncStates or {}) do
        if type(state) == "table" and state.active == true and matches(state.journalId, state.journalUUID) then return true end
    end
    for _, pending in pairs(BurdJournals.Client._pendingJournalMaterializations or {}) do
        if type(pending) == "table" and (matches(pending.newJournalId, pending.journalUUID) or matches(pending.oldJournalId, pending.oldJournalUUID)) then return true end
    end
    return false
end
...
function BurdJournals.Client.invalidateHydratedJournalSnapshot(journalId, journalUUID)
    ...
    for aliasKey, aliasTarget in pairs(BurdJournals.Client._hydratedJournalSnapshotAliases) do
        if aliasTarget == snapshotKey then
...
function BurdJournals.Client.rebindHydratedJournalSnapshot(oldJournalId, newJournalId, journalUUID)
    ...
    local rebound = type(snapshot) == "table"
        and BurdJournals.Client.cacheHydratedJournalSnapshot(
            newJournalId, journalUUID, snapshot, "journalMaterializedRebind") == true
        or false
...
function BurdJournals.Client.entryChunkPanelMatches(panel, player, journalId, journalUUID)
    ...
    if journalUUID then
        ...
        local panelUUID = BurdJournals.getJournalIdentityUUID
            and BurdJournals.getJournalIdentityUUID(panelData)
            or nil
...
journalUUIDsMatch = function(journal, journalUUID)
    ...
    local currentUUID = BurdJournals.getJournalIdentityUUID
...
local function journalMatchesAuthoritativeIdentity(journal, journalId, journalUUID)
    ...
    if type(journalUUID) == "string" and journalUUID ~= "" then
        if journalUUIDsMatch(journal, journalUUID) then
            return true
        end
        local journalData = BurdJournals.getJournalData and BurdJournals.getJournalData(journal) or nil
        local localUUID = BurdJournals.getJournalIdentityUUID
            and BurdJournals.getJournalIdentityUUID(journalData)
            or nil
        -- A UUID-less legacy shell must be allowed to accept the first
        -- authoritative UUID for the exact held item. Never let an item ID
        -- override a real conflicting local UUID.
        return localUUID == nil
            and journalId ~= nil
            and journal.getID
            ...
            if panelApplied then
                ...
                if sourceTag == "syncSuccess" or sourceTag == "notesSaved" then
                    ...
                    local hydratedForPanel = BurdJournals.Client.getHydratedJournalSnapshot
                        and BurdJournals.Client.getHydratedJournalSnapshot(panel.journal, journalUUID) or nil
                    if journalDataHasClientEntries(panelData) then
                        panel.pendingRecordJournalData = BurdJournals.normalizeJournalData and BurdJournals.normalizeJournalData(panelData) or panelData
    ...
    for key, entry in pairs(pending) do
        if type(entry) ~= "table" or entry.newJournalId == nil then
            ...
            if newJournal then
                ...
                if entry.journalUUID and newUUID and tostring(entry.journalUUID) ~= tostring(newUUID) then
                    newJournal = nil
                end
            end
            if newJournal then
...
function BurdJournals.Client.handleJournalMaterialized(player, args)
    ...
    local b41ClientInventoryMirror = args.b41ClientInventoryMirror == true
        or (BurdJournals.clientShouldUseServerAuthority
            and BurdJournals.clientShouldUseServerAuthority() == true)
    ...
    elseif not b41ClientInventoryMirror then
    ...
    local resolveExactOldJournalItem = exactOldJournalItem and not b41ClientInventoryMirror
    local pendingKey, pendingEntry = findPendingJournalMaterialization(
        oldJournalId,
        oldJournalUUID,
        resolveExactOldJournalItem
    )
    ...
    if type(args.journalData) == "table" then
        ...
        elseif oldJournal and (not exactOldJournalItem or b41ClientInventoryMirror) then
    ...
    -- A B41 dedicated server's reconstructed inventory item is an authority
    -- mirror, not an object that will replicate back into the client-owned
    -- character inventory.  Keep and hydrate the existing client item by UUID;
    -- polling for the server mirror's item id can only end in a false sync error.
    if b41ClientInventoryMirror and oldJournal then
        ...
        local normalizedJournalData = type(args.journalData) == "table"
            and (BurdJournals.normalizeJournalData
                and BurdJournals.normalizeJournalData(args.journalData) or args.journalData)
            or nil
        if normalizedJournalData and BurdJournals.Client.cacheHydratedJournalSnapshot then
            BurdJournals.Client.cacheHydratedJournalSnapshot(
                oldJournalId, materializedUUID, normalizedJournalData, "b41ClientInventoryMirror")
        end
        if panel and (panel.journal == oldJournal
            or journalMatchesAuthoritativeIdentity(panel.journal, oldJournalId, oldJournalUUID or materializedUUID))
        then
            local preservedRecordAllContinuation = panel.pendingRecordAllContinuation
                or BurdJournals.pendingRecordAllContinuation
            panel.journal = oldJournal
            panel.pendingNewJournalId = nil
            panel.pendingRecordJournalData = normalizedJournalData
            local refreshedFromPayload = refreshRecordPanelFromAuthoritativeData(
                panel, args.journalData, "b41ClientInventoryMirror")
            if not refreshedFromPayload then
                refreshPanelAfterJournalIdentityChange(panel, "b41ClientInventoryMirror")
            end
        ...
        then
            BurdJournals.Client.startJournalEntryChunkSync(player, {
                ...
                journalUUID = materializedUUID,
                fullJournalDataOmitted = true,
                fullJournalDataOmittedReason = args.fullJournalDataOmittedReason,
            }, "b41ClientInventoryMirror")
        end
        BurdJournals.debugPrint("[BurdJournals] Client: retained B41 client-owned journal id="
            .. tostring(oldJournalId) .. " for server mirror id=" .. tostring(args.newJournalId))
        return
    end
    ...
    if type(journalRequest) == "table" then
        local request = {
            ...
            journalFingerprint = type(journalRequest.journalFingerprint) == "string" and journalRequest.journalFingerprint ~= ""
                ...
                or nil,
            journalData = type(journalRequest.journalData) == "table" and journalRequest.journalData or nil,
            itemFullType = type(journalRequest.itemFullType) == "string" and journalRequest.itemFullType ~= ""
                and journalRequest.itemFullType
                or nil,
            b41ClientInventoryAuthority = journalRequest.b41ClientInventoryAuthority == true,
...
function BurdJournals.Client.handleYuletideOpenPrompt(player, args)
    ...
    if not ISModalDialog then
        ...
        queueLootJournalOpenTimedAction(player, {
            ...
            b41ClientInventoryAuthority = args.b41ClientInventoryAuthority == true,
...
function BurdJournals.Client.onServerCommand(module, command, args)
    ...
    elseif command == "convertSuccess" then
        if args and args.rebindCommitState
            and not BurdJournals.Client.applyB41RebindCommit(player, args.rebindCommitState)
        then
            BurdJournals.Client.showHaloMessage(
                player,
                BurdJournals.safeGetText(
                    "UI_BurdJournals_ServerRequestRejected",
                    "The journal rebind could not be applied locally."),
                BurdJournals.Client.HaloColors.ERROR)
            return
        end
...
function BurdJournals.Client.applyB41WritingToolCommit(player, writingToolState)
    if type(writingToolState) ~= "table"
        or writingToolState.b41ClientInventoryAuthority ~= true
    then
        return false
    end

    local itemId = tonumber(writingToolState.itemId)
    local uses = math.floor((tonumber(writingToolState.clientConsumeUses) or 0) + 0.5)
    local commitToken = type(writingToolState.commitToken) == "string"
        and writingToolState.commitToken or nil
    if not itemId or itemId < 0 or uses <= 0 or uses > 10000
        or not commitToken or commitToken == "" or #commitToken > 160
    then
        return false
    end

    BurdJournals.Client._b41WritingToolCommitTokens =
        BurdJournals.Client._b41WritingToolCommitTokens or {}
    BurdJournals.Client._b41WritingToolCommitOrder =
        BurdJournals.Client._b41WritingToolCommitOrder or {}
    if BurdJournals.Client._b41WritingToolCommitTokens[commitToken] == true then
        return true
    end

    local writingTool = BurdJournals.findItemByIdInPlayerInventory
        and BurdJournals.findItemByIdInPlayerInventory(player, itemId) or nil
    local itemType = writingTool
        and (writingTool.getFullType and writingTool:getFullType()
            or (writingTool.getType and writingTool:getType()))
        or nil
    if not writingTool
        or not BurdJournals.isWritingToolType
        or not BurdJournals.isWritingToolType(itemType)
    then
        return false
    end

    BurdJournals.consumeItemUses(writingTool, uses, player)
    BurdJournals.Client._b41WritingToolCommitTokens[commitToken] = true
    local order = BurdJournals.Client._b41WritingToolCommitOrder
    order[#order + 1] = commitToken
    while #order > 128 do
        local expiredToken = table.remove(order, 1)
        BurdJournals.Client._b41WritingToolCommitTokens[expiredToken] = nil
    end
    return true
end

function BurdJournals.Client.applyB41RebindCommit(player, commitState)
    if type(commitState) ~= "table"
        or commitState.b41ClientInventoryAuthority ~= true
        or not player
    then
        return false
    end

    local token = type(commitState.commitToken) == "string" and commitState.commitToken or nil
    if not token or token == "" or #token > 180 then return false end
    BurdJournals.Client._b41RebindCommitTokens = BurdJournals.Client._b41RebindCommitTokens or {}
    BurdJournals.Client._b41RebindCommitOrder = BurdJournals.Client._b41RebindCommitOrder or {}
    if BurdJournals.Client._b41RebindCommitTokens[token] == true then return true end

    local oldJournalId = tonumber(commitState.oldJournalId)
    local oldJournal = oldJournalId and BurdJournals.findItemByIdInPlayerInventory
        and BurdJournals.findItemByIdInPlayerInventory(player, oldJournalId) or nil
    if not oldJournal and commitState.oldJournalUUID and BurdJournals.findJournalByUUIDInPlayerInventory then
        oldJournal = BurdJournals.findJournalByUUIDInPlayerInventory(player, commitState.oldJournalUUID)
    end
    if not oldJournal or not BurdJournals.isRebindableLootJournal(oldJournal) then return false end

    local materials = type(commitState.materials) == "table" and commitState.materials or nil
    if not materials then return false end
    local resolvedMaterials = {}
    local seenIds = {}
    for _, category in ipairs({"leather", "thread", "needle"}) do
        local hint = materials[category]
        local itemId = type(hint) == "table" and tonumber(hint.itemId) or nil
        local hintedType = type(hint) == "table" and hint.itemFullType or nil
        if not itemId or itemId < 0 or itemId ~= math.floor(itemId) or seenIds[tostring(itemId)] then
            return false
        end
        local item = BurdJournals.findItemByIdInPlayerInventory(player, itemId)
        local itemType = item and item.getFullType and item:getFullType()
            or (item and item.getType and item:getType()) or nil
        local validType = itemType == hintedType
        local allowedTypes = BurdJournals.REPAIR_ITEMS and BurdJournals.REPAIR_ITEMS[category] or nil
        if validType and type(allowedTypes) == "table" then
            validType = false
            for i = 1, #allowedTypes do
                if itemType == allowedTypes[i] then
                    validType = true
                    break
                end
            end
        end
        if not item or not validType then return false end
        seenIds[tostring(itemId)] = true
        resolvedMaterials[category] = item
    end

    local normalizedData = type(commitState.newJournalData) == "table"
        and (BurdJournals.normalizeJournalData
            and BurdJournals.normalizeJournalData(commitState.newJournalData)
            or commitState.newJournalData)
        or nil
    if type(normalizedData) ~= "table"
        or normalizedData.isPlayerCreated ~= true
        or tostring(normalizedData.uuid or "") ~= tostring(commitState.newJournalUUID or "")
    then
        return false
    end

    local inventory = player:getInventory()
    local cleanJournal = inventory and inventory:AddItem("BurdJournals.BlankSurvivalJournal") or nil
    local cleanModData = cleanJournal and cleanJournal:getModData() or nil
    if not cleanModData then
        if cleanJournal and inventory then inventory:Remove(cleanJournal) end
        return false
    end
    cleanModData.BurdJournals = normalizedData
    BurdJournals.updateJournalName(cleanJournal, true)
    BurdJournals.updateJournalIcon(cleanJournal)

    local leatherContainer = resolvedMaterials.leather.getContainer
        and resolvedMaterials.leather:getContainer() or inventory
    leatherContainer:Remove(resolvedMaterials.leather)
    BurdJournals.consumeItemUses(resolvedMaterials.thread, 1, player)
    BurdJournals.consumeItemUses(resolvedMaterials.needle, 1, player)
    local oldContainer = oldJournal.getContainer and oldJournal:getContainer() or inventory
    oldContainer:Remove(oldJournal)
    if inventory and inventory.setDrawDirty then inventory:setDrawDirty(true) end

    BurdJournals.Client._b41RebindCommitTokens[token] = true
    local order = BurdJournals.Client._b41RebindCommitOrder
    order[#order + 1] = token
    while #order > 64 do
        BurdJournals.Client._b41RebindCommitTokens[table.remove(order, 1)] = nil
    end
    return true
end
...
function BurdJournals.Client.handleRecordSuccess(player, args)
    ...
    BurdJournals.Client.applyB41WritingToolCommit(player, args.writingToolState)
    ...
    if panelHandlesRecordSuccess then
        ...
        elseif args.journalDelta then
            ...
            local rawPendingRecordData = type(panel.pendingRecordJournalData) == "table"
                and panel.pendingRecordJournalData or nil
