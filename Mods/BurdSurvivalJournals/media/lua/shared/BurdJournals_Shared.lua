...
function BurdJournals.recordVhsSkillXP(player, skillDeltas, lineGuid, category, sourceTag)
    ...
    for skillName, delta in pairs(normalizedDeltas) do
        ...
        if deltaToRecord > 0 then
            local nextValue = math.min(
                BurdJournals.VHS_SKILL_XP_MAX_TOTAL,
                (tonumber(skillMap[skillName]) or 0) + deltaToRecord
            )
...
function BurdJournals.buildJournalCommandLookupArgs(journal, journalData, createIfMissing)
    ...
    -- Build 41 keeps carried InventoryItem ModData on the client.  Unlike B42,
    -- the dedicated server cannot mint an identity on that physical item and
    -- replicate it back before the first command.  Give the client-owned B41
    -- item one stable UUID here, then let the server validate the bounded
    -- snapshot and use that UUID for its authoritative mirror.  Without this,
    -- identical cursed/Yuletide items in a stack can only be distinguished by
    -- client-local item IDs that do not exist on the dedicated server.
    local b41ClientInventoryAuthority = strictMPClient == true
    local allowLocalUUIDCreation = createIfMissing ~= false
    ...
    return {
        ...
        -- Build 41 keeps character inventory (including InventoryItem ModData)
        -- on the client in multiplayer.  The B41 server therefore cannot
        -- resolve a carried journal until it has materialized its own validated
        -- mirror.  This marker is confined to the root/B41 runtime tree.
        b41ClientInventoryAuthority = b41ClientInventoryAuthority,
    }
end

function BurdJournals.ensureB41ClientOwnedCommandJournalData(journal, journalData, createIfMissing)
    if type(journalData) == "table"
        or createIfMissing == false
        or not journal
        or not journal.getFullType
        or not journal.getModData
        or not BurdJournals.clientShouldUseServerAuthority()
    then
        return journalData
    end

    -- Natural B41 Yuletide items can arrive as a completely raw script-item
    -- shell: their type is correct, but BurdJournals ModData has never existed
    -- on either side.  At the first explicit command boundary, create only the
    -- identity/type envelope needed to materialize the dedicated-server
    -- mirror.  Gift rolls, reward profiles, and unwrap mutations remain wholly
    ...
    local modData = journal:getModData()
    if not modData then return journalData end
    if type(modData.BurdJournals) ~= "table" then
        modData.BurdJournals = {}
    end
    local data = modData.BurdJournals
    data.isYuletideJournal = true
    data.yuletideState = BurdJournals.YULETIDE_STATE_WRAPPED or "wrapped"
    data.isPlayerCreated = false
    data.sourceType = "loot"
    data.originMode = "loot"
    return data
end

function BurdJournals.buildJournalCommandPayload(journal, journalData, createIfMissing)
    journalData = BurdJournals.ensureB41ClientOwnedCommandJournalData(journal, journalData, createIfMissing)
...
function BurdJournals.resolveJournalEntryStoreUUID(journalData, repairIdentity)
    ...
    -- Conflicting fields are a legacy/desync recovery boundary. Inspect both
    -- authoritative stores before converging so a populated store is never
    -- orphaned merely because the other field happened to win ordering.
    ...
    then
        ...
        if type(preferredEntry) == "table" and type(alternateEntry) == "table" then
            local merged = false
            for _, bucketName in ipairs(BurdJournals.ENTRY_STORE_BUCKETS or {}) do
                if type(preferredEntry[bucketName]) ~= "table" then
                    preferredEntry[bucketName] = {}
                end
