...
function FaceManager_Server.SyncBlood(player, bodyVisualSnapshot)
    if not player then return end

    local itemsWithBlood = FaceManager_Shared.GetWornBloodSyncItems(player)
    if not itemsWithBlood or #itemsWithBlood == 0 then return end

    local sourceVisual = FaceManager_Shared.GetBloodAndDirtVisual(player, bodyVisualSnapshot)
    if not sourceVisual then return end
    ...
    for _, item in ipairs(itemsWithBlood) do
        ...
        if itemVisual and FaceManager_Shared.AddBloodAndDirtToItem(itemVisual, sourceVisual) then
            changed = true
    ...
    if isServer and isServer() then
        sendServerCommand(player, "SPNCC", "ResetModelOnly", {})
    end
end
