...
local function GrowPlayerBodyHair(player, manager)
	...
	if not data then
		...
		if not missingDataWarning[username] then
			missingDataWarning[username] = true
			print("[SPNCC] Body hair growth skipped: missing SPNCharCustom data for " .. tostring(username))
		end
