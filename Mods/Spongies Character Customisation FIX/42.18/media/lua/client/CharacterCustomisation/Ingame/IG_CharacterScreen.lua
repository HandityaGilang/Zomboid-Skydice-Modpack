...
local function _normalizeModId(id)
	return id and tostring(id):lower():gsub("[\\/]", ""):gsub("%s+", "") or ""
end

local function isModLoaded(...)
	local mods = getActivatedMods and getActivatedMods() or nil
	if not mods then return false end

	for n = 1, select("#", ...) do
		local wanted = _normalizeModId(select(n, ...))
		for i = 0, mods:size() - 1 do
			if _normalizeModId(mods:get(i)) == wanted then
				return true
			end
		end
	end
...
local function isBetterCharacterInfoActive()
	return isModLoaded("twistbettercharacterinfo", "bettercharacterinfo")
end
...
local function SPNCC_RefreshCharacterScreenState(self)
	...
	local modData = self.char and self.char:getModData() or nil
	local data = modData and modData.SPNCharCustom or nil

	self.hasBodyHair = data and data.bodyHair or false
	self.hasStubbleHead = data and data.stubbleHead or false
	self.hasStubbleBeard = data and data.stubbleBeard or false
end

SPNCC = SPNCC or {}
SPNCC.CharacterScreen = SPNCC.CharacterScreen or {}
SPNCC.CharacterScreen.RefreshIngameState = SPNCC_RefreshCharacterScreenState
...
function ISCharacterScreen:DebugAddBeardStubble()
	...
	if _useLocalSingleplayerPath() then
		FaceManager_Local.AddPlayerStubble(getPlayer(), true, true)
		FaceManager_Local.SyncRemoveCustomisation(self.char)
		FaceManager_Local.OnClothingUpdated(self.char)
	else
...
function ISCharacterScreen:DebugAddHeadStubble()
	...
	if _useLocalSingleplayerPath() then
		FaceManager_Local.AddPlayerStubble(getPlayer(), false, true)
		FaceManager_Local.SyncRemoveCustomisation(self.char)
		FaceManager_Local.OnClothingUpdated(self.char)
	else
...
function ISCharacterScreen:DebugAddBodyHair()
	...
	if _useLocalSingleplayerPath() then
		FaceManager_Local.AddPlayerBodyHair(getPlayer(), true)
		FaceManager_Local.SyncRemoveCustomisation(self.char)
		FaceManager_Local.OnClothingUpdated(self.char)
	else
...
function ISCharacterScreen:customisationMenu(button)
	...
	-- -- CREATE BUTTONS
	-- ----------------------
if not isBetterCharacterInfoActive() then
	...
	function ISCharacterScreen.render(self)
		...
		self.shaveButton:setY(finalY - ((BUTTON_HGT - FONT_HGT_SMALL) / 2))
		...
		if self.CharCustomWindow then
			self.customisationButton.enable = false
		end

		self.customisationButton:setX(self.hairButton:getX())
		self.customisationButton:setY(finalY - ((BUTTON_HGT - FONT_HGT_SMALL) / 2))
