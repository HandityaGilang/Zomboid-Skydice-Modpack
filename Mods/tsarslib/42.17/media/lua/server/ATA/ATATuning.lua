...
function ATATuning.ModelByItemName(vehicle, part, item)
    ...
    --print('ATATuning.ModelByItemName '..vehicleName..' '..partName..' '..tab2str(part:getModData()))
...
function ATATuning.Create.Chance0(vehicle, part)
    --print('ATATuning.Create.Chance0 '..part:getId())
...
function ATATuning.Create.RoofTent(vehicle, part)
    --print("ATATuning.Create.RoofTent "..part:getId())
    ATATuning.Create.Chance0(vehicle, part)
...
function ATATuning.Init.RoofTent(vehicle, part)
    --print("ATATuning.Init.RoofTent "..part:getId())
    if part:getInventoryItem() then
        --print("ATATuning.Init.RoofTent: VISIBLE")
        part:setModelVisible("Default", true)
...
function ATATuning.InstallComplete.RoofTent(vehicle, part)
    --print("ATATuning.InstallComplete.RoofTent "..part:getId())
...
function ATATuning.synchronizeWheelPartFromPart(vehicle, wPart, part)
    local change = false
    if wPart and wPart:getInventoryItem() then
        if wPart:getCondition() ~= part:getCondition() then
            change = true
            wPart:setCondition(part:getCondition())
        end
        if wPart:getContainerContentAmount() ~= part:getContainerContentAmount() then
            change = true
            wPart:setContainerContentAmount(part:getContainerContentAmount())
            local wheelIndex = wPart:getWheelIndex()
            local cca = wPart:getContainerContentAmount()
            local cc = wPart:getContainerCapacity()
            if cc > 0 then
                local inflation = cca / cc
                --print('ATATuning.Update.ATAMotoTire Wheel '..tostring(wheelIndex or 'nil')..' '..tostring(cca or 'nil')..' '..tostring(cc or 'nil')..' '..tostring(inflation or 'nil')..' '..tostring(vehicle or 'nil'))
                if isServer() then
                    if not ATATuning.loggedMissingInflationUpdateOnServer then
                        ATATuning.loggedMissingInflationUpdateOnServer = true
                        print('ATATuning.Update.ATAMotoTire Wheel inflation not set to'..tostring(inflation or 'nil')..' because this is broken server side. TODO analyse ATATuning.lua around line883')
                    end
                else
                    vehicle:setTireInflation(wheelIndex, inflation)--broken server side afaik
                end
            end
        end
    end
...
function ATATuning.Update.ATAMotoTireFrontWheel(vehicle, part, elapsedMinutes)
    ...
    if part:getInventoryItem() then
        local changeW1 = ATATuning.synchronizeWheelPartFromPart(vehicle, wPart1, part)
        local changeW2 = ATATuning.synchronizeWheelPartFromPart(vehicle, wPart2, part)
        if changeW1 then vehicle:transmitPartCondition(wPart1) end
        if changeW2 then vehicle:transmitPartCondition(wPart2) end
        if changeW1 or changeW2 then vehicle:transmitPartCondition(part) end
    else
