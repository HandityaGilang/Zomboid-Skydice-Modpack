	...
	for i=1,vehicle:getPartCount() do
		...
		if part:isContainer() and part:getContainerContentType() == "Gasoline Storage" then
			...
			if part:getContainerContentAmount() > 0 then
				if ISVehiclePartMenu.getGasCanNotFull(playerObj, typeToItem) then
					if slice then
                        ...
                        ISVehiclePartMenu.doSiphonFuelMenu(playerObj, part, context)--B42
					end
				else 
					if slice then
					else
			...
			if fuel_truck_source and fuel_truck_source:getContainerContentAmount() > 0 and part:getContainerContentAmount() < part:getContainerCapacity() then
				--if square and part:getContainerContentAmount() < part:getContainerCapacity() then
					if slice then
						slice:addSlice(getText("ContextMenu_Fill_Gasoline_Storage_Tank_From_Fuel_Truck"), getTexture("media/ui/vehicles/vehicle_refuel_from_pump.png"), ISVehiclePartMenu.onPumpGasolineFromTruck, playerObj, part, fuel_truck_source)
					else
		...
		if not vehicle:isEngineStarted() and part:isContainer() and part:getContainerContentType() == "Gasoline" then
			if fuel_truck_source and fuel_truck_source:getContainerContentAmount() > 0 and part:getContainerContentAmount() < part:getContainerCapacity() then
				--if square and part:getContainerContentAmount() < part:getContainerCapacity() then
					if slice then
						slice:addSlice(getText("ContextMenu_Remove_Gasoline_From_Fuel_Truck"), getTexture("media/ui/vehicles/vehicle_refuel_from_pump.png"), ISVehiclePartMenu.onPumpGasolineFromTruck, playerObj, part, fuel_truck_source)
					else
...
---Tch Note: we may prefer parsing closeby (5 dist) squares than the whole cell vehicle Set
	local radius = 5
