...
ATA_ISVehicleMenu.old_ISVehicleMenu_onSwitchSeat = ISVehicleMenu.onSwitchSeat
...
function ISVehicleMenu.showRadialMenuOutside(playerObj)
    ...
    if vehicle then
        ...
        if not part then part = vehicle:getPartById("ATA2VehicleWrecker") end
...
function ATA_ISVehicleMenu.getVehicleAtRearOfTrailer(trailer)
    ...
    for i=0, 8, 0.5 do    
        ...
        if vehicle then
            local trailerScriptName = trailer:getScript():getName()
            local tlcConf = vehicle:getPartById("TCLConfig")
            if tlcConf then--Version AMC for vehicles that tells what trailer can haul them
                local tlcTable = tlcConf:getTable("TCLConfig")
                if tlcTable and tlcTable.wreckerName == trailerScriptName then
                    return vehicle
                end
            end
            
            local wreckerTable = ATA2WreckerTable[trailerScriptName]
            if trailerScriptName and wreckerTable[vehicle:getScript():getName()] then--generic compatibility
                return vehicle
            end
        end
    end
...
-----------------------------
-- Trailer switch seat B42 --
-----------------------------

--avoid doorPart deref when nil
function ISVehicleMenu.onSwitchSeat(playerObj, seatTo)
    ...
    if doorPart == nil then
        ...
        ATA_ISVehicleMenu.old_ISVehicleMenu_onSwitchSeat(playerObj, seatTo)
    end
end