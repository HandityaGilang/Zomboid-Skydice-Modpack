...
local tagTmpArray = ArrayList.new()--tmp memo for tag java interface
local function getTag(tagName)
    return tagName and ItemTag.get(ResourceLocation.of(tagName))
end
local function itemHasTag(item,tagName)
    if not item then return end
    local tag = getTag(tagName)
    return tag and item:hasTag(tag)
end
...
-- For non-evolved recipes
 ...
 -- {fullType = 4,}
function ISVehicleTuning2:getAvailableItemsType(RecipeItem)--Type or Tag
    local result = {};
    local recipeListBox = self:getRecipeListBox()
    ...
    if RecipeItem.use then
        for _, itemInList in pairs(RecipeItem.use) do
            ...
            local itemTag = itemInList.itemTag
            local tag = getTag(itemTag)
            for _, itemCont in pairs(self.containerListLua) do
                if itemInList.isDrainable then -- если предмет расходуемый, считает количество доступного использования
                    local array = tag and itemCont:getAllTag(tag) or itemCont:getAllType(fullType)
                    if tag and array:size() == 0 then array = itemCont:getAllType(fullType) end--patch for vanilla BlowTorch not having the BlowTorch tag.
                    local count = 0
                    ...
                    if tag then
                        result[itemTag] = (result[itemTag] or 0) + count
                    else
                    ...
                    local arraySize = tag and itemCont:getAllTag(tag):size()
                    if not arraySize or arraySize == 0 then arraySize = itemCont:getAllType(fullType):size() end
                    if tag then
                        result[itemTag] = (result[itemTag] or 0) + arraySize
                    else
...
function ISVehicleTuning2:HasAllRequiredItems(RecipeItem)
    ...
    if RecipeItem.use then
        for _, item in pairs(RecipeItem.use) do
            if item.count > (typesAvailable[item.fullType] or 0) and (not item.itemTag or item.count > (typesAvailable[item.itemTag] or 0)) then
                result = false
            end
        end
    end
    if RecipeItem.tools then
        for _, item in pairs(RecipeItem.tools) do
            if 1 > (typesAvailable[item.fullType] or 0) and (not item.itemTag or 1 > (typesAvailable[item.itemTag] or 0)) then
                result = false
            end
        end
    end
...
function ISVehicleTuning2:createRecipeItemFor(infoTable, partName, partTuningInfo, modelName, actionType, specialCategory) -- actionType = install or uninstall
    ...
    local partActionType = partTuningInfo[actionType]
    if partActionType.requireModel and partActionType.requireModel ~= "" then
        RecipeItem.requireModel = partActionType.requireModel
        local requiredModel = infoTable[RecipeItem.requireModel]
        if requiredModel.name and requiredModel.name ~= "" then
            RecipeItem.requireModelName = getText(requiredModel.name)
        else
            RecipeItem.requireModelName = getText("IGUI_VehiclePart" .. partName)
        end
    end
    
    if partActionType.area and partActionType.area ~= "" then
        RecipeItem.area = partActionType.area
    end
    
    if partActionType.recipes and #partActionType.recipes > 0 then
        RecipeItem.recipes = partActionType.recipes
    ...
    RecipeItem.time = partActionType.time
    
    if partActionType.tools and type(partActionType.tools) == "table" then
        RecipeItem.tools = {};
        local filled = false
        for useType, tool in pairs(partActionType.tools) do
            local item = nil
            ...
            if item then
                ...
                toolInList.itemTag = self:GetTagName(tool)
    ...
    if partActionType.use and type(partActionType.use) == "table" then
        RecipeItem.use = {};
        local filled = false
        for itemName, count in pairs(partActionType.use) do
            local item = nil
            ...
            if item then
                ...
                itemInList.itemTag = self:GetTagName(itemName)
    ...
    if partActionType.result and type(partActionType.result) == "table" then
        RecipeItem.resultTable = {};
        RecipeItem.result = {};
        local filled = false
        for itemName, count in pairs(partActionType.result) do
            local item = nil
            itemName = itemName:gsub("__", ".")
            item = self:GetItemInstance(itemName)
            if item then
    ...
    RecipeItem.skills = partActionType.skills
    RecipeItem.requireInstalled = partActionType.requireInstalled
    RecipeItem.requireUninstalled = partActionType.requireUninstalled
    ...
    if self.vehicle then
        ...
        for partName, infoTable in pairs(tuningTable.parts) do
            ...
            if part and part:getInventoryItem() then
                ...
                local RecipeItem = self:createRecipeItemFor(infoTable, partName, partTuningInfo, modelName, "uninstall", getText("IGUI_TuningCategory_Installed"))
            ...
            if part then
                for modelName, partTuningInfo in pairs(infoTable) do
                    if not installPartList[partName] or installPartList[partName] ~= modelName then
                        local RecipeItem = self:createRecipeItemFor(infoTable, partName, partTuningInfo, modelName, "install")
                        ...
                        local RecipeItem = self:createRecipeItemFor(infoTable, partName, partTuningInfo, modelName, "uninstall")
                        RecipeItem.condition = part:getCondition()
                        if not RecipeItem.hideIfNotValid or (RecipeItem.hideIfNotValid and RecipeItem.available) then
                            if not self.recipesList[RecipeItem.category] then
                                self.recipesList[RecipeItem.category] = {};
...
function ISVehicleTuning2:isWaterSource(item, count)
    -- Fk'n rounding differences between Java and Lua broke simple getCurrentUsesFloat()/getCurrentUsesFloat() here, so I added getCurrentUses()
...
function ISVehicleTuning2:drawNonEvolvedIngredient(y, item, alt)
    ...
    -- else
        local r,g,b = 1,1,1
        local r2,g2,b2,a2 = 1,1,1,0.9
        local typesAvailable = item.item.RecipeItem.typesAvailable
        if typesAvailable and not item.item.result then
            local availableByType = typesAvailable[item.item.fullType]
            local availableByTag = typesAvailable[item.item.itemTag]
            if (not availableByType or availableByType < item.item.count) and (not availableByTag or availableByTag < item.item.count) then
                r,g,b = 0.54,0.54,0.54
...
-- print("ISVehicleTuning2:refreshIngredientPanel()")
    ...
    if RecipeItem.result then
        for _,item in ipairs(RecipeItem.result) do
            ...
            data.icon = item.icon
            data.fullType = item.fullType
            data.itemTag = item.itemTag--unused for results
            data.count = item.count
            data.result = true
            -- data.recipe = RecipeItem.recipe
            data.multiple = false
...
function ISVehicleTuning2:equipRequiredItems(playerObj, tools, itemsUsed)
    ...
    for equip,toolItem in pairs(tools) do
        ...
        for _,itemObj in ipairs(itemsUsed) do
            if itemObj:getFullType() == toolItem.fullType or itemHasTag(itemObj,toolItem.itemTag) then
                item = itemObj
                break
            end
        end
...
function ISVehicleTuning2:onInstallPart(button)
    ...
    if RecipeItem.type == "install" then
        -- print("install")--
        local action = ISInstallTuningVehiclePart:new(self.character, part, RecipeItem.modelName)
        ISTimedActionQueue.add(action);
    else
        -- print("uninstall")
        local action = ISUninstallTuningVehiclePart:new(self.character, part, RecipeItem.modelName)
        ISTimedActionQueue.add(action);
    end
    -- print("close")
    self:close()
...
function ISVehicleTuning2:GetItemInstance(typeOrTag)--TODO2: ensure the fact it is a tag is visible by player ?
    if not self.ItemInstances then self.ItemInstances = {} end
    local item = self.ItemInstances[typeOrTag]
    if not item then
        itemType = typeOrTag
        local tag = getTag(typeOrTag)
        if tag then--get the first available item in scriptManager list that has the tag
            local items = getScriptManager():getItemsTag(tag)
            if items:size() > 0 then
                item = items:get(0)
                if item then
                    itemType = item:getFullName()
                end
            end
        end
        item = instanceItem(itemType)
        if item then
            self.ItemInstances[typeOrTag] = item
...
function ISVehicleTuning2:GetTagName(typeOrTag)
    return typeOrTag
end
...
function ISVehicleTuning2:debugGiveIngredients()
    ...
    if RecipeItem.use then
        for _,itemInList in ipairs(RecipeItem.use) do
            local itemTag = itemInList.itemTag
            local tag = getTag(itemTag)
            if tag then tagTmpArray:clear() end
            local itemsAlreadyThere = tag and self.character:getInventory():getAllTagRecurse(tag,tagTmpArray) or self.character:getInventory():getAllTypeRecurse(itemInList.fullType)
            if tag and itemsAlreadyThere:size() == 0 then itemsAlreadyThere = self.character:getInventory():getAllTypeRecurse(itemInList.fullType) end--patch for vanilla BlowTorch not having the BlowTorch tag.
            if itemInList.isDrainable then
                local count = 0
                for i=0, itemsAlreadyThere:size()-1 do--count remaining uses in items in inventory
                    local itemAlreadyThere = itemsAlreadyThere:get(i)
                    if itemAlreadyThere then
                        count = count + itemAlreadyThere:getCurrentUses()
                    end
                end
                local usesPerItem = 10
                if count < itemInList.count then
                    local drainableScriptItem = ScriptManager.instance:FindItem(itemInList.fullType)
                    if drainableScriptItem then
                        usesPerItem = math.floor((1.0 / drainableScriptItem:getUseDelta())+0.01)
                    end
                end
                
                while count < itemInList.count do
                    self:addItem(itemInList.fullType)
                    count = count + usesPerItem
                end
            else
                for i=itemsAlreadyThere:size()+1,itemInList.count do
                    self:addItem(itemInList.fullType)
                end
            end
        end
    ...
    if RecipeItem.tools then
        for _,itemInList in pairs(RecipeItem.tools) do
            ...
            local tag = getTag(itemTag)
            if tag then tagTmpArray:clear() end
            local array = tag and self.character:getInventory():getAllTagRecurse(tag,tagTmpArray) or self.character:getInventory():getAllTypeRecurse(itemInList.fullType)
            if tag and array:size() == 0 then array = self.character:getInventory():getAllTypeRecurse(itemInList.fullType) end--patch for vanilla BlowTorch not having the BlowTorch tag.
            for i=array:size()+1,itemInList.count do
                self:addItem(itemInList.fullType)
            end
        end
    end
...
function ISVehicleTuning2:addItem(fullType)
    local playerObj = self.character
    if not playerObj or playerObj:isDead() then return end
    if isClient() then
        SendCommandToServer("/additem \"" .. playerObj:getDisplayName() .. "\" \"" .. luautils.trim(fullType) .. "\"")
    else
        local item = instanceItem(fullType)
        if not item then print("ISVehicleTuning2:addItem invalid ItemType "..luautils.trim(fullType)) end
