...
function DAMN.Parts:mainPartFn(vehicle, part, partConfig, rootNsName, partNS)
	...
	part = part or vehicle:getPartById(partConfig["partId"]);
...
function DAMN.Parts:initPartFn(vehicle, part, partConfig, rootNsName, partNS)
    if DAMN["partDefaultsDebug"]
    then
        DAMN:log("Executing [" .. tostring(rootNsName) .. ".Init." .. tostring(partNS) .. "]");
    end

	if not vehicle:getPartById(partConfig["partId"])
	then
		DAMN:log(" -> Part slot " .. tostring(partConfig["partId"]) .. " not found!");
...
function DAMN.Parts:fixPropanePartCondition(part, partId)
	local item = part:getInventoryItem();

    if item and item:getFullType() == "Base.PropaneTank" and part:getCondition() > 10
    then
        part:setCondition(part:getCondition() / 10);
    end
end
