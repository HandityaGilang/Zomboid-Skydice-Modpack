...
function PleaseKeepColor(craftRecipeData, character)
    -- 1) Retrieve consumed items and result
    local itemsConsumed = craftRecipeData:getAllConsumedItems()
    local resultItems   = craftRecipeData:getAllCreatedItems()
    local resultItem    = resultItems and resultItems:get(0)
    if not resultItem then
        return
    end

    -- "player" is now "character"
    local player_Inventory = character:getInventory()
    local transferred_Items = {} -- table for storing items from the old bag

    -- We'll look for the bag in the consumed items
    local backpack, backpackVisual, resultVisual

    -- Find the old bag among the consumed items
    for i = 0, itemsConsumed:size() - 1 do
        local dItem = itemsConsumed:get(i)
        if dItem and dItem:getCategory() == "Container" then
            backpack = dItem
            break
        end
    end

    -- If we found the old backpack, gather its inventory
    if backpack then
        backpackVisual = backpack:getVisual()
        resultVisual   = resultItem:getVisual()

        -- If the player is wearing or holding the old bag, unequip it
        if character:getClothingItem_Back() == backpack then
            character:setClothingItem_Back(nil)
        end
        if character:getPrimaryHandItem() == backpack then
            character:setPrimaryHandItem(nil)
        end
        if character:getSecondaryHandItem() == backpack then
        ...
        if oldBagInv then
            ...
            for i2 = 0, oldBagItems:size() - 1 do
                local invItem = oldBagItems:get(i2)
                table.insert(transferred_Items, invItem)
            end
        end
    end

    -- Now move them into the new bag’s inventory
    local newInv = resultItem:getInventory()  -- new bag inventory
    for _, itemToMove in ipairs(transferred_Items) do
        local itemContainer = itemToMove:getContainer()
        if itemContainer then
            itemContainer:Remove(itemToMove)
        end
        if newInv then
            newInv:AddItem(itemToMove)
        end
    end
