...
function TMRadioChannel4.OnLoadRadioScripts()
	TMRadioChannel4.Init()
	table.insert(DynamicRadio.scripts, TMRadioChannel4)
end

function TMRadioChannel4.OnEveryHour(_channel, _gametime, _radio)
	local hour = _gametime:getHour()

	if hour<120 then
		local bc = TMRadioChannel4.CreateBroadcast(_gametime)
		_channel:setAiringBroadcast(bc)
    	end
end

function TMRadioChannel4.OnEveryTenMinutes(_channel, _gametime, _radio)
	local hour = _gametime:getHour()

	if hour<120 then
		local bc = TMRadioChannel4.CreateBroadcastEveryTenMinutes(_gametime)
        	_channel:setAiringBroadcast(bc)
    	end
end

function TMRadioChannel4.OnNewSong(_channel, _gametime, _radio, prettyName)
	local hour = _gametime:getHour()

	if hour<120 then	
		local bc = TMRadioChannel4.CreateBroadcastNewSong(_gametime, prettyName)
		_channel:setAiringBroadcast(bc)
    	end
end

Events.OnLoadRadioScripts.Add(TMRadioChannel4.OnLoadRadioScripts)

function TMRadioChannel4.CreateBroadcast(_gametime)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	TMRadioChannel4.FillBroadcast(_gametime, bc)
	return bc
end

function TMRadioChannel4.CreateBroadcastEveryTenMinutes(_gametime)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	TMRadioChannel4.FillBroadcastEveryTenMinutes(_gametime, bc)
	return bc
end

function TMRadioChannel4.CreateBroadcastNewSong(_gametime, prettyName)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	TMRadioChannel4.FillBroadcastNewSong(_gametime, bc, prettyName)
	return bc
end

function TMRadioChannel4.FillBroadcast(_gametime, _bc)
	local hour = _gametime:getHour()
	local c = { r=1.0, g=1.0, b=1.0 }

	_bc:AddRadioLine( RadioLine.new("[img=music]", c.r, c.g, c.b) )

	if SandboxVars.TrueMusicRadio.TMRRadioHordeNightBroadcast then
    		TMRadioChannel4.AddHordeNightNotice(c, _bc)
	end

	if SandboxVars.TrueMusicRadio.TMRRadioWeatherBroadcast then
    		TMRadioChannel4.AddForecasting(c, _bc, hour)
	end

	_bc:AddRadioLine( RadioLine.new("[img=music]", c.r, c.g, c.b) )
end

function TMRadioChannel4.FillBroadcastEveryTenMinutes(_gametime, _bc)
    	local hour = _gametime:getHour()
    	local c = { r=1.0, g=1.0, b=1.0 }
...
function TMRadioChannel4.FillBroadcastNewSong(_gametime, _bc, prettyName)
    	local hour = _gametime:getHour()
    	local c = { r=1.0, g=1.0, b=1.0 }
...
function TMRadioChannel4.AddHordeNightNotice(_c, _bc, _force)
	if not SandboxVars.TrueMusicRadio.TMRRadioHordeNightBroadcast then
		return
	end
...
function TMRadioChannel4.AddForecasting(_c, _bc, _hour)
    	local clim = getClimateManager()
    	local forecaster = clim:getClimateForecaster()

        -- forecast today and tomorrow
       	local forecast = forecaster:getForecast()
       	TMRadioChannel4.AddForecast(_c, _bc, forecast, getRadioText("AEBS_Pre_today"), _hour<12)

       	local forecast = forecaster:getForecast(1)
       	TMRadioChannel4.AddForecast(_c, _bc, forecast, getRadioText("AEBS_Pre_tomorrow"), true)

       	TMRadioChannel4.AddExtremesForecasting(_c, _bc, 2)
end

function TMRadioChannel4.AddForecast(_c, _bc, _forecast, _prefix, _doFog)
    	local s = _prefix
   	s = s .. TMRadioChannel4.GetForecastString(1, _forecast)
    	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )

    	s = TMRadioChannel4.GetForecastString(2, _forecast)
    	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )

    	if _doFog and _forecast:isHasFog() then
        	s = TMRadioChannel4.GetForecastString(3, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	end

    	if _forecast:isWeatherStarts() then
        	-- a new weather period starts
        	s = TMRadioChannel4.GetForecastString(4, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	elseif _forecast:getWeatherOverlap() then
        	-- a already started weather period overlaps this day
        	s = TMRadioChannel4.GetForecastString(5, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	end
end

function TMRadioChannel4.GetForecastString(_type, _forecast)
    	local s = ""
    	if _type==1 then
        	...
        	s = string.format(" " .. "average temperature " .. Temperature.getTemperatureString(a) .. ". Minimum: " .. Temperature.getTemperatureString(b) .. ". Maximum: " .. Temperature.getTemperatureString(c) .. ". Humidity: " .. d .. "%%...")
    	elseif _type==2 then
		...
		local a,b,c = v:getTotalMean(), v:getTotalMin(), v:getTotalMax()
        	...
        	s = string.format(w .. " wind from the " .. d .. ". Maximum of " .. c .. " expected. " .. e)
    	elseif _type==3 then
    	...
    	elseif _type==4 or _type==5 then
        	if _type==4 then
           		--s = string.format(getRadioText("AEBS_weather_0_a"), TMRadioChannel4.GetDaySegmentForHour(_forecast:getWeatherStartTime()))
           		s = string.format("WARNING: Period of weather predicted to start in the " .. TMRadioChannel1.GetDaySegmentForHour(_forecast:getWeatherStartTime()) .. "... ")
        	else
            		local endTime = _forecast:getWeatherEndTime()
            		if endTime>=22 then
                		s = getRadioText("AEBS_weather_0_b")
            		else
                		--s = string.format(getRadioText("AEBS_weather_0_c"), TMRadioChannel4.GetDaySegmentForHour(endTime))
                		s = string.format("Weather period predicted to end in the " .. TMRadioChannel1.GetDaySegmentForHour(endTime) .. "... ")
            		end
...
function TMRadioChannel4.GetDaySegmentForHour(_hour)
...
function TMRadioChannel4.AddExtremesForecasting(_c, _bc, offset, _len)
    	local clim = getClimateManager()
    	local forecaster = clim:getClimateForecaster()
    	...
    	for i=offset,offset+(_len or 3) do
        	...
        	if forecast and ( forecast:isHasBlizzard() or forecast:isHasTropicalStorm() or forecast:isHasStorm() ) then
            		...
            		local s = string.format("Severe Weather Warning. " .. type .. " Imminent. ETA " .. tostring(i) .. " days...")
            		_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
            		return
        	end
    	end
