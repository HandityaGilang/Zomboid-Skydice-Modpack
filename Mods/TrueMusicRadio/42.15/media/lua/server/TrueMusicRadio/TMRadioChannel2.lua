...
function TMRadioChannel2.OnLoadRadioScripts()
	TMRadioChannel2.Init()
	table.insert(DynamicRadio.scripts, TMRadioChannel2)
end

function TMRadioChannel2.OnEveryHour(_channel, _gametime, _radio)
	local hour = _gametime:getHour()

	if hour<120 then
		local bc = TMRadioChannel2.CreateBroadcast(_gametime)
		_channel:setAiringBroadcast(bc)
    	end
end

function TMRadioChannel2.OnEveryTenMinutes(_channel, _gametime, _radio)
	local hour = _gametime:getHour()

	if hour<120 then
		local bc = TMRadioChannel2.CreateBroadcastEveryTenMinutes(_gametime)
        	_channel:setAiringBroadcast(bc)
    	end
end

function TMRadioChannel2.OnNewSong(_channel, _gametime, _radio, prettyName)
	local hour = _gametime:getHour()

	if hour<120 then	
		local bc = TMRadioChannel2.CreateBroadcastNewSong(_gametime, prettyName)
		_channel:setAiringBroadcast(bc)
    	end
end

Events.OnLoadRadioScripts.Add(TMRadioChannel2.OnLoadRadioScripts)

function TMRadioChannel2.CreateBroadcast(_gametime)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	TMRadioChannel2.FillBroadcast(_gametime, bc)
	return bc
end

function TMRadioChannel2.CreateBroadcastEveryTenMinutes(_gametime)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	TMRadioChannel2.FillBroadcastEveryTenMinutes(_gametime, bc)
	return bc
end

function TMRadioChannel2.CreateBroadcastNewSong(_gametime, prettyName)
	local bc = RadioBroadCast.new("GEN-"..tostring(ZombRand(100000,999999)),-1,-1)
	TMRadioChannel2.FillBroadcastNewSong(_gametime, bc, prettyName)
	return bc
end

function TMRadioChannel2.FillBroadcast(_gametime, _bc)
	local hour = _gametime:getHour()
	...
	if SandboxVars.TrueMusicRadio.TMRRadioHordeNightBroadcast then
    		TMRadioChannel2.AddHordeNightNotice(c, _bc)
	end

	if SandboxVars.TrueMusicRadio.TMRRadioWeatherBroadcast then
    		TMRadioChannel2.AddForecasting(c, _bc, hour)
	end

	_bc:AddRadioLine( RadioLine.new("[img=music]", c.r, c.g, c.b) )
end

function TMRadioChannel2.FillBroadcastEveryTenMinutes(_gametime, _bc)
    	local hour = _gametime:getHour()
    	local c = { r=1.0, g=1.0, b=1.0 }
...
function TMRadioChannel2.FillBroadcastNewSong(_gametime, _bc, prettyName)
    	local hour = _gametime:getHour()
    	local c = { r=1.0, g=1.0, b=1.0 }
...
function TMRadioChannel2.AddHordeNightNotice(_c, _bc, _force)
...
function TMRadioChannel2.AddForecasting(_c, _bc, _hour)
    	local clim = getClimateManager()
    	local forecaster = clim:getClimateForecaster()

        -- forecast today and tomorrow
       	local forecast = forecaster:getForecast()
       	TMRadioChannel2.AddForecast(_c, _bc, forecast, getRadioText("AEBS_Pre_today"), _hour<12)

       	local forecast = forecaster:getForecast(1)
       	TMRadioChannel2.AddForecast(_c, _bc, forecast, getRadioText("AEBS_Pre_tomorrow"), true)

       	TMRadioChannel2.AddExtremesForecasting(_c, _bc, 2)
end

function TMRadioChannel2.AddForecast(_c, _bc, _forecast, _prefix, _doFog)
    	local s = _prefix
   	s = s .. TMRadioChannel2.GetForecastString(1, _forecast)
    	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )

    	s = TMRadioChannel2.GetForecastString(2, _forecast)
    	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )

    	if _doFog and _forecast:isHasFog() then
        	s = TMRadioChannel2.GetForecastString(3, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	end

    	if _forecast:isWeatherStarts() then
        	-- a new weather period starts
        	s = TMRadioChannel2.GetForecastString(4, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	elseif _forecast:getWeatherOverlap() then
        	-- a already started weather period overlaps this day
        	s = TMRadioChannel2.GetForecastString(5, _forecast)
        	_bc:AddRadioLine( RadioLine.new(comp(s), _c.r, _c.g, _c.b) )
    	end
end

function TMRadioChannel2.GetForecastString(_type, _forecast)
    	local s = ""
    	if _type==1 then
    	...
    	elseif _type==4 or _type==5 then
        	if _type==4 then
           		--s = string.format(getRadioText("AEBS_weather_0_a"), TMRadioChannel2.GetDaySegmentForHour(_forecast:getWeatherStartTime()))
           		s = string.format("WARNING: Period of weather predicted to start in the " .. TMRadioChannel1.GetDaySegmentForHour(_forecast:getWeatherStartTime()) .. "... ")
            		...
            		if endTime>=22 then
                		...
                		--s = string.format(getRadioText("AEBS_weather_0_c"), TMRadioChannel2.GetDaySegmentForHour(endTime))
...
function TMRadioChannel2.GetDaySegmentForHour(_hour)
...
function TMRadioChannel2.AddExtremesForecasting(_c, _bc, offset, _len)
    	local clim = getClimateManager()
    	local forecaster = clim:getClimateForecaster()
