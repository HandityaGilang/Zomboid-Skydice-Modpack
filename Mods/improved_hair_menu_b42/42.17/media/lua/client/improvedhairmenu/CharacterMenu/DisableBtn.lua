...
function CharacterCreationMain:disableBtn()
	...
	if self.hairMenu and self.hairMenu.syncSelectedInfoFromVisual then
		self.hairMenu:syncSelectedInfoFromVisual()
	end
	...
	if self.beardMenu then
		...
		if vis and self.beardMenu.syncSelectedInfoFromVisual then
			self.beardMenu:syncSelectedInfoFromVisual()
		end
	end
end