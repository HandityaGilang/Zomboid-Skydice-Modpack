...
IHM_ISColorPickerHSB = ISPanelJoypad:derive("IHM_ISColorPickerHSB");
...
local BTN_WIDTH = UI_BORDER_SPACING*2 + getTextManager():MeasureStringX(UIFont.Small, getText("UI_Close"))


function IHM_ISColorPickerHSB:createChildren()
	...
	self.hueSlider = ISSliderPanel:new(x2, y, SCROLL_BAR_WID, BUTTON_HGT, self, IHM_ISColorPickerHSB.onSliderChange);
	...
	self.satSlider = ISSliderPanel:new(x2, y, SCROLL_BAR_WID, BUTTON_HGT, self, IHM_ISColorPickerHSB.onSliderChange);
	...
	self.valSlider = ISSliderPanel:new(x2, y, SCROLL_BAR_WID, BUTTON_HGT, self, IHM_ISColorPickerHSB.onSliderChange);
	...
	local buttonClose = ISButton:new(x, self.height - UI_BORDER_SPACING - BUTTON_HGT - 1, BTN_WIDTH, BUTTON_HGT, getText("UI_Close"), self, self.onCloseButton)
	buttonClose:initialise()
	buttonClose:instantiate()
	buttonClose:setDisplayBackground(true)
	buttonClose:setBackgroundRGBA(0.45, 0.08, 0.08, 0.95)
	if buttonClose.setBorderRGBA then
		buttonClose:setBorderRGBA(0.9, 0.2, 0.2, 1.0)
	end
	buttonClose.backgroundColorMouseOver = { r = 0.60, g = 0.10, b = 0.10, a = 1.0 }
	self:addChild(buttonClose)
end

function IHM_ISColorPickerHSB:toHSB(Color)
	local r1 = Color:getR();
	local g1 = Color:getG();
	local b1 = Color:getB();
...
function IHM_ISColorPickerHSB:render()
...
function IHM_ISColorPickerHSB:applyLivePicked()
...
function IHM_ISColorPickerHSB:onSliderChange(value, slider)
	...
	self:applyLivePicked()
end

function IHM_ISColorPickerHSB:onMouseDown(x, y)
...
function IHM_ISColorPickerHSB:onMouseDownOutside(x, y)
...
function IHM_ISColorPickerHSB:onSave()
	if self.pickedFunc then
...
function IHM_ISColorPickerHSB:onCloseButton()
	self:removeSelf()
	return true
end

function IHM_ISColorPickerHSB:onMouseMove(dx, dy)
...
function IHM_ISColorPickerHSB:onMouseDownColorBox(x, y)
...
function IHM_ISColorPickerHSB:onMouseUp(x, y)
...
function IHM_ISColorPickerHSB:picked2(hide)
    if self.pickedFunc then
        self.pickedFunc(self.pickedTarget, self.pickedRGB, false, self.pickedArgs[1], self.pickedArgs[2], self.pickedArgs[3], self.pickedArgs[4])
    end
...
function IHM_ISColorPickerHSB:onMouseUpOutside(x, y)
	return self:onMouseUp(x, y)
end

function IHM_ISColorPickerHSB:onGainJoypadFocus(joypadData)
...
function IHM_ISColorPickerHSB:onLoseJoypadFocus(joypadData)
	ISPanelJoypad.onLoseJoypadFocus(self, joypadData)
	self:clearJoypadFocus(joypadData)
end

function IHM_ISColorPickerHSB:onJoypadDown(button)
	if button == Joypad.AButton or button == Joypad.BButton then
		self:removeSelf()
	end
end

function IHM_ISColorPickerHSB:removeSelf()
	if self.parent then
...
function IHM_ISColorPickerHSB:picked(hide)
	if hide then
		self:removeSelf()
	end
	if self.pickedFunc then
...
function IHM_ISColorPickerHSB:setInitialColor(initial)
	self.initialColor = initial:toColor();
	self.currentColor = initial:toColor();
	self.pickedRGB.r = self.currentColor:getRedFloat();
	self.pickedRGB.g = self.currentColor:getGreenFloat();
	self.pickedRGB.b = self.currentColor:getBlueFloat();
	self.h, self.s, self.b = IHM_ISColorPickerHSB:toHSB(initial:toColor());
...
function IHM_ISColorPickerHSB:setCurrentColor(h, s ,b)
...
function IHM_ISColorPickerHSB:setPickedFunc(func, arg1, arg2, arg3, arg4)
	self.pickedFunc = func
	self.pickedArgs = { arg1, arg2, arg3, arg4 }
end

function IHM_ISColorPickerHSB:new(x, y, initial)
	...
	o.b = 0.0;
		...
		o.h, o.s, o.b = IHM_ISColorPickerHSB:toHSB(initial:toColor());
	end
