...
function CharacterCreationMain:disableBtn()
	...
	if self.female ~= desc:isFemale() or CharacterCreationMain.forceUpdateCombo then
		...
		-- layout refresh is handled inside ReplaceCombobox.lua
	end
	...
	if self.hairMenu and self.hairMenu.info and self.hairTypeCombo and self.hairTypeCombo.selected and self.hairTypeCombo.selected > 0 then
	...
	if self.beardMenu then
		...
		if vis and self.beardMenu.info and self.beardTypeCombo and self.beardTypeCombo.selected and self.beardTypeCombo.selected > 0 then
