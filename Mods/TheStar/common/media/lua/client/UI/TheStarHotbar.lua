...
function ISHotbar:render()
  ...
  for i, slot in pairs(self.availableSlot) do
      ...
      if item then
          ...
          -- print("isWaterSource: "..tostring(isWaterSource))
          ...
          -- end
          if TheStarB42.Options:getOption("equippedItemOnly"):getValue() then
              showCondition = item:isEquipped()
          end
              ...
              if (hasConditionDropped and conditionLevel <= TheStar.Options.blinkCondition)
                  ...
                  if self.blinkCount <= TheStar.Config.BLINK_COUNT_MAX then
                      ...
                      if TheStarB42.Options:getOption("showProgressBar"):getValue() then
                          ...
                          if TheStarB42.Options:getOption("progressVertical"):getValue() == 2 then
                              self:drawRect(slotX + 1, self.margins + 1, self.slotWidth - 2, progressReversed, self.blinkAlpha, blinkColor.r, blinkColor.g, blinkColor.b)
                          else
                              self:drawRect(slotX + 1 + (self.slotWidth - 2) - progressReversed, self.margins + 1, progressReversed, self.slotHeight - 2, self.blinkAlpha, blinkColor.r, blinkColor.g, blinkColor.b)
                          end
                      else
                          -- Blink the whole bg
          ...
          if (isHandWeapon or isWaterSource or isDrainableComboItem)
            and TheStarB42.Options:getOption("showProgressBar"):getValue()
            and showCondition then

            -- print("getFluidContainer: " .. tostring(item:getFluidContainer()))

            -- Fluid container 여부와 관련 변수 초기화
            local isFluidContainer = item:getFluidContainer() ~= nil
            local fluidContainerRatio = 0
            if isFluidContainer then
                local container = item:getFluidContainer()
                -- print("amount: " .. container:getAmount() .. " getFilledRatio(): " .. container:getFilledRatio() .. " getCapacity(): " .. container:getCapacity())
                fluidContainerRatio = math.floor((self.slotWidth - 2) * container:getFilledRatio())
            end
            ...
            local progressColorA = TheStarB42.percentValues[TheStarB42.Options:getOption("progressBarOpacity"):getValue()]

            -- DrawRect 처리
            if TheStarB42.Options:getOption("progressVertical"):getValue() == 2 then
                -- 수직 진행바
                local rectHeight = isFluidContainer and fluidContainerRatio or progress
                -- print("progress: " .. tostring(progress) .. " slotX + 1 : " .. (slotX + 1) .. " self.slotHeight: " .. self.slotHeight .. " self.margins: " .. self.margins .. " self.slotWidth: " .. self.slotWidth)
                -- print("height: " .. (self.margins + 1 + self.slotHeight - 2 - rectHeight) .. ", " .. (self.slotWidth - 2))
                self:drawRect(slotX + 1, self.margins + 1 + self.slotHeight - 2 - rectHeight, self.slotWidth - 2, rectHeight, progressColorA, progressColor.r, progressColor.g, progressColor.b)
            else
                -- 수평 진행바
                local rectWidth = isFluidContainer and fluidContainerRatio or progress
                self:drawRect(slotX + 1, self.margins + 1, rectWidth, self.slotHeight - 2, progressColorA, progressColor.r, progressColor.g, progressColor.b)
            end
          end
          ...
          if TheStar.Options.showBatteryChargeHotbar and TheStar.Utils.isLightSource(item) and showCondition then
              local battery = TheStar.Utils.round(item:getCurrentUsesFloat() * 100) .. "%"
          ...
          -- end
          if isHandWeapon and TheStarB42.Options:getOption("showIcon"):getValue() and showCondition then
              local n = math.ceil(conditionLevel * 5)
              local iconTexture = TheStar.Utils.getIconTexture(n)
