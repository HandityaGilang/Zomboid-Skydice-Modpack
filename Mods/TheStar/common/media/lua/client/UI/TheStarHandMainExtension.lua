...
function TheStar.HandMainExtension.Ammo.checkAmmo(player, item)
  if item and TheStar.Utils.isRangedWeapon(item) and player:isLocalPlayer() then
      if TheStarB42.Options:getOption("showAmmoCountInventory"):getValue() and inventoryAmmoTicksPassed == 0 then
      ...
      local chamber = item:haveChamber() and (item:isRoundChambered() and "1" or "0") or "0"

      local text = getText(format, currentAmmo, maxAmmo, chamber)
      if not text or text == "" then
          text = string.format("%d / %d", currentAmmo, maxAmmo)
          if item:haveChamber() then
              text = string.format("%d+%s / %d", currentAmmo, chamber, maxAmmo)
          end
      end

      if TheStarB42.Options:getOption("showAmmoCountInventory"):getValue() then
          text = text .. " (" .. (inventoryAmmoCached or "0") .. ")"
      else
          inventoryAmmoTooltipCached = nil
      end

      local reload = currentAmmo == 0 and not item:isRoundChambered()

      -- handMainExtensionUI가 nil이 아닐 때만 UI 업데이트 수행
      if handMainExtensionUI then
          ...
          print("Warning: handMainExtensionUI is nil, skipping UI update.")
      end
...
function TheStar.HandMainExtension.Ammo.onEquipPrimary(player, item)
  ...
  if TheStar.Utils.isRangedWeapon(item) then
      ticksPassed = 0
      inventoryAmmoTicksPassed = 0
  ...
  if not TheStar.Utils.isLightSource(item) then
      hideUI()
  end
