...
local function generalInstrumentConditions(thisPlayer, item)

    if item and item.getFullType and item:getFullType() and string.find(item:getFullType(), "Broken") then return false; end
...
VanillaInstrumentsContextMenu.doBuildMenu = function(player, context, worldobjects, playableInstrument, ItemName)
    ...
    if not generalInstrumentConditions(thisPlayer, playableInstrument) then return; end
    ...
    if not playableInstrument then
        ...
        Type, InstrumentIconTexture, learnedTracksData, InstrumentTracksDuet = getInstrumetProperties(playableInstrument:getFullType(), playerdata, false)
...
VanillaInstrumentsContextMenu.doInventoryMenu = function(player, context, items, item)
    ...
    if not generalInstrumentConditions(thisPlayer, item) then return; end
