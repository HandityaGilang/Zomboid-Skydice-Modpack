...
function PlayerDanceWasAccepted(DanceProposer, DancePartner, PartnerX, PartnerY)
    ...
    --sendClientCommand(DanceProposer, "LSK", "StopDance", {DancePartner:getOnlineID()})
    --return
    --end
    ...
    for playerIndex = 0, getNumActivePlayers()-1 do
        ...
        if #playersList > 0 then
            for i,v in ipairs(playersList) do
                if v:getUsername() ~= playerObj:getUsername() and
                tostring(v:getUsername()) == DancePartner then
                    sendClientCommand(playerObj, "LSK", "FaceDanceProposer", {v:getOnlineID(), PX, PY})
                end
            end
        end            
    end
        ...
        if #playersList > 0 then
            for i,v in ipairs(playersList) do
                ...
                tostring(v:getUsername()) == DanceProposer then
                    sendClientCommand(playerObj, "LSK", "AcceptedDance", {v:getOnlineID(), dancePartner, PartnerX, PartnerY})
                end
            end
        end            
    end
    ...
    for playerIndex = 0, getNumActivePlayers()-1 do
        ...
        if #playersList > 0 then
            for i,v in ipairs(playersList) do
                if v:getUsername() ~= playerObj:getUsername() and
                tostring(v:getUsername()) == DanceProposer then
                    sendClientCommand(playerObj, "LSK", "OtherPlayerIsDancing", {v:getOnlineID(), IsDancing})
                end
            end
        end            
    end
