...
LSProductionInvPanel = ISPanel:derive("LSProductionInvPanel");
...
function LSProductionInvPanel:doDrawItem(y, item, alt)
...
function LSProductionInvPanel:initialise()
    ...
    local mediumNewTextHeight = getTextManager():getFontFromEnum(UIFont.MediumNew):getLineHeight()
        ...
        self.ScrollList.doDrawItem = LSProductionInvPanel.doDrawItem
    ...
    -- self.ScrollList:addScrollBars();
        ...
        for n=1,#self.knownGroups do
            local group = self.knownGroups[n]
            local tex = getTexture(self.groupRef[group])
            local name = getText("ContextMenu_Inventions_"..group)
            self.ScrollList:addItem(name, {texture=tex,data=group})
        end
    end

    self.ConfirmButton = ISButton:new(self.panelX+152, self.panelY+225, btnWid, btnHgt, "", self, LSProductionInvPanel.onClick)
    ...
    self.CloseButton = ISButton:new(self.panelX+56, self.panelY+225, btnWid, btnHgt, "", self, LSProductionInvPanel.onClick)
...
function LSProductionInvPanel:prerender()
    --[[
    local z = 20;
...
function LSProductionInvPanel:close()
    if self.parent then self.parent.invPanel = false; end
    self:setVisible(false)
    self:removeFromUIManager()
end

function LSProductionInvPanel:destroy()
    if self.parent then self.parent.invPanel = false; end
    self:setVisible(false)
    self:removeFromUIManager()
end

function LSProductionInvPanel:onClick(button)
    if button.internal == "CONFIRMINV" and self.params then
        self.parent:onInvPanelConfirmInv(self.params, self.cost)
    elseif button.internal == "CONFIRM" and self.ScrollList.selected and self.ScrollList.items[self.ScrollList.selected].item.data ~= self.selectedGroup then
...
function LSProductionInvPanel:update()
    self:bringToTop()
...
function LSProductionInvPanel:new(panel, selectedGroup, knownGroups, groupRef, cost, params, menuSkin)
    ...
    o.parent = panel
    o.knownGroups = knownGroups
    o.groupRef = groupRef
    o.width = width
    o.height = height
    o.selectedGroup = selectedGroup
    o.cost = cost
    o.params = params
    o.menuSkin = menuSkin
    local pX, pY = 14, 50
    ...
    o.panelX = pX
    o.panelY = pY
    LSProductionInvPanel.instance = o;
    return o;
end
