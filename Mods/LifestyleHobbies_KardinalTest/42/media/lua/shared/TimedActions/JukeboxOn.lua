...
function JukeboxOn:perform()
    ...
    --sendClientCommand(playerObj, "LSK", "JukeTurnedOn", {genre, x, y, z, JukeReusableID, playercommand})
    --print("before transmit")
    --self.Jukebox:transmitModData()
    
    if isClient() then
    sendClientCommand("LSK", "ModifyObjData", {{x, y, z, self.Jukebox:getSprite():getName()}, false, self.Jukebox:getModData()})
    sendClientCommand("LSK", "JukeboxStart", {x, y, z})
    else
    ...
    --sendClientCommand(playerObj, "LSK", "IsTurningJukeboxOn", {JukeboxID})
...
function JukeboxOn:complete()
    ...
    --local z = self.Jukebox:getZ()
    --self.Jukebox:transmitModData()
    --sendServerCommand("LSK", "JukeboxStart", {x, y, z})
    return true
end

function JukeboxOn:getDuration()
