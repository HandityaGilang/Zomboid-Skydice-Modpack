...
local function getCurrentKills(player)
    ...
    if success and type(kills) == "number" and kills >= 0 then
        return kills
    end
...
local function onServerCommand(module, command, args)
    ...
    elseif command == "KillsSynced" then
        ...
        if args.rewardsGiven ~= nil and args.rewardsGiven > bcrData.rewardsGiven then 
...
Events.OnPlayerUpdate.Add(function(player)
    ...
    if not ok then BCR.DebugPrint("[Client] OnPlayerUpdate error: " .. tostring(err)) end
end)
