...
function ISWearClothing:update()
    -- Mantém atualização de progresso para animação
    self.item:setJobDelta(self:getJobDelta());

    if not self.equipSound and self:getJobDelta() > 0.5 and self.item and self.item:getEquipSound() then
...
function ISWearClothing:complete()
    ...
    elseif self.item:getCategory() == "Clothing" or self.item:getCategory() == "AlarmClock" then
        if self.item:getBodyLocation() ~= "" then
            ...
            if self.character:getHumanVisual():getHairModel():contains("Mohawk") and 
               (self.item:getBodyLocation() == ItemBodyLocation.HAT or self.item:getBodyLocation() == ItemBodyLocation.FULL_HAT) then
...
function ISWearClothing:getDuration()
    ...
    return 15; -- Reduzido de 50 para 15 (3x mais rápido, mas com animação)
end
...
function ISWearClothing:new(character, item)
    ...
    o.stopOnWalk = false; -- Não para ao andar
    o.stopOnRun = false;  -- Não para ao correr
    o.stopOnAim = false;
    return o;
end
...
ISWearClothing.isStopOnWalk = function(item)
    return false -- Nunca para ao andar
end