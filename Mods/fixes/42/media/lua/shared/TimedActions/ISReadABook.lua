...
function ISReadABook:isValid()
    ...
    -- Verificar se jogador é analfabeto
    if self.character:hasTrait(CharacterTrait.ILLITERATE) then
        HaloTextHelper.addBadText(self.character, getText("IGUI_PlayerText_DontUnderstand"));
        return false
    end
    
    -- Verificar nível do livro vs nível do jogador
    if not self:canReadBook() then
        return false
    end
...
-- NOVO: Verificar se o jogador pode ler este livro
function ISReadABook:canReadBook()
    local skillBook = SkillBook[self.item:getSkillTrained()]
    
    -- Se não é um livro de habilidade, pode ler
    if not skillBook then
        return true
    end
    
    local perk = skillBook.perk
    local playerLevel = self.character:getPerkLevel(perk)
    local bookMinLevel = self.item:getLvlSkillTrained()
    local bookMaxLevel = self.item:getMaxLevelTrained()
    
    -- Livro muito avançado para o jogador (precisa ter pelo menos nível anterior - 1)
    if bookMinLevel > playerLevel + 1 then
        HaloTextHelper.addBadText(self.character, getText("IGUI_PlayerText_TooComplicated"));
        return false
    end
    
    -- Jogador já passou do nível máximo do livro
    if bookMaxLevel < playerLevel + 1 then
...
function ISReadABook:update()
    -- Verificar novamente se pode ler durante a leitura
    if not self:canReadBook() then
        self:forceStop()
        return
    end
...
function ISReadABook:start()
    -- NOVO: Verificar antes de iniciar
    if not self:canReadBook() then
        self:forceStop()
        return
    end
...
function ISReadABook:perform()
    -- NOVO: Verificação final antes de completar
    if not self:canReadBook() then
        self.character:setReading(false);
        self.item:setJobDelta(0.0);
...
function ISReadABook:complete()
    -- NOVO: Verificação final antes de aplicar benefícios
    if not self:canReadBook() then
        self.item:setJobDelta(0.0);
        return false
    end
...
function ISReadABook:serverStart()
    -- NOVO: Verificação no servidor antes de iniciar
    if not self:canReadBook() then
        self.netAction:forceComplete()
        return
    end
...
function ISReadABook:getDuration()
    if self.character:isTimedActionInstant() then
        -- NOVO: Mesmo com ação instantânea, verificar se pode ler
        if not self:canReadBook() then
            return 1
        end
        return 1;
