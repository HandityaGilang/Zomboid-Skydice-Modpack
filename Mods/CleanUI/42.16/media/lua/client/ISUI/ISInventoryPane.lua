...
local function CleanUI_getItemRarityMetrics(self, scrollBarWid)
    ...
    local cacheKey = tostring(self.width) .. "|" .. tostring(self.column3) .. "|" .. tostring(self.padding)
        .. "|" .. tostring(scrollBarWid) .. "|" .. tostring(self.font) .. "|" .. tostring(storedWidthKey)
    ...
    local maxRarityWidth = math.floor(math.min(measuredMaxWidth, math.max(50, availableWidth - minCategoryWidth - self.padding)))
    local hideThreshold = math.max(26, math.floor(maxRarityWidth * 0.45))

    local isHidden = false
    local rarityWidth = storedWidth
    if type(rarityWidth) ~= "number" then
        rarityWidth = maxRarityWidth
    end

    if availableWidth <= minCategoryWidth + hideThreshold + self.padding then
        isHidden = true
        rarityWidth = 0
    elseif rarityWidth <= hideThreshold then
        isHidden = true
        rarityWidth = 0
    else
        rarityWidth = math.floor(math.max(hideThreshold, math.min(rarityWidth, maxRarityWidth)))
    end

    local separatorX = nil
    if isHidden then
        separatorX = math.floor(contentRight - self.padding)
    else
        separatorX = math.floor(contentRight - rarityWidth - self.padding)
    end

    separatorX = math.max(categoryLeft + minCategoryWidth, separatorX)

    local metrics = {
        contentRight = contentRight,
        separatorX = separatorX,
        categoryRight = isHidden and contentRight or (separatorX - self.padding),
...
local function CleanUI_updateRarityColumnResizeButton(self, scrollBarWid)
    if not self.rarityColumnResizeButton then
        return
    end

    local rarityMetrics = CleanUI_getItemRarityMetrics(self, scrollBarWid)
    ...
    self.rarityColumnResizeButton:setX(math.max(self.column3, rarityMetrics.separatorX - self.columnResizeButtonSize))
    self.rarityColumnResizeButton:setY(0)
    self.rarityColumnResizeButton:setWidth(self.columnResizeButtonSize)
    self.rarityColumnResizeButton:setHeight(self.columnResizeButtonSize)
end
...
function ISInventoryPane:onMouseMove(dx, dy)
    ...
    if self.resizingRarityColumn then
        ...
        local categoryLeft = self.column3 + self.padding
        local newSeparatorX = math.floor(x + self.columnResizeButtonSize / 2)
        newSeparatorX = math.max(categoryLeft + rarityMetrics.minCategoryWidth, math.min(contentRight - self.padding, newSeparatorX))

        local proposedRarityWidth = math.floor(contentRight - newSeparatorX - self.padding)
        if type(ItemRarityUI) == "table" then
            if proposedRarityWidth <= rarityMetrics.hideThreshold then
                ItemRarityUI.rarityColumnWidth = 0
            else
                ItemRarityUI.rarityColumnWidth = math.min(proposedRarityWidth, rarityMetrics.maxRarityWidth)
            end
        end

        CleanUI_updateRarityColumnResizeButton(self, scrollBarWid)
        return
    end
               ...
               -- print("trace:j")
                ...
                if item:getJobDelta() > 0  then
                    ...
                    if count == 1 then
                        local rarityMetrics = CleanUI_getItemRarityMetrics(self, scrollBarWid)
                        local rightBoundary = self.width - self.padding - scrollBarWid
                        local categoryRightBoundary = rarityMetrics and rarityMetrics.categoryRight or rightBoundary
