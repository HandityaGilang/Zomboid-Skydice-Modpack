...
CleanUI_Vanilla_ISInventoryPane = ISPanel:derive("CleanUI_Vanilla_ISInventoryPane");

CleanUI_Vanilla_ISInventoryPane.MAX_ITEMS_IN_STACK_TO_RENDER = 50
CleanUI_Vanilla_ISInventoryPane.ghc = getCore():getGoodHighlitedColor()
...
--************************************************************************--
--** CleanUI_Vanilla_ISInventoryPane:initialise
--**
--************************************************************************--

function CleanUI_Vanilla_ISInventoryPane:initialise()
...
--**
--************************************************************************--
function CleanUI_Vanilla_ISInventoryPane:createChildren()
    ...
    self.expandAll = ISButton:new(1, 0, self.headerHgt, self.headerHgt, "", self, CleanUI_Vanilla_ISInventoryPane.expandAll);
  ...
  --  self.expandAll.backgroundColorMouseOver.a = 0;
    ...
    self.collapseAll = ISButton:new(1, 0, self.headerHgt, self.headerHgt, "", self, CleanUI_Vanilla_ISInventoryPane.collapseAll);
   ...
   -- self.collapseAll.backgroundColorMouseOver.a = 0;
    ...
    self.filterMenu = ISButton:new(self.headerHgt+1, 0, self.headerHgt, self.headerHgt, "", self, CleanUI_Vanilla_ISInventoryPane.onFilterMenu);
    ...
    self.nameHeader = ISResizableButton:new(self.column2, 0, (self.column3 - self.column2), self.headerHgt, getText("IGUI_invpanel_Type"), self, CleanUI_Vanilla_ISInventoryPane.sortByName);
	self.nameHeader:initialise();
	self.nameHeader.borderColor.a = 0.2;
	self.nameHeader.minimumWidth = 100
	self.nameHeader.onresize = { CleanUI_Vanilla_ISInventoryPane.onResizeColumn, self, self.nameHeader }
	self:addChild(self.nameHeader);

	self.typeHeader = ISResizableButton:new(self.column3-1, 0, self.column4 - self.column3 + 1, self.headerHgt, getText("IGUI_invpanel_Category"), self, CleanUI_Vanilla_ISInventoryPane.sortByType);
	...
	self.typeHeader.onresize = { CleanUI_Vanilla_ISInventoryPane.onResizeColumn, self, self.typeHeader }
    ...
    self.contextButton1 = ISButton:new(0, 0, btnWid, btnHgt, getText("ContextMenu_Grab"), self, CleanUI_Vanilla_ISInventoryPane.onContext);
    ...
    self.contextButton2 = ISButton:new(0, 0, btnWid, btnHgt, getText("IGUI_invpanel_Pack"), self, CleanUI_Vanilla_ISInventoryPane.onContext);
    ...
    self.contextButton3 = ISButton:new(0, 0, btnWid, btnHgt, getText("IGUI_invpanel_Pack"), self, CleanUI_Vanilla_ISInventoryPane.onContext);
...
function CleanUI_Vanilla_ISInventoryPane:onResizeColumn(button)
...
function CleanUI_Vanilla_ISInventoryPane:onResize()
...
function CleanUI_Vanilla_ISInventoryPane:onContext(button)
    ...
    if button.mode == "unpack" then
        ...
        local items = CleanUI_Vanilla_ISInventoryPane.getActualItems({k})
...
function CleanUI_Vanilla_ISInventoryPane:collapseAll(button)
...
function CleanUI_Vanilla_ISInventoryPane:expandAll(button)
    for k, v in pairs(self.collapsed) do
...
function CleanUI_Vanilla_ISInventoryPane:onFilterMenu(button)
    local playerObj = getSpecificPlayer(self.player)
    local playerInv = playerObj:getInventory()
    ...
    subMenuWeight:addOption(getText("IGUI_invpanel_ascending"), self, CleanUI_Vanilla_ISInventoryPane.sortByWeight, true);
    subMenuWeight:addOption(getText("IGUI_invpanel_descending"), self, CleanUI_Vanilla_ISInventoryPane.sortByWeight, false);
end

function CleanUI_Vanilla_ISInventoryPane:sortByWeight(_isAscending)
    if _isAscending and self.itemSortFunc ~= CleanUI_Vanilla_ISInventoryPane.itemSortByWeightAsc then
        self.itemSortFunc = CleanUI_Vanilla_ISInventoryPane.itemSortByWeightAsc;
        self:refreshContainer();
    end
    if (not _isAscending) and self.itemSortFunc ~= CleanUI_Vanilla_ISInventoryPane.itemSortByWeightDesc then
        self.itemSortFunc = CleanUI_Vanilla_ISInventoryPane.itemSortByWeightDesc;
        self:refreshContainer();
    end
end

CleanUI_Vanilla_ISInventoryPane.itemSortByWeightAsc = function(a,b)
...
CleanUI_Vanilla_ISInventoryPane.itemSortByWeightDesc = function(a,b)
    if a.equipped and not b.equipped then return false end
    if b.equipped and not a.equipped then return true end
    --if a.cat == b.cat then return not string.sort(a.name, b.name) end
...
CleanUI_Vanilla_ISInventoryPane.itemSortByNameInc = function(a,b)
    if a.equipped and not b.equipped then return false end
    if b.equipped and not a.equipped then return true end
...
CleanUI_Vanilla_ISInventoryPane.itemSortByNameDesc = function(a,b)
    if a.equipped and not b.equipped then return false end
    if b.equipped and not a.equipped then return true end
	if a.inHotbar and not b.inHotbar then return true end
	if b.inHotbar and not a.inHotbar then return false end
...
CleanUI_Vanilla_ISInventoryPane.itemSortByCatInc = function(a,b)
    if a.equipped and not b.equipped then return false end
    if b.equipped and not a.equipped then return true end
    if a.cat == b.cat then return not string.sort(a.name, b.name) end
    return not string.sort(a.cat, b.cat);
...
CleanUI_Vanilla_ISInventoryPane.itemSortByCatDesc = function(a,b)
    if a.equipped and not b.equipped then return false end
    if b.equipped and not a.equipped then return true end
    if a.cat == b.cat then return not string.sort(a.name, b.name) end
    return string.sort(a.cat, b.cat);
...
function CleanUI_Vanilla_ISInventoryPane:sortByName(button)
    if self.itemSortFunc == CleanUI_Vanilla_ISInventoryPane.itemSortByNameInc then
        self.itemSortFunc = CleanUI_Vanilla_ISInventoryPane.itemSortByNameDesc;
    else
        self.itemSortFunc = CleanUI_Vanilla_ISInventoryPane.itemSortByNameInc;
    end
    self:refreshContainer();
end

function CleanUI_Vanilla_ISInventoryPane:sortByType(button)
    if self.itemSortFunc == CleanUI_Vanilla_ISInventoryPane.itemSortByCatInc then
        self.itemSortFunc = CleanUI_Vanilla_ISInventoryPane.itemSortByCatDesc;
    else
        self.itemSortFunc = CleanUI_Vanilla_ISInventoryPane.itemSortByCatInc;
    end
    self:refreshContainer();
end

function CleanUI_Vanilla_ISInventoryPane:SaveLayout(name, layout)
...
function CleanUI_Vanilla_ISInventoryPane:RestoreLayout(name, layout)
...
function CleanUI_Vanilla_ISInventoryPane:rowAt(x, y)
...
function CleanUI_Vanilla_ISInventoryPane:topOfItem(index)
	local rowCount = math.floor((self:getScrollHeight() - self.headerHgt) / self.itemHgt)
	if rowCount > 0 then
...
function CleanUI_Vanilla_ISInventoryPane:onMouseWheel(del)
...
function CleanUI_Vanilla_ISInventoryPane:selectIndex(index)
    ...
    if not instanceof(listItem, "InventoryItem") and not self.collapsed[listItem.name] then
        for i=2,#listItem.items do
            ...
            if i == CleanUI_Vanilla_ISInventoryPane.MAX_ITEMS_IN_STACK_TO_RENDER + 1 then break end
        end
    else
...
function CleanUI_Vanilla_ISInventoryPane:onMouseMoveOutside(dx, dy)
...
function CleanUI_Vanilla_ISInventoryPane:hideButtons()
    self.contextButton1:setVisible(false);
    self.contextButton2:setVisible(false);
    self.contextButton3:setVisible(false);
end

function CleanUI_Vanilla_ISInventoryPane:doButtons(y)
...
function CleanUI_Vanilla_ISInventoryPane:toggleStove()
    local stove = self.inventory:getParent();
    stove:Toggle();
    return stove:Activated();
end

function CleanUI_Vanilla_ISInventoryPane:sortItemsByType(items)
...
function CleanUI_Vanilla_ISInventoryPane:sortItemsByWeight(items)
	table.sort(items, function(a,b)
		if a:getContainer() and a:getContainer() == b:getContainer() and a:getDisplayName() == b:getDisplayName() then
			return a:getContainer():getItems():indexOf(a) < b:getContainer():getItems():indexOf(b)
		end
...
function CleanUI_Vanilla_ISInventoryPane:sortItemsByTypeAndWeight(items)
	local indexMap = {}
	local containers = {}
	local allIndexMap = {}
...
function CleanUI_Vanilla_ISInventoryPane:transferItemsByWeight(items, container)
	local playerObj = getSpecificPlayer(self.player)
	if true then
...
function CleanUI_Vanilla_ISInventoryPane:removeAll(player)
	...
	local modal = ISModalDialog:new(x,y, width, height, getText("IGUI_ConfirmDeleteItems"), true, self, CleanUI_Vanilla_ISInventoryPane.onConfirmDelete, player);
	modal:initialise()
...
function CleanUI_Vanilla_ISInventoryPane:onConfirmDelete(button)
...
function CleanUI_Vanilla_ISInventoryPane:lootAll()
	local playerObj = getSpecificPlayer(self.player)
...
function CleanUI_Vanilla_ISInventoryPane:transferAll()
	local playerObj = getSpecificPlayer(self.player)
...
function CleanUI_Vanilla_ISInventoryPane:onMouseMove(dx, dy)
...
function CleanUI_Vanilla_ISInventoryPane:updateTooltip()
...
--************************************************************************--
--** CleanUI_Vanilla_ISInventoryPane:onMouseUpOutside
--**
--************************************************************************--
function CleanUI_Vanilla_ISInventoryPane:onMouseDownOutside(x, y)
	self.dragging = nil;
...
function CleanUI_Vanilla_ISInventoryPane:onMouseUpOutside(x, y)
...
function CleanUI_Vanilla_ISInventoryPane.getActualItems(items)
	local ret = {}
	local contains = {}
	for _,item in ipairs(items) do
...
function CleanUI_Vanilla_ISInventoryPane:doContextualDblClick(item)
	local playerObj = getSpecificPlayer(self.player);
...
function CleanUI_Vanilla_ISInventoryPane:onMouseDoubleClick(x, y)
...
function CleanUI_Vanilla_ISInventoryPane:onMouseUp(x, y)
	if self.player ~= 0 then return end
    ...
    if ISMouseDrag.dragging ~= nil and ISMouseDrag.draggingFocus ~= self and ISMouseDrag.draggingFocus ~= nil then
        if getCore():getGameMode() ~= "Tutorial" then
            if self:canPutIn() then
                local doWalk = true
                local items = {}
                local dragging = CleanUI_Vanilla_ISInventoryPane.getActualItems(ISMouseDrag.dragging)
...
function CleanUI_Vanilla_ISInventoryPane:canPutIn()
    local playerObj = getSpecificPlayer(self.player)
...
CleanUI_Vanilla_ISInventoryPaneDraggedItems = {}
local DraggedItems = CleanUI_Vanilla_ISInventoryPaneDraggedItems
...
function DraggedItems:update()
    ...
    if not self.items then
        self.items = CleanUI_Vanilla_ISInventoryPane.getActualItems(ISMouseDrag.dragging)
...
function CleanUI_Vanilla_ISInventoryPane:doJoypadExpandCollapse()
...
function CleanUI_Vanilla_ISInventoryPane:doGrabOnJoypadSelected()
...
function CleanUI_Vanilla_ISInventoryPane:doContextOnJoypadSelected()
...
function CleanUI_Vanilla_ISInventoryPane:onRightMouseUp(x, y)
...
function CleanUI_Vanilla_ISInventoryPane:onMouseDown(x, y)
...
function CleanUI_Vanilla_ISInventoryPane:getScrollAreaHeight()
	return self:getHeight() - self.headerHgt
end

function CleanUI_Vanilla_ISInventoryPane:updateSmoothScrolling()
...
function CleanUI_Vanilla_ISInventoryPane:isMouseOverScrollBar()
	return self:isVScrollBarVisible() and self.vscroll:isMouseOver()
end

--************************************************************************--
--** CleanUI_Vanilla_ISInventoryPane:prerender
--**
--************************************************************************--
function CleanUI_Vanilla_ISInventoryPane:prerender()
...
function CleanUI_Vanilla_ISInventoryPane:rendericons()
	local xpad = 10;
	local ypad = 10;
	local iw = 40;
	local ih = 40;
...
function CleanUI_Vanilla_ISInventoryPane:update()
...
function CleanUI_Vanilla_ISInventoryPane:saveSelection(selected)
...
function CleanUI_Vanilla_ISInventoryPane:restoreSelection(selected)
    local row = 1
...
function CleanUI_Vanilla_ISInventoryPane:refreshContainer()
    self.itemslist = {}
    self.itemindex = {}
...
CleanUI_Vanilla_ISInventoryPane.highlightItem = nil;
function CleanUI_Vanilla_ISInventoryPane:renderdetails(doDragged)
               ...
               -- print("trace:g");
                ...
                if self.selected[y+1] ~= nil and not self.highlightItem then -- clicked/dragged item
                    ...
                    --(item:getModData().teachedRecipe ~= nil and player:getKnownRecipes():contains(item:getModData().teachedRecipe)))) then
                        --self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.3, CleanUI_Vanilla_ISInventoryPane.ghc:getR(), CleanUI_Vanilla_ISInventoryPane.ghc:getG(), CleanUI_Vanilla_ISInventoryPane.ghc:getB());
					--else
                ...
                elseif self.mouseOverOption == y+1 and not self.highlightItem then -- called when you mose over an element
                    ...
                    --(item:getTeachedRecipes() ~= nil and player:getKnownRecipes():containsAll(item:getTeachedRecipes())) or
                    --(item:getModData().teachedRecipe ~= nil and player:getKnownRecipes():contains(item:getModData().teachedRecipe)))) then
                    --    --self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.15, CleanUI_Vanilla_ISInventoryPane.ghc:getR(), CleanUI_Vanilla_ISInventoryPane.ghc:getG(), CleanUI_Vanilla_ISInventoryPane.ghc:getB());
					--else
                    ...
                    if count == 1 then -- normal background (no selected, no dragging..)
                        ...
                        if self.highlightItem and self.highlightItem == item:getType() then
                            ...
                            --(item:getModData().teachedRecipe ~= nil and player:getKnownRecipes():contains(item:getModData().teachedRecipe)))) then
                            --    if alt then
                            --        --self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.1, CleanUI_Vanilla_ISInventoryPane.ghc:getR(), CleanUI_Vanilla_ISInventoryPane.ghc:getG(), CleanUI_Vanilla_ISInventoryPane.ghc:getB());
                            --    else
                            --        --self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.05, CleanUI_Vanilla_ISInventoryPane.ghc:getR(), CleanUI_Vanilla_ISInventoryPane.ghc:getG(), CleanUI_Vanilla_ISInventoryPane.ghc:getB());
                            --    end
                            --else
                                if alt then
...
--                 end
            ...
            if count == CleanUI_Vanilla_ISInventoryPane.MAX_ITEMS_IN_STACK_TO_RENDER + 1 then
                break
            end
            count = count + 1;
           -- print("trace:zz");
...
function CleanUI_Vanilla_ISInventoryPane:isLiteratureRead(playerObj, item)
    if not item then return false end
...
function CleanUI_Vanilla_ISInventoryPane:getProgressBarWidth()
    return 200
end

function CleanUI_Vanilla_ISInventoryPane:getProgressBarHeight()
    local height = 5
...
function CleanUI_Vanilla_ISInventoryPane:drawProgressBar(x, y, w, h, f, fg)
...
function CleanUI_Vanilla_ISInventoryPane:drawTextAndProgressBar(text, fraction, xoff, top, fgText, fgBar)
...
function CleanUI_Vanilla_ISInventoryPane:drawItemDetails(item, y, xoff, yoff, red)
...
function CleanUI_Vanilla_ISInventoryPane:doWorldObjectHighlight(_item)
...
function CleanUI_Vanilla_ISInventoryPane:clearWorldObjectHighlights()
...
function CleanUI_Vanilla_ISInventoryPane:findItemForWorldObjectHighlight(_itemTest)
    if _itemTest then
...
function CleanUI_Vanilla_ISInventoryPane:updateWorldObjectHighlight()
    --reset highlights
...
function CleanUI_Vanilla_ISInventoryPane:render()
...
function CleanUI_Vanilla_ISInventoryPane:setMode(mode)
    self.mode = mode;

end

function CleanUI_Vanilla_ISInventoryPane:onInventoryFontChanged()
...
function CleanUI_Vanilla_ISInventoryPane:setItemsToHighlight(owner, itemTable)
    if itemTable then
...
--************************************************************************--
--** CleanUI_Vanilla_ISInventoryPane:new
--**
--************************************************************************--
function CleanUI_Vanilla_ISInventoryPane:new (x, y, width, height, inventory, zoom)
	local o = {}
	--o.data = {}
    ...
    o.itemSortFunc = CleanUI_Vanilla_ISInventoryPane.itemSortByNameInc; -- how to sort the items...
