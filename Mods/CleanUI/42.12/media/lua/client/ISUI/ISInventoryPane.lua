...
function ISInventoryPane:new (x, y, width, height, inventory, zoom)
    ...
    o.favoriteRecipeYes = getTexture("media/ui/CleanUI/ICON/FavouriteYes.png")
    o.favoriteRecipeNo = getTexture("media/ui/CleanUI/ICON/FavouriteNo.png")
    ...
    for i = 0, it:size()-1 do
        ...
        if add and self.parent.onCharacter then
            ...
            if hideEquipped then
                if isEquipped[item] and not item:hasTag("Generator") then
                    add = false
                ...
                if tex ~= nil then
                    ...
                    if count == 1  then
                        ...
                        if item:isNoRecipes(player) then
                            self:drawTextureScaled(self.favoriteRecipeNo, texOffsetX, texOffsetY+auxDXY, favoriteRecipeInputStarSize, favoriteRecipeInputStarSize, 1, 0.8, 0.8, 0.8)
                        elseif item:isFavouriteRecipeInput(player) then
                            self:drawTextureScaled(self.favoriteRecipeYes, texOffsetX, texOffsetY+auxDXY, favoriteRecipeInputStarSize, favoriteRecipeInputStarSize, 1, 0.8, 0.2, 0.2)
                        end
                ...
                if count == 1 then
					...
					if doDragged and ISMouseDrag.dragging and #ISMouseDrag.dragging > 0 then
                        ...
                        if clipX < clipX2 and clipY < clipY2 then
                            ...
                            local favoriteIconSize = math.floor(self.fontHgt * 0.8)
               ...
               -- print("trace:j")
                ...
                if item:getJobDelta() > 0  then
                    if  (count > 1 or self.collapsed[v.name]) then
                        ...
                        local truncatedJob = CleanUI.truncateText(jobText, maxJobWidth, self.font)
