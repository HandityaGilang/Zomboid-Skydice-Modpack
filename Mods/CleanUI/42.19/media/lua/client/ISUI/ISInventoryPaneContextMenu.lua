...
require "CleanUI/CleanUI_FixingHelper"

local function CleanUI_safeSplitIcon(texture)
    -- Safely split context-menu textures. Some modded items can expose
    -- wearable actions without a valid texture, which would otherwise break
    -- the whole right-click menu before it is shown.
    if not texture then
        return nil
    end

    local ok, splitIcon = pcall(function()
        return texture:splitIcon()
    end)
    ...
    print("[CleanUI] Failed to split context-menu icon texture: " .. tostring(splitIcon))
    return nil
end

local function CleanUI_safeTriggerInventoryContextEvent(eventName, ...)
    -- Guard context menu extension hooks from other mods so one bad listener
    -- does not break the whole right-click menu in CleanUI's override path.
    local ok, err = pcall(triggerEvent, eventName, ...)
    if not ok then
        print("[CleanUI] triggerEvent failed for " .. tostring(eventName) .. ": " .. tostring(err))
    end
    return ok
end
...
local function CleanUI_reportEvolvedRecipeRenameError(item, err)
    -- Report each item/error pair once so repeated right-clicks do not spam the console.
    local itemName = "unknown"
    if item then
        local okName, fullType = pcall(function()
            return item:getFullType()
        end)
        if okName and fullType then
            itemName = tostring(fullType)
        end
    end

    local key = itemName .. "|" .. tostring(err)
    if CleanUI_reportedEvolvedRecipeRenameErrors[key] then
        return
    end

    CleanUI_reportedEvolvedRecipeRenameErrors[key] = true
    print("[CleanUI] Skipped evolved recipe rename test for " .. itemName .. ": " .. tostring(err))
end

local function CleanUI_canRenameEvolvedFood(testItem)
    -- Keep the right-click menu alive when a modded food item or evolved recipe
    -- throws while testing the vanilla rename eligibility.
    if not testItem or not instanceof(testItem, "Food") then
        return false
    end

    local okExtraItems, hasEnoughExtraItems = pcall(function()
        return testItem:haveExtraItems() and testItem:getExtraItems() and testItem:getExtraItems():size() >= 3
    end)

    if not okExtraItems then
        CleanUI_reportEvolvedRecipeRenameError(testItem, hasEnoughExtraItems)
        return false
    end

    if not hasEnoughExtraItems then
        return false
    end

    local okRecipes, evolvedRecipes = pcall(getEvolvedRecipes)
    if not okRecipes or not evolvedRecipes then
        if not okRecipes then
            CleanUI_reportEvolvedRecipeRenameError(testItem, evolvedRecipes)
        end
        return false
    end

    for i = 0, evolvedRecipes:size() - 1 do
        local evoRecipeTest = evolvedRecipes:get(i)
        if evoRecipeTest then
            local okRecipe, isResultItem = pcall(function()
                return evoRecipeTest:isResultItem(testItem)
            end)

            if okRecipe and isResultItem then
                return true
            end

            if not okRecipe then
                CleanUI_reportEvolvedRecipeRenameError(testItem, isResultItem)
            end
        end
    end
...
ISInventoryPaneContextMenu.createMenu = function(player, isInPlayerInventory, items, x, y, origin)
    ...
    for i,v in ipairs(items) do
        ...
        if CleanUI_canRenameEvolvedFood(testItem) then
    ...
    CleanUI_safeTriggerInventoryContextEvent("OnPreFillInventoryObjectContextMenu", player, context, items);
    ...
    if tests.brokenObject then
        -- Build the repair menu without calling FixingManager.getFixes(), which can crash
        -- when another mod registers a fixing entry without a valid Require list.
        local fixingList = CleanUI_FixingHelper.getSafeFixes(tests.brokenObject)

        if fixingList ~= nil and not fixingList:isEmpty() then
            ...
            if #subMenuFix.options > 0 then
                local fixOption = context:addOption(
                    getText("ContextMenu_Repair") .. " " ..
                    getItemNameFromFullType(tests.brokenObject:getFullType()),
                    items,
                    nil
                )
    ...
    CleanUI_safeTriggerInventoryContextEvent("OnFillInventoryObjectContextMenu", player, context, items);
...
function ISInventoryPaneContextMenu.doRecipeListForItem(context, text, recipeItem, playerObj)
    ...
    local iconTexture = CleanUI_safeSplitIcon(getTexture("media/ui/Sidebar/80/Carpentry_Off_80.png"))
    if iconTexture then
...
ISInventoryPaneContextMenu.doWearClothingMenu = function(player, clothing, items, context)
    ...
    if item then
        local iconTexture = CleanUI_safeSplitIcon(item:getTex())
        if iconTexture then
            option.iconTexture = iconTexture
        end
        ...
        if color then
            option.color = { r = color:getR(), g = color:getG(), b = color:getB()}
        end
    end
...
function ISInventoryPaneContextMenu.transferIfNeeded(playerObj, item, preventTransferWorldObjects)
    -- CleanUI: clear stale retrigger state before queuing inventory transfer actions.
    -- This keeps read/craft/eat paths from inheriting an old physical-action relaunch state.
    if ISInventoryPaneContextMenu.cleanUIResetStaleTimedActionRetrigger then
        ISInventoryPaneContextMenu.cleanUIResetStaleTimedActionRetrigger(playerObj, true, "transferIfNeeded")
    end
...
ISInventoryPaneContextMenu.onDrinkFluid = function(item, percent, playerObj, openingRecipe, realItem)
	ISInventoryPaneContextMenu.cleanUIResetStaleEatingRetrigger(playerObj, true)
    ...
    if not (eatPercentage and openingRecipe) then
        -- Return the original item only when it still exists after the action.
...
ISInventoryPaneContextMenu.onFix = function(brokenObject, player, fixingNum, fixerNum, vehiclePart)
    ...
    local fixing = CleanUI_FixingHelper.getSafeFixing(brokenObject, fixingNum);
    if not fixing then return end;
    local fixer = fixing:getFixers():get(fixerNum)
    if not fixer then return end;
    local items = fixing:getRequiredItems(playerObj, fixer, brokenObject);
    if not items then return end;
    ISInventoryPaneContextMenu.transferIfNeeded(playerObj, items);
    if vehiclePart then
        local action = ISFixVehiclePartAction:new(playerObj, vehiclePart, fixingNum, fixerNum)
        if action then
            ISTimedActionQueue.add(action);
        end
    else
        local action = ISFixAction:new(playerObj, brokenObject, fixingNum, fixerNum)
        if action then
            ISTimedActionQueue.add(action);
        end
    end
    ...
    if button.internal == "OK" then
        ...
        if button.parent.entry:getText() and button.parent.entry:getText() ~= "" then
			if length <= MAXIMUM_RENAME_LENGTH then 
                ...
                -- Sync renamed food fields so MP and refreshed UI stay consistent on newer builds.
                item:syncItemFields();
...
function CraftTooltip:layoutContents(x, y)
    ...
    -- B42.20 groups duplicate recipe inputs in the tooltip; keep the original order for readability.
    local fullList = {}
    local fullOrder = {}
    ...
    for i=0,items:size()-1 do
        ...
        if item then
            ...
            if not fullList[fullType] then
                fullList[fullType] = { item = item, count = 1 }
                table.insert(fullOrder, fullType)
            else
                fullList[fullType].count = fullList[fullType].count + 1
            end
        end
    end

    for _, fullType in ipairs(fullOrder) do
        local entry = fullList[fullType]
        local x2 = x1
        self:addImage(x2, y1 + imageDY, entry.item:getTex():getName(), 1.0, 1.0, 1.0)
        x2 = x2 + IMAGE_SIZE + 6
        self:addText(x2, y1 + textDY, entry.item:getName() .. (entry.count > 1 and " x" .. entry.count or ""))
        y1 = y1 + lineHeight
    end
...
ISInventoryPaneContextMenu.addNewCraftingDynamicalContextMenu = function(selectedItem, context, recipeList, player, containerList)
    ...
    if recipeList:size() > 1 then
        ...
        local iconTexture = CleanUI_safeSplitIcon(selectedItem:getIcon())
        if iconTexture then
            option.iconTexture = iconTexture
        end
        local subMenu = context:getNew(context);
...
-- CleanUI: guard against stale timed-action retrigger state.
-- Physical/context actions can leave IsoPlayer.timedActionToRetrigger pointing at
-- an action that should not be relaunched before a new inventory action starts.
-- B42.17 includes an engine-side cleanup for one self-reference case; this Lua
-- guard keeps CleanUI-triggered inventory actions safer across supported branches.
ISInventoryPaneContextMenu.cleanUINotifyTimedActionFix = function(playerObj, reason)
    -- Show an optional diagnostic Halo Note only when enabled in CleanUI Mod Options.
    if not playerObj then return end
    if type(CleanUI_shouldNotifyTimedActionFix) ~= "function" or not CleanUI_shouldNotifyTimedActionFix() then
        return
    end
    if type(HaloTextHelper) ~= "table" or type(HaloTextHelper.addText) ~= "function" then
        return
    end
    ...
    -- Avoid spamming Halo Notes when a single menu action moves several items.
    local last = ISInventoryPaneContextMenu.cleanUITimedActionFixLastHaloTime or 0
    if now > 0 and now - last < 2500 then
        return
    end
    ISInventoryPaneContextMenu.cleanUITimedActionFixLastHaloTime = now

    local text = getText("UI_CleanUI_TimedActionFixHalo")
    HaloTextHelper.addText(playerObj, text, "", HaloTextHelper.getColorWhite())

    if type(print) == "function" then
        print("[CleanUI] Timed-action guard applied" .. (reason and (" (" .. tostring(reason) .. ")") or ""))
    end
end

ISInventoryPaneContextMenu.cleanUIResetStaleTimedActionRetrigger = function(playerObj, force, reason)
    if not playerObj then return false end

    local okGet, actionToRetrigger = pcall(function()
        return playerObj:getTimedActionToRetrigger()
    end)
    if not okGet or not actionToRetrigger then return false end

    local queue = ISTimedActionQueue and ISTimedActionQueue.queues and ISTimedActionQueue.queues[playerObj]
    local current = queue and queue.current

    -- Match the B42.17 engine-side cleanup, or force-clear before CleanUI starts
    -- a new explicit inventory action such as transfer/read/craft/eat.
    if force or (current and current.action == actionToRetrigger) then
        local okSet = pcall(function()
            playerObj:setTimedActionToRetrigger(nil)
        end)
        if okSet then
            ISInventoryPaneContextMenu.cleanUINotifyTimedActionFix(playerObj, reason)
            return true
        end
    end
...
ISInventoryPaneContextMenu.cleanUIResetStaleEatingRetrigger = function(playerObj, force)
    -- Backward-compatible alias for the 2.5.15 eating-action hotfix calls.
    return ISInventoryPaneContextMenu.cleanUIResetStaleTimedActionRetrigger(playerObj, force, "eating")
end
...
ISInventoryPaneContextMenu.dropItem = function(item, player)
    ...
    if playerObj:isHandItem(item) then
        -- Keep old B42.16/B42.17 compatibility through isDropping while also passing
        -- the B42.18+ reason parameter so forced heavy items are handled as drops.
        local action = ISUnequipAction:new(playerObj, item, 1, "drop")
...
ISInventoryPaneContextMenu.doClothingItemExtraMenu = function(context, clothingItemExtra, playerObj)
    if (clothingItemExtra:IsClothing() or clothingItemExtra:IsInventoryContainer()) and clothingItemExtra:getClothingExtraSubmenu() then
        ...
        local iconTexture = CleanUI_safeSplitIcon(clothingItemExtra:getTex())
        if iconTexture then
            option.iconTexture = iconTexture
        end
        local subMenu = context:getNew(context);
    ...
    for i=0,clothingItemExtra:getClothingItemExtraOption():size()-1 do
        ...
        -- Keep the clothing extra option visible even if the preview item is missing.
        -- Some mods can reference an item type that is not currently loaded, which would
        -- make getItemInstance() return nil and crash the comparison tooltip builder.
        if item then
...
ISInventoryPaneContextMenu.getEatingMask = function( playerObj, removeMask )
    ...
    if mask and removeMask then
        ISInventoryPaneContextMenu.cleanUIResetStaleEatingRetrigger(playerObj)
