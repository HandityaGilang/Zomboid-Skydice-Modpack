...
function VehicleDashboardReplacer:prerender()
	...
	local engineVehiclePart = self.vehicle:getPartById("Engine") -- Because weird unimog RV thing without an engine... 
	...
	local oilLevel = 0;
	local oilPan = engine and engine:getItemContainer():getFirstTag("EngineOilPan")
	...
	if self.vehicle:isEngineRunning() or self.vehicle:isKeysInIgnition() or self.vehicle:isStarting() then
		...
		local fanBelt = GetPartCondition(engine,"EngineFanBelt") * 0.01;
		local alternator = GetPartCondition(engine,"EngineAlternator") * 0.01;
