...
function ISVehicleMechanics:doPartContextMenu(part, x,y)
	...
	if part:getId() == "Engine" then
		...
		if self.vehicle:getPartById("Engine") then
			...
			--if self:CanCheat() then
			if ISVehicleMechanics.cheat and ((getDebug() and not isClient()) or getSpecificPlayer(self.playerNum):getRole():hasAdminPower()) then
...
-- disable mechanics cheat
--ISEngineMechanics.cheat = false; --getDebug(); now using ISVehicleMechanics.cheat
	...
	for i=1,self.vehicle:getPartCount() do
		...
		local category = part:getCategory() or "Other";
			if part:getId() == "Engine" then
				...
				newPart.tag = PSC_Tags.EngineSparkplug
				newPart.info = getText("Tooltip_EngineSparkplug") 
				newPart.skill = 2
				table.insert(currentCat.parts, newPart);

				newPart = {};
				newPart.name = getText("IGUI_EnginePanel_PartType_CylinderHead");
				newPart.tag = PSC_Tags.EngineCylinderHead
				newPart.info = getText("Tooltip_EngineCylinderHead") 
				newPart.skill = 7
				table.insert(currentCat.parts, newPart);

				newPart = {};
				newPart.name = getText("IGUI_EnginePanel_PartType_HeadGasket");
				newPart.tag = PSC_Tags.EngineHeadGasket
				newPart.info = getText("Tooltip_EngineHeadGasket") 
				newPart.skill = 7
				table.insert(currentCat.parts, newPart);
				...
				newPart.tag = PSC_Tags.EnginePistons
				newPart.info = getText("Tooltip_EngineHeadGasket") 
				newPart.skill = 7
				table.insert(currentCat.parts, newPart);
				...
				newPart.tag = PSC_Tags.EngineCrankshaft
				newPart.info = getText("Tooltip_EngineCrankshaft") 
				newPart.skill = 7
				table.insert(currentCat.parts, newPart);

				newPart = {};
				newPart.name = getText("IGUI_EnginePanel_PartType_Flywheel");
				newPart.tag = PSC_Tags.EngineFlywheel
				newPart.info = getText("Tooltip_EngineFlywheel") 
				newPart.skill = 5
				table.insert(currentCat.parts, newPart);

				newPart = {};
				newPart.name = getText("IGUI_EnginePanel_PartType_Starter");
				newPart.tag = PSC_Tags.EngineStarter
				newPart.info = getText("Tooltip_EngineStarter") 
				newPart.skill = 4
				table.insert(currentCat.parts, newPart);
				...
				newPart.tag = PSC_Tags.EngineTransmission
				newPart.info = getText("Tooltip_EngineTransmission") 
				newPart.skill = 5
				table.insert(currentCat.parts, newPart);

				newPart = {}; 
				newPart.name = getText("IGUI_EnginePanel_PartType_TorqueConverter");
				newPart.tag = PSC_Tags.EngineTorqueConverter
				newPart.info = getText("Tooltip_EngineTorqueConverter") 
				newPart.skill = 5
				table.insert(currentCat.parts, newPart);
				...
				newPart.tag = PSC_Tags.EnginePowerSteeringPump
				...
				newPart.tag = PSC_Tags.EngineOilPan
				...
				newPart.tag = PSC_Tags.EngineOilPan
				newPart.info = getText("Tooltip_EngineOilPan") 
				newPart.skill = 3
				table.insert(currentCat.parts, newPart);

				newPart = {};
				newPart.name = getText("IGUI_EnginePanel_PartType_OilFilter");
				newPart.tag = PSC_Tags.EngineOilFilter
				newPart.info = getText("Tooltip_EngineOilFilter") 
				newPart.skill = 1
				table.insert(currentCat.parts, newPart);
				...
				newPart.tag = PSC_Tags.EngineRadiator
				...
				newPart.tag = PSC_Tags.EngineRadiator
				newPart.info = getText("Tooltip_EngineRadiator") 
				newPart.skill = 2
				table.insert(currentCat.parts, newPart);
				...
				newPart.tag = PSC_Tags.EngineFanBelt;
				newPart.info = getText("Tooltip_EngineFanBelt") 
				newPart.skill = 2
				table.insert(currentCat.parts, newPart);

				newPart = {};
				newPart.name = getText("IGUI_EnginePanel_PartType_Alternator");
				newPart.tag = PSC_Tags.EngineAlternator;
				newPart.info = getText("Tooltip_EngineAlternator") 
				newPart.skill = 4
				table.insert(currentCat.parts, newPart);

				newPart = {};
				newPart.name = getText("IGUI_EnginePanel_PartType_WaterPump");
				newPart.tag = PSC_Tags.EngineWaterPump;
				newPart.info = getText("Tooltip_EngineWaterPump") 
				newPart.skill = 4
				table.insert(currentCat.parts, newPart);
				...
				newPart.tag = PSC_Tags.EnginePowerSteeringPump;
				newPart.info = getText("Tooltip_EnginePowerSteeringPump") 
				newPart.skill = 4
				table.insert(currentCat.parts, newPart);
				...
				newPart.tag = PSC_Tags.EngineBrakeBooster;
				newPart.info = getText("Tooltip_EngineBrakeBooster") 
				newPart.skill = 3
				table.insert(currentCat.parts, newPart);

				newPart = {}; 
				newPart.name = getText("IGUI_EnginePanel_PartType_AirConditioner");
				newPart.tag = PSC_Tags.EngineAirConditioner;
				newPart.info = getText("Tooltip_EngineAirConditioner") 
				newPart.skill = 4
				table.insert(currentCat.parts, newPart);

				newPart = {}; 
				newPart.name = getText("IGUI_EnginePanel_PartType_HeaterCore");
				newPart.tag = PSC_Tags.EngineHeaterCore;
				newPart.info = getText("Tooltip_EngineHeaterCore") 
				newPart.skill = 4
				table.insert(currentCat.parts, newPart);
...
function ISEngineMechanics:doPartContextMenu(item, x,y)
		...
		if item.part == nil then
			...
			if partList:size() ~= 0 then
				for i=0, partList:size() - 1 do
					...
					itemOpt.notAvailable = (ISEngineMechanics.getWrench(playerObj) == nil) and not self:CanCheat() or (curPart:getMechanicType() ~= self.vehicle:getScript():getMechanicType())
			...
			option.notAvailable = (ISEngineMechanics.getWrench(playerObj) == nil) and not self:CanCheat()
	...
	--if (getDebug() and ISVehicleMechanics.cheat and not isClient()) or isAdmin() then
	...
	if (getDebug() and not isClient()) or getSpecificPlayer(self.playerNum):getRole():hasAdminPower() then
		if ISVehicleMechanics.cheat then
			self.context:addOption("DBG: ISVehicleMechanics.cheat=false", playerObj, ISEngineMechanics.onCheatToggle)
		else
			self.context:addOption("DBG: ISVehicleMechanics.cheat=true", playerObj, ISEngineMechanics.onCheatToggle)
		end
	end
...
function ISEngineMechanics.onAddFluid(playerObj, item, newPart, engineMechanics)
	...
	if not engineMechanics:CanCheat() then
	...
	if engineCover and not engineMechanics:CanCheat() then
		ISTimedActionQueue.add(ISOpenVehicleDoor:new(playerObj, engineMechanics.vehicle, engineCover))
		ISTimedActionQueue.add(ISTransferEngineFluid:new(playerObj, newPart, item.part, false));
...
function ISEngineMechanics.onRemoveFluid(playerObj, item, newPart, engineMechanics)
	...
	if engineCover and not engineMechanics:CanCheat() then
		ISTimedActionQueue.add(ISOpenVehicleDoor:new(playerObj, engineMechanics.vehicle, engineCover))
		ISTimedActionQueue.add(ISTransferEngineFluid:new(playerObj, item.part, newPart, true));
		--ISTimedActionQueue.add(ISCloseVehicleDoor:new(playerObj, part:getVehicle(), engineCover))
	else
		ISTimedActionQueue.add(ISTransferEngineFluid:new(playerObj, item.part, newPart, true));
	end	
...
function ISEngineMechanics.onAddPart(playerObj, item, newPart, engineMechanics)
	...
	ISTimedActionQueue.add(ISInstallEnginePart:new(playerObj, engineMechanics.vehicle, item.skill, newPart))

--[[
	local wrench = ISEngineMechanics.getWrench(playerObj)
	if (wrench == nil and not ISVehicleMechanics.cheat) then return end
...
function ISEngineMechanics.onRemovePart(playerObj, item, engineMechanics)
	...
	if engineCover and not engineMechanics:CanCheat() then
		ISTimedActionQueue.add(ISOpenVehicleDoor:new(playerObj, engineMechanics.vehicle, engineCover))
		ISTimedActionQueue.add(ISUninstallEnginePart:new(playerObj, engineMechanics.vehicle, item.part, item.skill))
...
function ISEngineMechanics:onRightMouseUp(x, y)
	...
	if not part and ((getDebug() and not isClient()) or getSpecificPlayer(self.playerNum):getRole():hasAdminPower())then
...
function ISEngineMechanics:close()
	...
	-- reopen mechanic menu. 
	local ui = getPlayerMechanicsUI(self.chr:getPlayerNum());
	ui.vehicle = self.vehicle;
	ui.usedHood = self.usedHood
	ui:initParts();
	ui:setVisible(true, JoypadState.players[self.chr:getPlayerNum()+1])
	ui:addToUIManager()
