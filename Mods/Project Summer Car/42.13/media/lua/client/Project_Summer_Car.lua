...
function RealisticStartingUpdate()
	...
	if vehicle then
		...
		if vehicle:isStarting() then
			...
			starter = engine:getItemContainer():getFirstTag(PSC_Tags.EngineStarter)
...
function Commands.UpdateFluid(args)
	--print("Received updateFluid with args ", args);
	...
	local item = itemContainer:getItemWithID(args.partID);
	local fluidContainer = item:getFluidContainer();
	fluidContainer:removeFluid();
	-- Next, parse args.fluidArray
	for index,value in ipairs(args.fluidArray) do
		fluidContainer:addFluid(value.fluidType,value.amount);
	end
end

VehicleCommands.OnServerCommand = function(module, command, args)
...
Events.OnServerCommand.Add(VehicleCommands.OnServerCommand)
